...
#define SYSLNK_MAX_TEST_DEPTH (strlen("../../devices/soc/1c1b000.ehci1-controller/usbx/x-x/x-x:x.x"))
...
static int getNodeBySysLnk(char *syslink, char *outNode) {
    ...
    while ((dirp = readdir(dp)) != NULL) {
        ...
        if (S_ISLNK(file_stat.st_mode)) {
            ...
            //比较其设备路径和设备号
            const char *syslink_name = strrchr(syslink, '/');
            const char *tmplink_name = strrchr(tmplink, '/');
            if(strncmp(syslink, tmplink, SYSLNK_MAX_TEST_DEPTH) == 0
                    && strncmp(syslink_name, tmplink_name, strlen(syslink_name)) == 0){
...
static int testFile(char *relative_path, char *md5) {
    ...
    int ret = do_md5(full_path, file_md5);
...
int updateFromMutliDevice(const char *opt , char *outpath) {
    ...
    if (testFile(relative_path, md5) == 0) {
        fprintf(stderr,"This is not the correct file.\n");
        return 0;
    }
