...
struct file_data {
    int fd;  // the underlying internal or external file
...
static int read_block_file(void* cookie, uint32_t block, uint8_t* buffer, uint32_t fetch_size) {
    ...
    if (TEMP_FAILURE_RETRY(lseek64(fd->fd, offset, SEEK_SET)) == -1) {
        fprintf(stderr, "seek on internal or external failed: %s\n", strerror(errno));
        return -EIO;
    }
    ...
    while (fetch_size > 0) {
        ...
        if (r == -1) {
            fprintf(stderr, "read on internal or external failed: %s\n", strerror(errno));
            return -EIO;
        }
        fetch_size -= r;
        buffer += r;
...
bool start_fuse_core(const char* path, const char* unmount_path){
    struct stat sb;
    if (stat(path, &sb) == -1) {
    ...
    // The installation process expects to find the internal or external unmounted.
    // Unmount it with MNT_DETACH so that our open file continues to
    // work but new references see it as unmounted.
    umount2("/data/media/0", MNT_DETACH);
