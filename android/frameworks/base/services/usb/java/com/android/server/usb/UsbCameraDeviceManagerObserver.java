...
public class UsbCameraDeviceManagerObserver extends UEventObserver {
    private static final String TAG = "UsbCameraDeviceManagerObserver";
    ...
    public void onUEvent(UEventObserver.UEvent event) {
        ...
        try{
            ...
            Log.d(TAG, "UsbCameraDeviceManagerObserver android.hardware.Camera.getNumberOfCameras() = " + android.hardware.Camera.getNumberOfCameras());

            if(action.equals("add")){
                mUsbCameraTotalNumber++;
                Log.d(TAG,"action.equals(add) mUsbCameraTotalNumber = " +mUsbCameraTotalNumber);
                updateState(devName,mUsbCameraTotalNumber,UsbCameraManager.PLUG_IN);
            }else if(action.equals("remove")){
                if(mUsbCameraTotalNumber>0){
                    mUsbCameraTotalNumber--;
                }
                Log.d(TAG,"action.equals(remove) mUsbCameraTotalNumber = " +mUsbCameraTotalNumber); 
