...
//#define LOG_NDEBUG 0
#define LOG_TAG "subtitleNativeDisplayNew"
#include <cdx_log.h>
...
static SkColorType convertPixelFormat(PixelFormat ePixelFormat)
{
    switch (ePixelFormat)
    {
...
{
    SkImageInfo info = SkImageInfo::Make(sub_info->subPicWidth, sub_info->subPicHeight,
...
int CedarXSub::startRender()
{
    ...
    err = surface->lock(&nativeWindowBuffer, NULL/*false*/);
    if (err < 0)
    {
    ...
    SkBitmap bitmap;
    SkImageInfo info = SkImageInfo::Make(nativeWindowBuffer.width,
            nativeWindowBuffer.height,
            convertPixelFormat(nativeWindowBuffer.format),
            kOpaque_SkAlphaType);
    bitmap.setInfo(info);
    ...
    if (nativeWindowBuffer.width > 0 && nativeWindowBuffer.height > 0)
    {
        ...
        bitmap.setPixels(nativeWindowBuffer.bits);
    }
    else
    {
        // be safe with an empty bitmap.
                        ...
                        int initSetFontSize, int initSetFontStyle, int Ypercent)
{
    ...
    if(mSubMode & TEXT_SUBTITLE)
    {
        logd("CedarXSub::updatePara mText = %s\n",mText);
        if(mTextBox == NULL)
        {
