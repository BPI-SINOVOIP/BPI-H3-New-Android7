 ...
 *
 */
#define LOG_TAG "soundControl_android"
...
static void __SdSetFormat(SoundCtrl* s, CdxPlaybkCfg* cfg)
{
    ...
    if(sc->eStatus != SOUND_STATUS_STOPPED)
    {
        loge("Sound device not int stop status, can not set audio params. eStatus %d", sc->eStatus);
        abort();
    }
        ...
        if(sc->needDirectOutPut)
        {
            ...
            switch(sc->data_type)
            {
                ...
                case AUDIO_RAW_DATA_DTS_HD:
                    logd("cedarx passthrough mat or dts_hd");
            ...
            if(sc->data_type == AUDIO_RAW_DATA_PCM)
            {
                ...
                {
                    case 32:
                        logd("cedarx passthrough 32 bit lpcm");
                        format = AUDIO_FORMAT_PCM_32_BIT;
                        break;
                    case 24:
                        logd("cedarx passthrough 24 bit packed lpcm");
                ...
                {
                    case 6:
                        logd("cedarx passthrough 5.1 ch lpcm");
                        chanmask = AUDIO_CHANNEL_OUT_5POINT1;
                        break;
                    case 8:
                        logd("cedarx passthrough 7.1 ch lpcm");
            ...
            else
            {
                ...
                {
                    case 6:
                        logd("cedarx passthrough 5.1 ch iec stream");
                        chanmask = AUDIO_CHANNEL_OUT_5POINT1;
                        break;
                    case 8:
                        logd("cedarx passthrough 7.1 ch iec stream");
                        chanmask = AUDIO_CHANNEL_OUT_7POINT1;
                        break;
                    default:
                        break;
