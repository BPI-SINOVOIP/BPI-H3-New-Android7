...
static int SendThreeBlackFrameToGpu(ANativeWindow* pNativeWindow)
{
    ...
    for(i = 0;i < 1;i++)
    {
        ...
        pNativeWindow->perform(pNativeWindow,
                                NATIVE_WINDOW_SET_VIDEO_BUFFERS_INFO,
                                nBufAddr[0],
                                nBufAddr[1],
...
int CTC_MediaProcessorImpl::RequestBuffer(unsigned char** ppBuf, unsigned int* pSize, int MediaType)
{
    ...
    while(1)
    {
        ...
        {
            logd(" waiting for video stream buffer, nBufSize0 %d, nBufSize1 %d",
                            nBufSize0, nBufSize1);
            if (mIsQuitedRequest)
                return 0;
...
int CTC_MediaProcessorImpl::VideoShow()
{
    ...
    {
        mHoldLastPicture = true;
        PlayerSetHoldLastPicture(mPlayer, 1);
    }
...
JNIEnv* CTC_MediaProcessorImpl::getJNIEnv()
{
    if (mEnv == NULL)
    {
        JavaVM *jvm = NULL;
        ...
        VMargs.options = &VMopt;
        VMargs.nOptions = 1;
        VMargs.ignoreUnrecognized = JNI_FALSE;

        void* libdvmDso = dlopen("libdvm.so", RTLD_NOW);
        ALOGE_IF(!libdvmDso, "setVolumeJNI: %s", dlerror());

        void* libAndroidRuntimeDso = dlopen("libandroid_runtime.so", RTLD_NOW);
        ALOGE_IF(!libAndroidRuntimeDso, "setVolumeJNI: %s", dlerror());

        if (!libdvmDso || !libAndroidRuntimeDso)
        {
            ALOGE("open so failed");
            if (libAndroidRuntimeDso) {
                dlclose(libAndroidRuntimeDso);
            }
            if (libdvmDso) {
                dlclose(libdvmDso);
            }
        ...
        jint (*JNI_CreateJavaVM)(JavaVM** p_vm, JNIEnv** p_env, void* vm_args) = NULL;
        JNI_CreateJavaVM = (typeof JNI_CreateJavaVM)dlsym(libdvmDso, "JNI_CreateJavaVM");
        ALOGE_IF(!JNI_CreateJavaVM, "setVolumeJNI: %s", dlerror());

        jint (*registerNatives)(JNIEnv* mEnv, jclass clazz) = NULL;
        registerNatives = (typeof registerNatives)dlsym(libAndroidRuntimeDso,
                    "Java_com_android_internal_util_WithFramework_registerNatives");
        ALOGE_IF(!registerNatives, "setVolumeJNI: %s", dlerror());

        if (JNI_CreateJavaVM(&jvm, &mEnv, &VMargs) != 0) {
            ALOGD("JNI_CreateJavaVM failed");
            mEnv = NULL;
        } else if (registerNatives(mEnv, 0) != 0) {
            ALOGD("registerNatives failed");
            mEnv = NULL;
        }
    }
    logd("++++ mEnv: %p", mEnv);
...
int CTC_MediaProcessorImpl::setVolumeJNI(int index)
{
    ...
    if (env)
    {
        ...
        ALOGD("clazz=%p, method=%p", clazz, setVolume);
        if (clazz && setVolume)
...
int CTC_MediaProcessorImpl::GetVolume()
{
    ...
    int index;
    int device = AudioSystem::getDevicesForStream(AUDIO_STREAM_MUSIC);
    int ret = AudioSystem::getStreamVolumeIndex(AUDIO_STREAM_MUSIC, &index, device);
    logd("PlayerGetVolume, ret = %d, index = %d", ret, index);
    if(ret != 0)
    {
