...
extern int SecureMemAdapterRead(void *src, void *dest, size_t n);
extern int SecureMemAdapterCopy(void *dest, void *src, size_t n);
extern void* SecureMemAdapterGetPhysicAddressCpu(void *virt);
...
static cdx_int32 PlayReadyRead(MOVContext *c, CdxStreamT *fp, CdxPacketT *pkt)
{
    ...
    if (c->senc_data && sample_index >= 0) {
        ...
        if(pkt->type == CDX_MEDIA_VIDEO) {
            if (pkt->buf)
                SecureMemAdapterCopy(pkt->buf, opaque_buf, pkt->buflen);
            if (pkt->ringBuf)
                SecureMemAdapterCopy(pkt->ringBuf, opaque_buf + pkt->buflen, pkt->length -
                pkt->buflen);
        } else {
            if (pkt->buf)
                SecureMemAdapterRead(opaque_buf, pkt->buf, pkt->buflen);
            if (pkt->ringBuf)
                SecureMemAdapterRead(opaque_buf + pkt->buflen, pkt->ringBuf, pkt->length -
                pkt->buflen);
        }
#else
        if (pkt->buf)
