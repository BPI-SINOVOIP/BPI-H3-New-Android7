...
static int UpdateHlsParser(CdxHlsParser *hlsParser, const char *url)
{
    ...
    return RefreshPlaylist(hlsParser);
}
...
static int CallbackProcess(void* pUserData, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        case PARSER_ATTEMPT_SWITCH_BANDWIDTH:
        {
            ...
            property_get("epg.default_player_quality", value, "0");
            if(!strcasecmp(value, "0") || !strcasecmp(value, "1"))
                CDX_LOGW("cur play quality[high(0),fluent(1),auto(2)]= %s cannot switchBandwidth",value);
            else
...
static cdx_int32 HlsParserSeekTo(CdxParserT *parser, cdx_int64  timeUs)
{
    ...
    if(hlsParser->isMasterParser)
    {
        ...
        for(i = 0; i < 3; i++)
        {
            if(hlsParser->child[i])
            {
                ret = CdxParserSeekTo(hlsParser->child[i], timeUs);
                if(ret < 0)
                {
...
_next:
    ...
    else
    {
        ...
        {
            /*
               DEFINITION_HIGH_QUALITY =0  // default high_quality
               DEFINITION_HIGH_FLUENT  =1  // fluent
               DEFINITION_PLAYER_AUTO  =2  // auto
            */
            char value[PROP_VALUE_MAX];
            property_get("epg.default_player_quality", value, "0");
            CDX_LOGD("HLSinit:cur play quality value[high(0),fluent(1),auto(2)]= %s",value);
            if (strcasecmp(value, "1") == 0) //fluent_quality or low_quality
            {
                hlsParser->u.master.curBandwidthIndex = p - 1;
                logd("hls start with lowest bitrate");
            }
            else if (strcasecmp(value, "0") == 0) //high_quality
            {
                hlsParser->u.master.curBandwidthIndex = 0;
                logd("hls start with highest bitrate");
            }
            else  //auto_quality
