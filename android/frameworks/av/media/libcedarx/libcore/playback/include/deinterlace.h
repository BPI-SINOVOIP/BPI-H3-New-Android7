...
struct DeinterlaceOps
{
    ...
    int (*setVideoSize)(Deinterlace* di,
                        int w, int h);
};
                        ...
                        int nField)
{
 ...
 static inline int CdxDiSetVideoSize(Deinterlace* di, int w, int h)
{
    CDX_CHECK(di);
    CDX_CHECK(di->ops);
    CDX_CHECK(di->ops->setVideoSize);
    return di->ops->setVideoSize(di, w, h);
}
