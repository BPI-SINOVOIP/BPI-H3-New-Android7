...
int PlayerConfigDropLaytedFrame(Player* pl, int bDropLaytedFrame)
{
    ...
    if(p->pVideoDecComp != NULL)
    {
        ...
        return VideoDecCompSetDropDelayFrames(p->pVideoDecComp, bDropLaytedFrame);
    }
    return 0;
}
...
static int CallbackProcess(void* pSelf, int eMessageId, void* param)
{
    ...
    switch(eMessageId)
    {
        ...
        case PLAYER_VIDEO_RENDER_NOTIFY_FIRST_PICTURE:
        {
            ...
            else
            {
                ...
                while(p->pAvTimer->GetStatus(p->pAvTimer) != TIMER_STATUS_START)
                {
                    ...
                    // when audio first pts < video first pts.
                    // this strategy let audio putout directly, so pAvTimer is running,
                    // so while function cannot run any more,
                    // but video need to wait clock to putout.
                    // turn to [video wait clock to putout] waiting,
                    // putout video just when clock come.

#elif CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 1
                    /* if the video first pts is smaller (too much) than the
                     * audio first pts, discard this frame to catch up the
                     * audio first frame.
                     */
                    ...
                    if(p->nFirstVideoRenderPts != -1)
                    {
                        if(p->nFirstVideoRenderPts <= p->nFirstAudioRenderPts)
                        {
                                    ...
                                    CONFIG_THREASHOLD_OF_PTS_DIFFERENCE_TO_JUDGE_PTS_jUMP)
                            {
...
#if CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 0  /// small pts as start time
                                logv("CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 0,"
                                   "p->nFirstVideoRenderPts <= p->nFirstAudioRenderPts,"
                                   "|diff| = %" PRId64 "ms",
                                   p->nFirstAudioRenderPts - p->nFirstVideoRenderPts);

                                // start timer and let audio wait
                                // sometime for clock run to nFirstAudioRenderPts.
                                pthread_mutex_lock(&p->timerMutex);
                                p->nTimeBase = p->nFirstVideoRenderPts;
                                p->pAvTimer->SetTime(p->pAvTimer, p->nTimeBase);
                                p->pAvTimer->Start(p->pAvTimer);
                                ...
                                logv("CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 1,"
                                   "p->nFirstVideoRenderPts <= p->nFirstAudioRenderPts, "
                                   "|diff| = %" PRId64 " ms",
                                   p->nFirstAudioRenderPts - p->nFirstVideoRenderPts);
                                ...
                                if(p->bProcessingCommand && nWaitTime < CONFIG_MAX_WAIT_TIME_FOR_SYNC )
                                {
                                    // the waiting process for first audio video sync is broken by new
                                    // command. tell the audio render notify the first pts message later.
                                    p->nFirstAudioRenderPts = -1;
                                    return TIMER_NEED_NOTIFY_AGAIN;
                                }
...
#elif CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 2   /// donot drop video frame as start time
                                logv("CONFIG_PLAYER_AUDIO_VIDEO_SYNC_START_STRATEGY == 2,"
                                   "p->nFirstVideoRenderPts <= p->nFirstAudioRenderPts,"
                                   "|diff| = %" PRId64 " ms",
                                   p->nFirstAudioRenderPts - p->nFirstVideoRenderPts);

                                //* start timer and let audio wait sometime
                                //* for clock run to nFirstAudioRenderPts...
                                pthread_mutex_lock(&p->timerMutex);
                                p->nTimeBase = p->nFirstVideoRenderPts;
                                p->pAvTimer->SetTime(p->pAvTimer, p->nTimeBase);
                                p->pAvTimer->Start(p->pAvTimer);
                                ...
                                nWaitTime = 0;
                                logw(" above while (audio waite clock) p->pAvTimer =%" PRId64
                                    "nWaitTime = %" PRId64 " ms ", p->nFirstVideoRenderPts,nWaitTime);
                                ...
                                while(p->nFirstAudioRenderPts > (p->pAvTimer->GetTime(p->pAvTimer) + 50000))
                                {
                                    ...
                                    usleep(50000);
                                    nWaitTime += 50000;
                                    logw("audio video sync, audio wait clock to putout. nWaitTime = %d",
									    nWaitTime);
                                }
                        ...
                        else    //* p->nFirstVideoRenderPts > p->nFirstAudioRenderPts
                        {
                                    ...
                                    CONFIG_THREASHOLD_OF_PTS_DIFFERENCE_TO_JUDGE_PTS_jUMP)
                            {
                                ...
                                /* Drop audio frame if video pts is larger than audio pts a lot,
                                 * else we can halve the video frame rate to let audio catch up.
                                 *
                                 * The duration of audio_video sync is two times of the pts
                                 * difference.
                                 *
                                 * The unit of av_sync_duration is millisecond.
                                 */
                                int syncDura =
                                        GetConfigParamterInt("av_sync_duration", 3000) * 1000;
                                if (p->nFirstVideoRenderPts - p->nFirstAudioRenderPts >
                                        syncDura / 2)
                                {
                                    //* difference is more than 50ms.
                                    //* do not start the timer, audio first frame
                                    //* callback will to come again after
        ...
        case PLAYER_VIDEO_RENDER_NOTIFY_PICTURE_PTS:
        {
            ...
            int64_t nVjumpT = 0;
            int     nTestVideoBrokenThreshold = 0;
            ...
            // for pts giant jump lead to playback isnot smooth. --START
            // set 100ms(empirical value)  or nFrameDuration as threshold to test video pts jump.
            if(p->pVideoFramerateEstimater != NULL)
                nFrameDuration = FramerateEstimaterGetFrameDuration(p->pVideoFramerateEstimater);
            nTestVideoBrokenThreshold = nFrameDuration*2 > 100*1000 ? nFrameDuration*2 : 100*1000;

            nVjumpT = nVideoPts - p->nPreVideoPts;  //video pts jump step value.
            if((nVjumpT < 0) && p->mAudioJumpTime[0]<0)
            {
                //It's m3u8 segment pts jump back, donothing.
                logw("[strategy:d1]It's m3u8 segment pts jump back, doNothing.\
                    A/V[jumpT,bJ,aJ;Threshold]=[%lld,%lld,%lld]/[%lld,%lld,%lld;%d]ms",
                    p->mAudioJumpTime[0]/1000,p->mAudioJumpTime[1]/1000,p->mAudioJumpTime[2]/1000,
                    nVjumpT/1000,p->nPreVideoPts/1000,nVideoPts/1000,nTestVideoBrokenThreshold/1000);
            ...
            else if((nVjumpT > nTestVideoBrokenThreshold) && (nVjumpT < 10*nTestVideoBrokenThreshold)
                     &&(p->mAudioJumpTime[0] > 0)) //only appropriate giant pts jump do the strategy.
            {
                //video and audio pts gaint jump at the same time.
                if ((p->mAudioJumpTime[2]>=p->nPreVideoPts) && (p->mAudioJumpTime[2]<=nVideoPts))
                {
                    logw("[strategy:a2]video and audio pts giant jump,need drop %lld ms audio.\
                        A/V[jumpT,bJ,aJ;Threshold]=[%lld,%lld,%lld]/[%lld,%lld,%lld;%d]ms", nVjumpT/1000,
                        p->mAudioJumpTime[0]/1000,p->mAudioJumpTime[1]/1000,p->mAudioJumpTime[2]/1000,
                        nVjumpT/1000,p->nPreVideoPts/1000,nVideoPts/1000,nTestVideoBrokenThreshold/1000);
                    ...
                    pthread_mutex_lock(&p->timerMutex);
                    p->pAvTimer->SetTime(p->pAvTimer, p->pAvTimer->GetTime(p->pAvTimer) + p->mNeedDorpAudioTime );
                    p->pAvTimer->Start(p->pAvTimer);
                    pthread_mutex_unlock(&p->timerMutex);
                ...
                {
                    logw("[strategy:a1]video and audio pts giant jump,need drop %lld ms audio.\
                        A/[VjumpT,bJ,aJ;Threshold]=[%lld,%lld,%lld]/[%lld,%lld,%lld;%d]ms", nVjumpT/1000,
                        p->mAudioJumpTime[0]/1000,p->mAudioJumpTime[1]/1000,p->mAudioJumpTime[2]/1000,
                        nVjumpT/1000,p->nPreVideoPts/1000,nVideoPts/1000,nTestVideoBrokenThreshold/1000);

                    p->mNeedDorpAudioTime = nVjumpT;
                    pthread_mutex_lock(&p->timerMutex);
                ...
                {
                    logw("[strategy:a3]audio jump more seriousr or timer reset by audio. neednot drop audio.\
                        A/V[jumpT,bJ,aJ;Threshold]=[%lld,%lld,%lld]/[%lld,%lld,%lld;%d]ms,\
                        Recode AjumpT in case of VjumpT giant again.",
                        p->mAudioJumpTime[0]/1000,p->mAudioJumpTime[1]/1000,p->mAudioJumpTime[2]/1000,
                        nVjumpT/1000,p->nPreVideoPts/1000,nVideoPts/1000,nTestVideoBrokenThreshold/1000);
                    ...
                    p->mAudioJumpTime[2] = p->mAudioJumpTime[2];
                }
            }
            else if((nVjumpT > nTestVideoBrokenThreshold) && (nVjumpT < 10*nTestVideoBrokenThreshold)
                     &&(p->mAudioJumpTime[0] == 0)) //only appropriate giant pts jump do the strategy.
            {
                logw("[strategy:base]video pts giant jump and audio normal.\
                    V[jumpT,bJ,aJ;Threshold]=[%lld,%lld,%lld;%d]ms",
                    nVjumpT/1000,p->nPreVideoPts/1000,nVideoPts/1000,nTestVideoBrokenThreshold/1000);

                p->mNeedDorpAudioTime += nVjumpT;//accumulat V_pts jump time.
                pthread_mutex_lock(&p->timerMutex);
                p->pAvTimer->SetTime(p->pAvTimer, p->pAvTimer->GetTime(p->pAvTimer) +
                                      (nVideoPts - p->nPreVideoPts));
                p->pAvTimer->Start(p->pAvTimer);
            ...
            // for pts giant jump lead to playback isnot smooth. --END

            //only normal pts to updata the estimater
            if(p->pVideoFramerateEstimater != NULL && nVideoPts-p->nPreVideoPts>2000)
        ...
        case PLAYER_AUDIO_RENDER_NOTIFY_PTS_AND_CACHETIME:
        {
            ...
            if(p->mNeedDorpAudioTime >0) //need to drop audio frames(video frames broken).
            {
                ...
                {
                    logv("Drop audio[NeedDorp,droped,PreAPts,curAPts]=[%lld,%lld,%lld,%lld]ms",
                        p->mNeedDorpAudioTime/1000,(nAudioPts- p->nPreAudioPts)/1000,
                        p->nPreAudioPts/1000,nAudioPts/1000);
                    pthread_mutex_unlock(&p->timerMutex);
                    return TIMER_DROP_AUDIO_DATA;
                }
                else
            ...
            int nResetDiff = 100000;
            if( NULL != p->pVideoStreamInfo)
            {
               int eCodecFormat = p->pVideoStreamInfo->eCodecFormat;
               int width = p->pVideoStreamInfo->nWidth;
               int height = p->pVideoStreamInfo->nHeight;

               // some 4k265 could not put 400ms pcm into audio device in 100ms,
               // because of av interleaved and video decode.
               // unnecessary reset happened, so enlarged to 200ms.
               if (eCodecFormat == VIDEO_CODEC_FORMAT_H265 && width >= 3840 && height >= 2160)
                  nResetDiff = 200000;
             }
            ...
            if(CdxAbs(nTimeDiff) > nResetDiff)   //* difference is more than nResetDiff ms.
            {
                ...
                logd("reset the timer to %.3f, time difference is %.3f",
