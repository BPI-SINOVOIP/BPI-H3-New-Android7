...
static int InitDeinterlace(VideoRenderComp* p)
{
    ...
    if(p->mFbmBufInfo.nBufWidth <= 2048 &&  p->mFbmBufInfo.nBufHeight <= 1088)
    {
        if (CdxDiInit(p->di) == 0)
        {
			...
			//* Set Video Size for di
            CdxDiSetVideoSize(p->di, p->mFbmBufInfo.nBufWidth , p->mFbmBufInfo.nBufHeight);
        }
        else
        {
    ...
    else
    {
        logw(" open deinterlace failed , we not support the size %dx%d !",
             p->mFbmBufInfo.nBufWidth , p->mFbmBufInfo.nBufHeight);
    }
