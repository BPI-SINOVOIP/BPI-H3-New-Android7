...
#ifdef CONF_OMX_BIG_SIZE_SCALEDOWN
#define SCALEDOWN_WHEN_RESOLUTION_MOER_THAN_1080P (1)
#else
#define SCALEDOWN_WHEN_RESOLUTION_MOER_THAN_1080P (0)
#endif

#define LIMIT_STREAM_NUM (30)
#define DISABLE_DEINTERLACE_FUNCTION (0)
#define DROP_REBASE_TIME_GAP (1000000)
...
#ifdef OMX_DROP_B_FRAME_4K
#define DROP_FRAME_INFO (0)
#endif
...
#else
#define DI_PROCESS_3_PICTURE (0)	//* DI will process 2 input pictures anyway.
#endif
#ifdef __ANDROID__
...
static VIDDEC_CUSTOM_PARAM sVideoDecCustomParams[] =
{
    ...
    {VIDDEC_CUSTOMPARAM_SEND_BASE_PTS,
     (OMX_INDEXTYPE)AWOMX_IndexParamVideoSendBasePts}
};
...
static OMX_S32 setOutBufferStatus(omx_vdec* pSelf, VideoPicture* pPicture,
                                     enum OutBufferStatus eStatus)
{
    ...
    pSelf->mOutputBufferInfo[i].mStatus = eStatus;
    return 0;

}

static int omxGetVirAddrOfMetadataBuffer(omx_vdec* pSelf,
                                ...
                                unsigned char** ppViraddress)
{
    ...
    if(hnd != NULL)
    {
        ret = ion_import(pSelf->mIonFd, hnd->metadata_fd, &handle_ion);
        if(ret < 0)
        {
    ...
    ret = ion_map(pSelf->mIonFd, handle_ion, nVirsize,
    ...
    if(ret < 0)
    {
        ...
        if(nMapfd >= 0)
            close(nMapfd);
        ion_free(pSelf->mIonFd, handle_ion);
        *ppViraddress = 0;
        *pVirsize = 0;
        *pMapfd = 0;
        *pHandle_ion = 0;
...
static int omxFreeVirAddrOfMetadataBuffer(int mIonFd,
...
static void omxSetIonMetadataFlag(omx_vdec* pSelf,
                                    buffer_handle_t buffer_handle)
{
    ...
    if (pSelf->nExtendFlag & AW_VIDEO_HDR_FLAG)
    {
        hnd->ion_metadata_flag |= SUNXI_METADATA_FLAG_HDR_SATIC_METADATA;
    }

    if (pSelf->nExtendFlag & AW_VIDEO_AFBC_FLAG)
    {
...
static void omxFreeSecureBuffer(omx_vdec* pSelf,OMX_U8* pBuffer)
{
    OMX_U8* pInBuffer = NULL;
    if(pSelf->bSurpporNativeHandle == OMX_TRUE)
    {
    ...
    char*   pVirtAddr = (char*)CdcMemGetVirtualAddressCpu(pSelf->omxMemOps, pPhyAddr);
    #endif

    VideoDecoderFreeIonBuf(pSelf->m_decoder, pVirtAddr);
    return ;
...
omx_vdec::omx_vdec()
{
    ...
    nPhyOffset                = 0;
    nExtendFlag               = 0;
    bSurpporNativeHandle      = OMX_FALSE;

    mCurrentDataPts   = 0;
    mLastSYSTime  = 0;
    bGetBaseSYSTimeFlag =  OMX_TRUE;

    mDi             = NULL;
    mEnableDiFlag   = -1;
    mDiProcessNum   = 1;   //* 1 or 2
    mDiProcessCount = 0; //* from 0 to 1
    ...
    memset(&mDiOutBufferInfo, 0, sizeof(DiOutBufferInfo));
    ...
    else
    {
        ...
        logd("** nPhyOffset = %x",nPhyOffset);
    }

    #ifdef CONF_ENABLE_OPENMAX_DI_FUNCTION
    mDi = DeinterlaceCreate_Omx();
    if(mDi == NULL)
    {
...
omx_vdec::~omx_vdec()
{
    ...
    for(nIndex=0; nIndex<m_sInBufList.nBufArrSize; nIndex++)
    {
        ...
        if(m_sInBufList.pBufArr[nIndex].pBuffer != NULL)
        {
            if(m_sInBufList.nAllocBySelfFlags & (1<<nIndex))
            {
                ...
                {
                    omxFreeSecureBuffer(this, m_sInBufList.pBufArr[nIndex].pBuffer);
                }
                else
    ...
    if (m_sOutBufList.pBufArr != NULL)
    {
        int nBufCnt = m_sOutBufList.nBufArrSize;
        for (nIndex = 0; nIndex < nBufCnt; nIndex++)
        {
            if(m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate != NULL)
                free(m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate);
        }
    ...
    for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
    {
        ...
        {
            logd("ion_free: handle_ion[%p],i[%d]",(void*)mOutputBufferInfo[i].handle_ion,i);
            int len = mOutputBufferInfo[i].nVirAddrSize;
            unsigned char *pVirAddr = (unsigned char *)mOutputBufferInfo[i].pBufVirAddr;
            CdcIonUnmap(len, pVirAddr);
            if(mOutputBufferInfo[i].nBufFd >= 0)
                close(mOutputBufferInfo[i].nBufFd);
            ion_free(mIonFd,mOutputBufferInfo[i].handle_ion);
        }

        if(mOutputBufferInfo[i].metadata_handle_ion != ION_NULL_VALUE)
        {
            omxFreeVirAddrOfMetadataBuffer(mIonFd, mOutputBufferInfo[i].metadata_handle_ion,
                                           mOutputBufferInfo[i].nMetaDataMapFd,
                                           mOutputBufferInfo[i].nMetaDataVirAddrSize,
                                           mOutputBufferInfo[i].pMetaDataVirAddr);
        }
    }
...
OMX_ERRORTYPE omx_vdec::component_init(OMX_STRING pName)
{
    ...
    #ifdef CONF_ENABLE_OPENMAX_DI_FUNCTION
    // Be careful, it is a "def type".
    m_sOutPortDefType.format.video.eColorFormat      = OMX_COLOR_FormatYVU420SemiPlanar;
    #else
    ...
    if(mIsSecureVideoFlag == OMX_TRUE)
    {
        m_sOutPortDefType.nBufferCountMin      = MAX_NUM_OUT_BUFFERS_SECURE;
        m_sOutPortDefType.nBufferCountActual   = MAX_NUM_OUT_BUFFERS_SECURE;
    }
    else
    {
        m_sOutPortDefType.nBufferCountMin      = MAX_NUM_OUT_BUFFERS;
    ...
    #ifdef CONF_ENABLE_OPENMAX_DI_FUNCTION
    m_sOutPortFormatType.eColorFormat      = OMX_COLOR_FormatYVU420SemiPlanar; //NV21.
    #else
    m_sOutPortFormatType.eColorFormat      = OMX_COLOR_FormatYUV420Planar; //YV12
    #endif
    ...
    */
    ...
    mBasePTS                                  = 0;
    mLastPTS                                =0;
    #ifdef OMX_DROP_B_FRAME_4K_SET_PARA_H3
    mReferenceTimeGap                    = 300000;
    mBaseTimeGap                      =300000;
    #else
    mReferenceTimeGap                    = 100000;
    mBaseTimeGap                      = 100000;
    #endif
                                      ...
                                      OMX_IN OMX_PTR         pCmdData)
{
    ...
    switch (eCmd)
    {
        ...
        default:
            if((OMX_COMMANDTYPE)eCmd == AWOMX_IndexParamVideoSendBasePts)
            {
                logi(" COMPONENT_SEND_COMMAND: AWOMX_IndexParamVideoSendBasePts");
                mBasePTS = (OMX_S64) uParam1;
                return OMX_ErrorNone;
            }
            else
            {
    ...
    switch(eParamIndex)
    {
        ...
        default:
        {
                ...
                == AWOMX_IndexParamVideoGetAndroidNativeBufferUsage)
            {
                ...
                if(m_sInPortDefType.format.video.eCompressionFormat == OMX_VIDEO_CodingVP6 ||
                    m_sInPortDefType.format.video.eCompressionFormat == OMX_VIDEO_CodingWMV1||
                    m_sInPortDefType.format.video.eCompressionFormat == OMX_VIDEO_CodingWMV2)
                {
                ...
                logd(" ===== get usage: %x, %x", param->nUsage, GRALLOC_USAGE_HW_2D);
                break;
            }
            else if((AW_VIDEO_EXTENSIONS_INDEXTYPE)eParamIndex
                == AWOMX_IndexParamVideoGetAfbcHdrFlag)
            {
                logd(" COMPONENT_GET_PARAMETER: AWOMX_IndexParamVideoGetAfbcHdrFlag: %d",
                     nExtendFlag);
                OMX_U32* pExtendFlag = (OMX_U32*)pParamData;
                *pExtendFlag = nExtendFlag;
                *(pExtendFlag + 1) = nExtendFlag;
                break;
            }
            else
            {
        ...
        case OMX_IndexParamPortDefinition:
        {
                     ...
                     == m_sOutPortDefType.nPortIndex)
            {
                   ...
                   != m_sOutPortDefType.nBufferCountActual)
                {
                    ...
                    if(m_sOutBufList.pBufArr != NULL)
                    {
                        nBufCnt = m_sOutBufList.nBufArrSize;
                        for (nIndex = 0; nIndex < nBufCnt; nIndex++)
                        {
                            if(m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate != NULL)
                                free(m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate);
                    ...
                    for (nIndex = 0; nIndex < nBufCnt; nIndex++)
                    {
                        OMX_CONF_INIT_STRUCT_PTR (&m_sOutBufList.pBufArr[nIndex],
                                                  OMX_BUFFERHEADERTYPE);
                        m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate =
                            (OMX_PTR)malloc(sizeof(AW_OMX_VIDEO_HDR_INFO));
                        memset(m_sOutBufList.pBufArr[nIndex].pOutputPortPrivate, 0,
                               sizeof(AW_OMX_VIDEO_HDR_INFO));
                    }
                    ...
                    if(mExtraOutBufferNum == 0)
                    {
                        ...
                        if(mExtraOutBufferNum < 0)
                        {
                            loge("error: mExtraOutBufferNum is %d, modify it to 4\n", mExtraOutBufferNum);
        ...
        default:
        {
                    ...
                    == AWOMX_IndexParamVideoEnableAndroidNativeBuffers)
            {
                ...
                break;
                    ...
                    == AWOMX_IndexParamVideoAllocateNativeHandle)
            {
                logi(" COMPONENT_SET_PARAMETER: AWOMX_IndexParamVideoAllocateNativeHandle");
                                      ...
                                      OMX_IN    OMX_U8*                 pBuffer)
{
    ...
    else
    {
        ...
        *ppBufferHdr = &m_sOutBufList.pBufArr[nIndex];
        logv("**** out buffer addr : %p",&m_sOutBufList.pBufArr[nIndex]);
...
//        return OMX_ErrorIncorrectStateOperation;
    ...
    if (nPortIndex == m_sInPortDefType.nPortIndex)
    {
        ...
        if(mIsSecureVideoFlag == OMX_TRUE)
        {
            ...
            while(mHadInitDecoderFlag == OMX_FALSE && nTimeout < OMX_MAX_TIMEOUTS)
            {
                nTimeout++;
            ...
            OMX_U8* pInBuffer = NULL;
            #if(ADJUST_ADDRESS_FOR_SECURE_OS_OPTEE)
            pInBuffer = (OMX_U8*)VideoDecoderPallocIonBuf(m_decoder, nSizeBytes);
            #else
            char* pSecureBuf = (char *)VideoDecoderPallocIonBuf(m_decoder, nSizeBytes);
            pInBuffer = (OMX_U8*)CdcMemGetPhysicAddressCpu(omxMemOps, pSecureBuf);
            #endif

            if(bSurpporNativeHandle == OMX_TRUE)
            {
                native_handle_t *nh = native_handle_create(0 /*numFds*/, 1 /*numInts*/);
                nh->data[0] = (int)(uintptr_t)pInBuffer;
                m_sInBufList.pBufArr[nIndex].pBuffer = (OMX_U8*)nh;
            }
            else
            {
                m_sInBufList.pBufArr[nIndex].pBuffer = pInBuffer;
            }
                ...
                for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
                {
                    if(mOutputBufferInfo[i].handle_ion != ION_NULL_VALUE)
                    {
                        logd("ion_free: handle_ion[%p],i[%d]",
                              (void*)mOutputBufferInfo[i].handle_ion,i);
                    ...
                    if(mOutputBufferInfo[i].metadata_handle_ion != ION_NULL_VALUE)
                    {
                        omxFreeVirAddrOfMetadataBuffer(mIonFd,
                                                       mOutputBufferInfo[i].metadata_handle_ion,
                                                       mOutputBufferInfo[i].nMetaDataMapFd,
                                                       mOutputBufferInfo[i].nMetaDataVirAddrSize,
                                                       mOutputBufferInfo[i].pMetaDataVirAddr);
                    }
...
static inline void setStateIdle(omx_vdec* pSelf)
{
    ...
    else
    {
        ...
        if (pSelf->m_state == OMX_StatePause || pSelf->m_state == OMX_StateExecuting)
        {
            ...
            pSelf->pCurPicture    = NULL;
            pSelf->pPrePicture    = NULL;
			#if (DI_PROCESS_3_PICTURE)
            pSelf->pPrePrePicture = NULL;
			#endif
            memset(&pSelf->mDiOutBufferInfo, 0, sizeof(DiOutBufferInfo));
        }
    ...
    if (cmddata == 0x1 || (OMX_S32)cmddata == -1)
    {
        ...
        if(pSelf->m_useAndroidBuffer == OMX_TRUE)
        {
            ...
            for(i = 0; i < OMX_MAX_VIDEO_BUFFER_NUM; i++)
            {
                ...
                {
                    logd("ion_free: handle_ion[%p],i[%lu]",
                          (void*)pSelf->mOutputBufferInfo[i].handle_ion,i);

                    int len = pSelf->mOutputBufferInfo[i].nVirAddrSize;
                    unsigned char *pVirAddr = (unsigned char *)pSelf->mOutputBufferInfo[i].pBufVirAddr;
                    CdcIonUnmap(len, pVirAddr);
                    if(pSelf->mOutputBufferInfo[i].nBufFd >= 0)
                        close(pSelf->mOutputBufferInfo[i].nBufFd);
                    ion_free(pSelf->mIonFd,pSelf->mOutputBufferInfo[i].handle_ion);
                    pSelf->mOutputBufferInfo[i].handle_ion = 0;
                }

                if(pSelf->mOutputBufferInfo[i].metadata_handle_ion != ION_NULL_VALUE)
                {
                    omxFreeVirAddrOfMetadataBuffer(pSelf->mIonFd,
                            pSelf->mOutputBufferInfo[i].metadata_handle_ion,
                            pSelf->mOutputBufferInfo[i].nMetaDataMapFd,
                            pSelf->mOutputBufferInfo[i].nMetaDataVirAddrSize,
                            pSelf->mOutputBufferInfo[i].pMetaDataVirAddr);
                }
            }
...
int vdecoderPrepare(omx_vdec* pSelf)
{
    ...
    #ifdef CONF_ENABLE_OPENMAX_DI_FUNCTION
    pSelf->m_videoConfig.eOutputPixelFormat = PIXEL_FORMAT_NV21;	//* Used to be YV12.
    #else
    pSelf->m_videoConfig.eOutputPixelFormat = PIXEL_FORMAT_YV12;    //* Used to be YV12.
    #endif
    //pSelf->m_videoConfig.bCalledByOmxFlag   = 1;

#ifdef CONF_SURPPORT_METADATA_BUFFER
    pSelf->m_videoConfig.eCtlAfbcMode       = ENABLE_AFBC_JUST_BIG_SIZE;
#endif
    ...
    if (pSelf->m_streamInfo.nWidth < 3840 && pSelf->m_streamInfo.nHeight < 2160)
        nDiBufferNum = DI_BUFFER_NEED_TOTAL_NUM;

    logd("set nDisplayHoldingFrameBufferNum : %d, nDiBufferNum = %d",
          pSelf->mExtraOutBufferNum, nDiBufferNum);

    pSelf->m_videoConfig.nDeInterlaceHoldingFrameBufferNum = nDiBufferNum;
...
#if (DROP_FRAME_INFO)
    SetDecodePerformCmd(pSelf->m_decoder, VDECODE_SETCMD_START_CALDROPFRAME);
#endif
...
static inline void vdecoderClose(omx_vdec* pSelf)
{
    ...
    {
#if (DROP_FRAME_INFO)
        SetDecodePerformCmd(pSelf->m_decoder, VDECODE_SETCMD_STOP_CALDROPFRAME);
#endif
        DestroyVideoDecoder(pSelf->m_decoder);
        pSelf->m_decoder           = NULL;
...
static inline void vdecoderDecodeStream(omx_vdec* pSelf)
{
    ...
    /*************OMX_DROP_B_NEW********************************************************************************
            ...
            This strategy to dorp 4K-viedo B frame for Kodi is based on mBasePTS (a continuous-refresh render time) from releaseoutputbuffer() in mediacodec,
        then calculate (or estimate) nCurTimeUs (the current render time) according to nTimeUs1 and mLastSYSTime (system clock),
        mReferenceTimeGap (a refernce time between the real render pts and the current decoding frame,may be set to another),
        mCurrentDataPts(the pts of the current stream copying into decoder).
            After many experiments,we find the normal D-value between mCurrentDataPts and nCurTimeUs is within 700ms,if D-value more than 700ms,
        we increase mReferenceTimeGap to drop some B frames which will make D-value come back to normal state,and decrease mReferenceTimeGap
        when the D-value is normal in case of dropping too many B frames.Continue to do that ,the playback will be in a well-balanced state.

    *************************************************************************************************************/
    nTimeUs1 = OMX_GetNowUs();
    if (pSelf->mBasePTS != pSelf->mLastPTS)
    {
        pSelf->bGetBaseSYSTimeFlag = OMX_TRUE;
    }

    if (pSelf->bGetBaseSYSTimeFlag)
    {
        pSelf->mLastPTS = pSelf->mBasePTS;
        pSelf->mLastSYSTime= nTimeUs1;
        pSelf->bGetBaseSYSTimeFlag = OMX_FALSE;
    }

    if (pSelf->mBasePTS > 0)
    {
        nCurTimeUs = nTimeUs1 - pSelf->mLastSYSTime+ pSelf->mBasePTS+ pSelf->mReferenceTimeGap;
        logv("nCurTimeUs = %lld, nSysCurTime = %lld, lastTime = %lld, mBssePTS = %lld, mReferenceTimeGap = %lld",nCurTimeUs, nTimeUs1,
		pSelf->mLastSYSTime,pSelf->mBasePTS,pSelf->mReferenceTimeGap);
    if (pSelf->mCurrentDataPts - nCurTimeUs > 700000 && pSelf->mReferenceTimeGap >= pSelf->mBaseTimeGap)
    {
        pSelf->mReferenceTimeGap = pSelf->mReferenceTimeGap + 10000;
    }
    else if (pSelf->mCurrentDataPts - nCurTimeUs < 700000 && pSelf->mReferenceTimeGap >= (pSelf->mBaseTimeGap + 10000))
    {
        pSelf->mReferenceTimeGap = pSelf->mReferenceTimeGap - 10000;
    }
        decodeResult = DecodeVideoStream(pSelf->m_decoder,0,0,1,nCurTimeUs);
        logv("***drob decodeResult = %d",decodeResult);
    }
    else
    {
...
#if (DROP_FRAME_INFO)
    VDecodePerformaceInfo info;
    VDecodePerformaceInfo *pinfo = &info;
    GetDecodePerformInfo(pSelf->m_decoder,VDECODE_GETCMD_DROPFRAME_INFO, &pinfo);
    logv("pinfo->nDropFrameNum = %d, pinfo->nFrameDuration = %d\n", pinfo->nDropFrameNum, pinfo->nFrameDuration);
#endif
#else
    decodeResult = DecodeVideoStream(pSelf->m_decoder,0,0,0,0);
...
static int initDeinterlace(omx_vdec* pSelf)
{
    #if(DISABLE_DEINTERLACE_FUNCTION == 1)
    pSelf->mEnableDiFlag = 0;

    logd("**** initDeinterlace: enableFlag = %d, num = %d",
        (int)pSelf->mEnableDiFlag, (int)pSelf->mDiProcessNum);
    return 0;
    #endif

    if(CdcDiInit(pSelf->mDi) == 0)
    {
        int di_flag = CdcDiFlag(pSelf->mDi);
        pSelf->mDiProcessNum = (di_flag == DE_INTERLACE_HW) ? 2 : 1;
        pSelf->mEnableDiFlag = 1;
    }
    else
    {
...
int OmxCopyInputDataToDecoder(omx_vdec* pSelf)
{
    ...
    if(pInBufHdr->nTimeStamp >= 0)
    {
        ...
        DataInfo.bValid = 1;
        pSelf->mCurrentDataPts = DataInfo.nPts;
    }
    else
    {
        DataInfo.nPts   = -1;
    ...
    if(pSelf->mIsSecureVideoFlag == OMX_TRUE)
    {
        ...
        if(pSelf->omxMemOps == NULL)
        {
            pSelf->omxMemOps = SecureMemAdapterGetOpsS();
            CdcMemOpen(pSelf->omxMemOps);
        }
        ...
        #else
        pData  = (unsigned char*)CdcMemGetVirtualAddressCpu(pSelf->omxMemOps, pInBuffer);
        #endif
    ...
    pSelf->mFirstPictureFlag = OMX_FALSE;
...
static OMX_BUFFERHEADERTYPE* drainOutBufferInterlace(omx_vdec* pSelf)
{
    OMX_U32      i = 0;
    ...
    logv("drainOutBufferInterlace pSelf->mDiOutBufferInfo.nValidSize = %d",
          pSelf->mDiOutBufferInfo.nValidSize);
    if(pSelf->mDiOutBufferInfo.nValidSize <= 0)
        return NULL;
    else
    {
        for(i = 0; i < DI_BUFFER_NUM_KEEP_IN_OMX; i++)
        {
            if(pSelf->mDiOutBufferInfo.pArrPic[i] != NULL)
                break;
        }
        ...
        {
            pDiOutPicture = pSelf->mDiOutBufferInfo.pArrPic[i];
            nDiBufferIndex = i;
        }
    }
    //* request picture
    if(pSelf->pCurPicture == NULL)
    {
        pSelf->mDiProcessCount = 0;
        ...
        pSelf->pCurPicture = pPicture;
        setOutBufferStatus(pSelf, pSelf->pCurPicture, OWNED_BY_US);
    }

	#if (DI_PROCESS_3_PICTURE)
    if(pSelf->pPrePrePicture == NULL)
    {
        pSelf->pPrePrePicture = pSelf->pCurPicture;
        pSelf->pPrePicture    = pSelf->pCurPicture;
    }
	#else
    if(pSelf->pPrePicture == NULL)
    {
        pSelf->pPrePicture    = pSelf->pCurPicture;
    }
	#endif
...
di_process:
    ...
    //* di process
    #if (DI_PROCESS_3_PICTURE)
    int ret = CdcDiProcess2(pSelf->mDi, pSelf->pPrePrePicture, pSelf->pPrePicture,
                            pSelf->pCurPicture, pDiOutPicture, pSelf->mDiProcessCount);
	logv("**deinterlace: cdc di processes 3 input pictures");
	#else
    int ret = CdcDiProcess(pSelf->mDi, pSelf->pPrePicture,
                            pSelf->pCurPicture, pDiOutPicture, pSelf->mDiProcessCount);
	logv("**deinterlace: cdc di processes 2 input pictures");
	#endif
    logv("** di process , ret = %d",ret);
    if(ret != 0)
    {
        CdcDiReset(pSelf->mDi);
		#if (DI_PROCESS_3_PICTURE)
        if(pSelf->pPrePrePicture != pSelf->pPrePicture)
        {
            ReturnPicture(pSelf->m_decoder, pSelf->pPrePrePicture);
            setOutBufferStatus(pSelf, pSelf->pPrePrePicture, OWNED_BY_DECODER);
        }
		#endif

        if(pSelf->pPrePicture != pSelf->pCurPicture)
        {
            ReturnPicture(pSelf->m_decoder, pSelf->pPrePicture);
            setOutBufferStatus(pSelf, pSelf->pPrePicture, OWNED_BY_DECODER);
        }
    ...
    //* return picture
    pSelf->mDiProcessCount++;
    int bNeedReturnPrePic = 0;
    if(pSelf->mDiProcessNum == 2)
    {
        if(pSelf->mDiProcessCount >= 2)
            bNeedReturnPrePic = 1;
    }
    else
        bNeedReturnPrePic = 1;

    if(bNeedReturnPrePic == 1)
    {
		#if (DI_PROCESS_3_PICTURE)
        if(pSelf->pPrePrePicture != pSelf->pPrePicture && pSelf->pPrePrePicture != NULL)
        {
            ReturnPicture(pSelf->m_decoder, pSelf->pPrePrePicture);
            setOutBufferStatus(pSelf, pSelf->pPrePrePicture, OWNED_BY_DECODER);
        }

        pSelf->pPrePrePicture = pSelf->pPrePicture;
		#else
		if(pSelf->pPrePicture != pSelf->pCurPicture && pSelf->pPrePicture != NULL)
        {
            ReturnPicture(pSelf->m_decoder, pSelf->pPrePicture);
            setOutBufferStatus(pSelf, pSelf->pPrePicture, OWNED_BY_DECODER);
        }
    ...
    for(i = 0;i<pSelf->m_sOutBufList.nAllocSize;i++)
    {
        if(pSelf->mOutputBufferInfo[i].pVideoPicture == pDiOutPicture)
            break;
    }
    if(i == pSelf->m_sOutBufList.nAllocSize)
    {
    ...
    setOutBufferStatus(pSelf, pDiOutPicture, OWNED_BY_RENDER);
    pSelf->mDiOutBufferInfo.pArrPic[nDiBufferIndex] = NULL;
    pSelf->mDiOutBufferInfo.nValidSize--;
...
static OMX_BUFFERHEADERTYPE* drainOutBufferProgressive(omx_vdec* pSelf)
{
    ...
    if(pSelf->m_useAndroidBuffer == OMX_TRUE)
    {
        ...
        {
            /*used by cts test only.*/
            if(!pSelf->mIsSecureVideoFlag)
...
void OmxDrainVideoPictureToOutputBuffer(omx_vdec* pSelf)
{
    ...
    if(pSelf->mEnableDiFlag == 1)
    {
        pOutBufHdr = drainOutBufferInterlace(pSelf);
    }
    else
    {
                                                    ...
                                                    OMX_BOOL mInitBufferFlag)
{
    ...
    if(mInitBufferFlag == OMX_TRUE)
    {
        ...
        if(pSelf->mIonFd > 0)
        {
            nBufFd = CdcIonGetFd(pSelf->mIonFd, (uintptr_t)handle_ion);
            if(nBufFd < 0)
            {
        ...
        CdcIonMmap(nBufFd,hnd->size,(unsigned char **)&dst);
        ...
        omxGetVirAddrOfMetadataBuffer(pSelf, pBufferHandle, &metaData_handle_ion,
                                      &mMetaDataMapFd, & mMetaDataVirSize, &pMetaDataVirAddr);

        omxSetIonMetadataFlag(pSelf, pBufferHandle);

        //lc->mRelateGpuInfo[i].pWindowBuf   = pWindowBuf;
        pSelf->mOutputBufferInfo[i].nVirAddrSize = hnd->size;
        pSelf->mOutputBufferInfo[i].handle_ion   = handle_ion;
        pSelf->mOutputBufferInfo[i].nBufFd   = nBufFd;
        ...
        pSelf->mOutputBufferInfo[i].pMetaDataVirAddr     = pMetaDataVirAddr;
        pSelf->mOutputBufferInfo[i].nMetaDataVirAddrSize = mMetaDataVirSize;
        pSelf->mOutputBufferInfo[i].nMetaDataMapFd       = mMetaDataMapFd;
        pSelf->mOutputBufferInfo[i].metadata_handle_ion  = metaData_handle_ion;
    }
    ...
    pPicBufInfo->pMetaData   = pSelf->mOutputBufferInfo[i].pMetaDataVirAddr;
...
static inline int OmxGetVideoFbmBufInfo(omx_vdec* pSelf)
{
    ...
    if(pFbmBufInfo != NULL)
    {
        ...
        if(pFbmBufInfo->bProgressiveFlag == 0 && pSelf->mDi != NULL
           && pSelf->mEnableDiFlag == -1)
        {
            initDeinterlace(pSelf);
        }
        ...
        logd("**** nExtendFlag = %x", nCurExtendFlag);
        ...
        if(bBufferNumMatchFlag == OMX_TRUE
            ...
            && nCurExtendFlag == pSelf->nExtendFlag/*
        ...
        logd("****  buffer num: %d, %d; w&h: %d, %d, %d, %d",
        ...
        pSelf->nExtendFlag = nCurExtendFlag;
...
static inline int OmxReturnBufferToDecoder(omx_vdec* pSelf)
{
    ...
    VideoPicture* pPicAsDiOut = NULL;
    OMX_BUFFERHEADERTYPE*  pOutBufHdr = NULL;
    OMX_S32 nAsDiOutBufferFlag = 0;
    OMX_S32 nForbitUseFlag = 0;
    memset(&mVideoPicture, 0, sizeof(VideoPicture));

    if(pSelf->mEnableDiFlag == 1 && pSelf->mDiOutBufferInfo.nValidSize < DI_BUFFER_NUM_KEEP_IN_OMX)
    {
        nAsDiOutBufferFlag = 1;
    }
    nForbitUseFlag = nAsDiOutBufferFlag;
            ...
            if(RequestReleasePicture(pSelf->m_decoder) != NULL)
            {
                ...
                if(mRet == 0)
                {
                    ...
                    pVideoPicture = ReturnRelasePicture(pSelf->m_decoder,
                                                        &mVideoPicture, nForbitUseFlag);
                    pSelf->mOutputBufferInfo[mVideoPicture.nBufId].pVideoPicture = pVideoPicture;

                    if(nAsDiOutBufferFlag == 1)
                    {
                        pPicAsDiOut = pVideoPicture;
                        setOutBufferStatus(pSelf, pPicAsDiOut, OWNED_BY_US);
                    }
                }
                else
                {
    ...
    if(nAsDiOutBufferFlag == 1 && pPicAsDiOut != NULL)
    {
        for(i = 0; i<DI_BUFFER_NUM_KEEP_IN_OMX; i++)
        {
            if(pSelf->mDiOutBufferInfo.pArrPic[i] == NULL)
        ...
        {
            pSelf->mDiOutBufferInfo.pArrPic[i] = pPicAsDiOut;
            pSelf->mDiOutBufferInfo.nValidSize++;
        }
    }
    return 0;
...
process_command:
            ...
            (pSelf->mResolutionChangeFlag == OMX_FALSE))
        {
            ...
            if(pSelf->m_sInBufList.nSizeOfList > 0 && nVbvStreamNum < LIMIT_STREAM_NUM)
            {
            ...
            if(pSelf->m_useAndroidBuffer == OMX_TRUE)
            {
                ...
                while(pSelf->mHadGetVideoFbmBufInfoFlag == OMX_FALSE)
                {
                    ...
                    {
                        if(pSelf->mVdrvNotifyEosFlag)  //for cts test
                        {
                            OmxSetOutEos(pSelf);
                        }
            ...
            if(ValidPictureNum(pSelf->m_decoder, 0)
               || (pSelf->mEnableDiFlag == 1 && pSelf->pCurPicture != NULL))
            {
