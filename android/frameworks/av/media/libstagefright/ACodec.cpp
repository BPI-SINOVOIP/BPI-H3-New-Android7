...
status_t ACodec::setSupportedOutputFormat(bool getLegacyFlexibleFormat) {
    ...
    for (OMX_U32 index = 0; ; ++index) {
        ...
        if (format.eColorFormat == OMX_COLOR_FormatYUV420Planar
                ...
                || format.eColorFormat == OMX_COLOR_FormatYVU420SemiPlanar
...
static int64_t ACodec_getNowUs() {
    struct timeval now;
...
void ACodec::BaseState::onOutputBufferDrained(const sp<AMessage> &msg) {
    ...
    if (mCodec->mNativeWindow != NULL
        ...
        //send the current render time to omx_vdec_newdisplay
        int64_t timenowUs = ACodec_getNowUs();
        if (mCodec->mRefresh) {
            mCodec->mPts1 = timestampNs /1000;
            mCodec->mOst1 = timenowUs;
            mCodec->mRefresh = false;
        }
	 int32_t BasePTS = (int32_t) (timestampNs /1000);
        if (timenowUs - mCodec->mOst1 >= 100000) {
            mCodec->mRefresh = true;
            OMX_INDEXTYPE index = OMX_IndexComponentStartUnused;
            status_t err = mCodec->mOMX->getExtensionIndex(mCodec->mNode,"OMX.google.android.index.sendBasePts",&index);
            ALOGV("** sendBasePts index: %d, %d, %x", err, OK, index);
            if (err == OK) {
                ALOGV("** sendBasePts: BasePts  %d",BasePTS);
                mCodec->mOMX->sendCommand(mCodec->mNode, (OMX_COMMANDTYPE) index, BasePTS);
            }
        }
