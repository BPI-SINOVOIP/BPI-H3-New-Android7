...
RETRY:
	...
	if(ret == 1){
		INFO("soc is secure. (return value:%x)\n", ret);
	}else{
		INFO("soc is normal. (return value:%x)\n", ret);
		/*For sun8iw7**, we must call another io to check secure bit*/
		/*new add begin*/
		char value[PROP_VALUE_MAX] = {0};
		ret = __system_property_get("ro.hardware", value);
		if (ret < 0)
		{
			close(fd);
			return 0;
		}
		else if(memcmp(value, "sun8iw7", strlen("sun8iw7")) == 0)
		{
			ret = ioctl(fd, CHECK_SOC_BOOT, NULL);
			if (ret == 1)
			{
				INFO("soc is secure. (return value:%x)\n", ret);
				ret = 1;
