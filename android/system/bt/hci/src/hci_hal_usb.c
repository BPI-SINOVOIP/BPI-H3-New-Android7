...
static my_allocator_t my_allocator;

extern void usb_hal_packet_finished();
...
static bool hal_init(const hci_hal_callbacks_t *upper_callbacks, thread_t *upper_thread) {
  ...
  thread = upper_thread;
  my_allocator.alloc = buffer_allocator->alloc;
  my_allocator.dealloc = buffer_allocator->free;
  usb_init(upper_thread, &my_allocator);
  return true;
}
...
static size_t read_data(serial_data_type_t type, uint8_t *buffer, size_t max_size) {
  ...
  return usb_read(buffer, max_size);
}
...
static uint16_t transmit_data(serial_data_type_t type, uint8_t *data, uint16_t length) {
  ...
  *(data) = type;
  ...
  while (length > 0) {
    ssize_t ret;
    OSI_NO_INTR(ret = usb_write(data + transmitted_length, length));
    switch (ret) {
      case -1:
...
void event_usb_has_bytes(void *context) {

  if (stream_has_interpretation) {
    callbacks->data_ready(current_data_type);
  } else {
    uint8_t type_byte;
    if (usb_read(&type_byte, 1) == 0) {
