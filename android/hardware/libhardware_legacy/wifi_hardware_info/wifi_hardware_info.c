...
static int wifi_power_on(void)
{
    ...
    char to_write = '1';
    ALOGE("wifi power on");
    fd = open(WIFI_POWER_STATE_PATH, O_RDONLY);
    if (fd < 0) {
    ...
    if (size <= 0) {
        ALOGE("read wifi_power_state_path error!");
        close(fd);
        return -1;
    }
    ...
    if (size < 0) {
        ALOGE("write wifi_power_state_path error!");
        close(fd);
        return -1;
    }*/

    close(fd);
    property_set("sys.wifi.power.on","1");
...
static int wifi_power_off(void)
{
    ...
    close(fd);*/
    property_set("sys.wifi.power.on","0");
    return 0;
}
...
static int wifi_scan_device(int val)
{
    ...
    ALOGE("wifi_scan_device");
    fd = open(WIFI_SCAN_DEVICE_PATH, O_WRONLY);
    if (fd < 0) {
        ALOGE("open wifi_scan_device_path error!");
        return -1;
    }

    size = write(fd, &to_write, sizeof(to_write));
    if (size < 0) {
        ALOGE("write wifi_scan_device_path error!");
        close(fd);
        return -1;
    }
    close(fd);*/
    if (1 == val) {
        property_set("sys.wifi.scan.device","1");
    } else {
        property_set("sys.wifi.scan.device","0");
    }
    return 0;
}
