...
#define BUFFER_ITEM_COOKIE_MAGIC	"cookie"
...
}BufferExtractCookieItem;

#define MAX_BUFFER_COOKIE_ITEM_COUNT	(10)
...
typedef struct {
	...
	BufferExtractCookieItem cookie_item[MAX_BUFFER_COOKIE_ITEM_COUNT];
	int reserve[28];
}BufferExtractCookieItemHead;

typedef struct sunxi_dtc_param {
	char *inname;
	char *outname;
	char *fexname;
}sunxi_dtc_param_t;
...
#define bb_debug(fmt, args...) printf("libboot:" fmt, ## args)
