...
{
    F_LOG;
    mCamMemOpsS = MemCamAdapterGetOpsS();
    mCamMemOpsS->open_cam();
...
{
    LOGD("%s:zjw,  cnt:%d, width:%d, height:%d",__FUNCTION__,cnt,width,height);
    int err = 0;
    int ret;
    ...
    LOGD("undequeued_num:%d",min_undequeued_num);
    ret = mPreviewWindow->set_buffers_geometry(mPreviewWindow,
                                                   ALIGN_16B(width),/* for encoder align*/
                                                   ALIGN_16B(height),/* for decoder align*/
                                                   format);
    LOGD("zjw,set_buffers_geometry ");
    if (ret != NO_ERROR) {
    ...
    LOGV("zjw,set_crop out");

    LOGD("zjw,set_buffer_count");
    ret = mPreviewWindow->set_buffer_count(mPreviewWindow, cnt/* we should plus min_undequeued_num when we work in picture mode*/ +( (cnt == NB_BUFFER) ? 0:min_undequeued_num ) );
    ...
    {
        int stride = 0;
        LOGV("zjw, dequeue buf \n");
        ret = mPreviewWindow->dequeue_buffer(mPreviewWindow, &mBufferHandle[i], &stride);
        LOGV("zjw,after dequeue buf \n");
        if (ret) {
            LOGE("zjw,dequeue preview idx = %d err = %d",i,ret);
        }
        LOGV("dequeue buf mBufferHandle[%d] =%p", i, mBufferHandle[i]);

        /*for H6 use private_handle_t , that rely on the GPU definity. */
        /* private_handle_t* */
        mPrivateHandle[i] = (private_handle_t *)(*mBufferHandle[i]);
        /* get share fd for take picture */
        mShareFd[i] = mPrivateHandle[i]->share_fd;

        /* get aw_buf_id for record */
        aw_cam_buf_id[i] = mPrivateHandle[i]->aw_buf_id;
        ...
        LOGD("zjw index changed :%d,fd:%d,buf Size:%d,vir:%p, aw_buf_id = %lld,aw_cam_buf_id = %lld,format = %d,\n",i,mPrivateHandle[i]->share_fd,mPrivateHandle[i]->size,ddr_vir[i],ddr_phy[i],mPrivateHandle[i]->aw_buf_id,aw_cam_buf_id,mPrivateHandle[i]->format);
...
int PreviewWindow::freeBufferHandle(int cnt,buffer_handle_t** mBufferHandle)
{
//do not thing here..
    return NO_ERROR;
}

int PreviewWindow::lockBuffer(int width,int height,int index)
{
    int ret = -1;
    void * virFromGralloc;
        ...
        ret = grbuffer_mapper.lock(*mBufferHandle[index], GRALLOC_USAGE_SW_WRITE_OFTEN, rect, &virFromGralloc);
    LOGV("%s: zjw,virFromGralloc   = %p ",__FUNCTION__, virFromGralloc);
...
int PreviewWindow::unlockBuffer(int width,int height,int index)
{
    int ret = -1;
    /* let the graphics framework to lock the buffer, and get the virtual address. */
    const Rect rect(width, height);
...
int PreviewWindow::onNextFrameAvailable2(int index)
{
    ...
    int err = NO_ERROR;
    int dequeuedIdx = -1;
    int64_t timedebugbefore = 0;
    int width = 0;
    int height = 0;

    timedebugbefore = systemTime() / 1000000;
    LOGV("zjw,time enqueue_buffer = %lld," , timedebugbefore);

#if 1/* jiangwei test grbuffer_mapper.lock */

    width = mPrivateHandle[index]->width;
    height = mPrivateHandle[index]->height;

    err = lockBuffer(width,height,index);
    if(err != NO_ERROR)
    {
    ...
    enqueue_buffer_num ++;
    LOGV("zjw,now enqueue num:%d,min unenqueue num:%d",enqueue_buffer_num,min_undequeued_num);

    if(err != NO_ERROR) {
        LOGE("enqueue_buffer failed, err = %d \n", err);
    } else {
        LOGV("zjw,PreviewWindow::onNextFrameAvailable2 enqueue buffer index = %d .\n",index);
    }
    ...
    if(enqueue_buffer_num > min_undequeued_num){
        ...
        enqueue_buffer_num --;
        LOGV("zjw,time after dequeue_buffer = %lld,",systemTime() / 1000000-timedebugbefore);

        private_handle_t * tPrivateHandle;
        tPrivateHandle = (private_handle_t *)(*buffer_handle);

        if (err == NO_ERROR && tPrivateHandle != NULL)
        {
            for(int i = 0; i < NB_BUFFER/* this is the MAX buffer num,actually need to find the mapped buffer num*/; i++)
            {
                LOGV("zjw,dequed buf handle we gettPrivateHandle->aw_buf_id = %lld,and we allocate the buf aw_cam_buf_id[i] =%lld ",tPrivateHandle->aw_buf_id,aw_cam_buf_id[i]);
                if(aw_cam_buf_id[i] == tPrivateHandle->aw_buf_id)
                {
                    LOGV("zjw,PreviewWindow::onNextFrameAvailable2 get dequeue buffer index = %d ", i);
                    dequeuedIdx = i;
                    break;
                }
            }

#if 1/* jiangwei test grbuffer_mapper.unlock */
            LOGV("--------------zjw,add test-------------------------------------- \n");
            LOGV("zjw,add test for  aw_buf_id = %lld ,dequeuedIdx = %d!\n",tPrivateHandle->aw_buf_id,dequeuedIdx);
            err = unlockBuffer(width,height,dequeuedIdx);
            if(err != NO_ERROR)
            {
...
bool PreviewWindow::onNextFrameAvailable(const void* frame)
{
    ...
    unsigned long preview_addr_phy = 0L;
    unsigned long preview_addr_vir = 0L;
    int preview_width = 0;
    ...
    mCamMemOpsS->flush_cache_cam((void*)preview_addr_vir, mPreviewFrameSize);
        ...
        || preview_format == V4L2_PIX_FMT_NV12)
    {
        ...
        if (stride == ALIGN_16B(preview_width))
        {
            ...
            #if (SW_CHIP_PLATFORM == H8_CHIP_PLATFORM)
...
bool PreviewWindow::adjustPreviewDimensions(V4L2BUF_t* pbuf)
{
        ...
        && (pbuf->thumbUsedForPreview == 1))        // use thumb frame for preview
    {
        ...
        mPreviewFrameSize = calculateFrameSize(pbuf->thumbWidth,
                                               pbuf->thumbHeight,
                                               pbuf->thumbFormat);
    }
    else
