...
#include <camerahal/drv_display.h>
...
*/
    ...
    else
    {
                ...
                && (new_status & FOCUS_STATUS_DONE))
        {
            // focus end
            LOGV("focus %s, %d -> %d",
                    (new_status == FOCUS_STATUS_SUCCESS)?"ok":"fail",
                    mFocusStatus,
                    new_status);
...
bool CameraHardware::faceDetectionThread()
{
    ...
    if (mSmileDetectionEnable == true)
    {
        ...
        if (mSmileDetection != 0)
        {
            ...
            mSmileDetection->ioctrl(mSmileDetection,
                                    SMILE_OPS_CMD_START,
                                    0,
                                    &mFrameFaceData);
        }
    ...
    if (mBlinkDetectionEnable == true)
    {
        ...
        if (mBlinkDetection != 0)
        {
            ...
            mBlinkDetection->ioctrl(mBlinkDetection,
...
//        pthread_mutex_lock(&mCommandMutex);
    ...
    switch(cmd)
    {
        ...
        case CMD_QUEUE_SET_FLASH_MODE:
        {
            ...
            {
              LOGE("ERR(%s):Fail on mV4L2Camera->setFlashMode(flash(%d))",
                __FUNCTION__, new_flash);
            }
        ...
        case CMD_QUEUE_START_FACE_DETECTE:
        {
            ...
            {
                int scale = pV4L2Device->getSuitableThumbScale(mCaptureWidth,
                                                               mCaptureHeight);
                if (scale <= 0)
                {
                    LOGE("error thumb scale: %d, full-size: %dx%d",
                        scale, mCaptureWidth, mCaptureHeight);
                    break;
                }
...
status_t CameraHardware::Initialize()
{
    ...
    mFaceDetection->ioctrl(mFaceDetection,
...
void CameraHardware::initDefaultParameters()
{
    ...
    if (mHalCameraInfo.is_uvc)
    {
        ...
        // add temp for CTS
        p.set(CameraParameters::KEY_PREVIEW_FPS_RANGE, "5000,60000");
        p.set(CameraParameters::KEY_SUPPORTED_PREVIEW_FPS_RANGE, "(5000,60000)");
    ...
    LOGV("supportPreviewSizeValue: [%s] %s",
        CameraParameters::KEY_SUPPORTED_PREVIEW_SIZES, value);
    ...
    LOGV("supportPreviewSizeValue: [%s] %s",
        CameraParameters::KEY_SUPPORTED_PICTURE_SIZES, value);
    ...
    LOGV("supportFrameRateValue: [%s] %s",
        CameraParameters::KEY_SUPPORTED_PREVIEW_FRAME_RATES, value);
...
void CameraHardware::stopRecording()
{
    ...
    {
        //mV4L2CameraDevice->set3ALock(~V4L2_LOCK_FOCUS);
        mV4L2CameraDevice->set3ALock((~V4L2_LOCK_EXPOSURE) &
                                     (~V4L2_LOCK_WHITE_BALANCE)&
                                     (~V4L2_LOCK_FOCUS));
    }

    mV4L2CameraDevice->clearBuffer();
    //doStopPreview();
    //doStartPreview();
}
...
void CameraHardware::releaseRecordingFrame(const void* opaque)
{
    if (mUseHwEncoder)
    {
        ...
        private_handle_t* mMetadata;
        int releaseBufferIndex;
        int ret = -1;

        if(buffer_type == kMetadataBufferTypeCameraSource)
        {
            ALOGV("The buffer type is kMetadataBufferTypeCameraSource !\n");
        }else if(buffer_type == kMetadataBufferTypeNativeHandleSource)
        {
            ALOGV("The buffer type is kMetadataBufferTypeNativeHandleSource !\n");
        }else
        {
            ALOGE("The buffer type  = %d that we do not support !\n",buffer_type);
        }

        mMetadata = (private_handle_t *)(((VideoNativeHandleMetadata *)((int)opaque))->pHandle);
        for(int i = 0; i < NB_BUFFER/* this is the MAX buffer num,actually need to find the mapped buffer num*/; i++) {
                if(mV4L2CameraDevice->mPreviewWindow->aw_cam_buf_id[i] == mMetadata->aw_buf_id) {
                releaseBufferIndex = i;
                break;
            }
        }

        LOGV("zjw index changed ,fd:%d,buf Size:%d, base = %p,format = %d,aw_buf_id = %lld \n",mMetadata->share_fd,mMetadata->size,mMetadata->base,mMetadata->format,mMetadata->aw_buf_id);
        ...
        {
            LOGV("zjw,CameraHardware::releaseRecordingFrame index = %d \n",releaseBufferIndex);
            mV4L2CameraDevice->releasePreviewFrame(releaseBufferIndex);
            mV4L2CameraDevice->refreshBufferState(releaseBufferIndex,false);
            //if(mV4L2CameraDevice->checkBufferState(releaseBufferIndex))
            {
                LOGV("zjw,CameraHardware::releaseRecordingFrame index = %d ,mV4L2CameraDevice->checkBufferState is true \n",releaseBufferIndex);
                ret = native_handle_close(mMetadata);
                if(ret != 0)
                {
                    LOGD("zjw,CameraHardware::releaseRecordingFrame native_handle_close error  = %d\n",ret);
                }
                ret = native_handle_delete(mMetadata);
                if(ret != 0)
                {
                    LOGV("zjw,CameraHardware::releaseRecordingFrame native_handle_delete error  = %d\n",ret);
                }
            }
        }
    }
...
status_t CameraHardware::doTakePicture()
{
    ...
    if (mCaptureWidth == frame_width
        ...
        && mIsImageCaptureIntent == false)
    {
        LOGV("current capture size[%dx%d] is the same as picture size",
    ...
    LOGD("Starting camera: %dx%d -> %.4s(%s)",
         ...
         pix_fmt);
    status_t res = mV4L2CameraDevice->startDevice(mCaptureWidth,
                                                  mCaptureHeight,
                                                  org_fmt,
                                                  video_hint);
    if (res != NO_ERROR)
...
void CameraHardware::getCurrentOriention(int * oriention)
{
    *oriention = mOriention;
    ...
    //do it in AWGallery
    //for direction of front camera facedetection
    //if(mHalCameraInfo.facing == CAMERA_FACING_FRONT)
/*    {
        if(*oriention == 90)
        {
            *oriention = 270;
...
status_t CameraHardware::setParameters(const char* p)
{
    ...
    if (new_picture_mode != NULL)
    {
            ...
            || !strcmp(new_picture_mode, PICTURE_MODE_CONTINUOUS_FAST))
        {
            ...
            {
                LOGW("%s : invalid path: %s in %s picture mode",
                    __FUNCTION__, new_path, new_picture_mode);
...
#endif
        //add for CTS Verifier by fuqiang
        if (strcmp(mCallingProcessName, "com.android.cts.verifier") == 0
    ...
    if (mCameraConfig->supportExposureCompensation())
    {
        int now_exposure_compensation = mParameters.getInt(
                                CameraParameters::KEY_EXPOSURE_COMPENSATION);
        int new_exposure_compensation = params.getInt(
                                CameraParameters::KEY_EXPOSURE_COMPENSATION);
        int max_exposure_compensation = params.getInt(
                                CameraParameters::KEY_MAX_EXPOSURE_COMPENSATION);
        int min_exposure_compensation = params.getInt(
        ...
        {
            LOGE("ERR(%s):invalid exposure compensation: %d",
                __FUNCTION__, new_exposure_compensation);
            return -EINVAL;
        }
    }
    ...
    if (mCameraConfig->supportFlashMode())
    {
        ...
        if (!strcmp(new_flash_mode_str, CameraParameters::FLASH_MODE_OFF))
            ...
            //twilight, shade
            LOGE("ERR(%s):Invalid flash(%s)", __FUNCTION__, new_flash_mode_str);
            ret = UNKNOWN_ERROR;
        }
    ...
    const char *new_gps_processing_method_str = params.get(
...
{
    LOGV("dumpCamera camera id: %d", fd);
...
status_t CameraHardware::doStartPreview()
{
    ...
    if (preview_format != NULL)
    {
        if (strcmp(preview_format, CameraParameters::PIXEL_FORMAT_YUV420SP) == 0)
        {
            ...
            //add by jiangwei for using useptr mode to set y addr and uv addr
            #ifdef USE_CSI_VIN_DRIVER
            org_fmt = V4L2_PIX_FMT_NV21M;
            #endif
#endif
        }
