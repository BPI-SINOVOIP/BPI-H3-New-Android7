...
int HALCameraFactory::getCameraHardwareNum()
{
    ...
    if (mAttachedCamerasNum == 2)
    {
        for (int i = 0; i < MAX_NUM_OF_CAMERAS; i++)
        {
            ...
            // mCameraConfig[i]->supportFastPictureMode();
    ...
    else if (mAttachedCamerasNum == 1)
    {
        ...
        // mCameraConfig[0]->supportFastPictureMode();
        mHalCameraInfo[0].fast_picture_mode    = false;
        mHalCameraInfo[0].is_uvc        = false;

#if USE_CSI_VIN_DRIVER //jiangwei
        LOGD("zhengjiangwei,  -------------- use csi vin driver--------------");
        // There may be another USB camera. Then, the device node would
        // be "/dev/video0" or "/dev/video1".

        sprintf(dev_node, "/dev/video2");
        ...
        if(ret == 0)
        {
            ...
            mHalCameraInfo[1].facing    = (mCameraConfig[0]->cameraFacing() == CAMERA_FACING_BACK) ?
                                            CAMERA_FACING_FRONT :
                                            CAMERA_FACING_BACK;
    ...
    LOGD("There is %d attached cameras and %d removable cameras",
        mAttachedCamerasNum, mRemovableCamerasNum);
