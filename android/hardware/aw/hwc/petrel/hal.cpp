...
static tv_para_t g_tv_para[]=
{
    ...
    {5, DISP_TV_MOD_3840_2160P_50HZ,  3840,   2160, 50,0xff},
...
int is_yuv420_sampling_mode(int mode)
{
	switch (mode) {
...
int saveDispDeviceConfig(unsigned int hwDisp, struct disp_device_config *config)
{
    ...
    if (config->eotf != DISP_EOTF_GAMMA22 ||
        ...
        ALOGE("saveDispDeviceConfig: default save as DISP_EOTF_GAMMA22, "
              "not need to open HDR on boot");
...
int resetDispConfig(int disp, int type, int mode, disp_device_config *out_config)
{
    int change = 0;
    sunxi_hdmi_capability_t cap;

    ALOGD("resetDispConfig: enter");
    if (type != DISP_OUTPUT_TYPE_HDMI) {
        return 0;
    }

    if ((getDispDeviceConfigs(disp, out_config) != 0) ||
            (out_config->type != type)) {
        ALOGE("resetDispConfig: Get display device config error");
        return 0;
    }
    if (getHdmiCapability(&cap)) {
        ALOGE("resetDispConfig: Get hdmi capability error");
        return 0;
    }

    if (out_config->mode != mode) {
        out_config->mode = (enum disp_tv_mode)mode;
        change++;
    }

    if (is_yuv420_sampling_mode(mode)) {
        if (out_config->format != DISP_CSC_TYPE_YUV420) {
            /* Check if support other sampling mode */
            unsigned int regular = 0;
            for (int idx = 0; idx < cap.total_ycbcr420_vic; idx++) {
                if (cap.ycbcr420_vic[idx].sunxi_disp_mode == mode) {
                    regular = cap.ycbcr420_vic[idx].support_regular_sampling_mode;
                    break;
                }
            }
            if (!regular) {
                ALOGE("resetDispConfig: mode %d not support format %d",
                      mode, out_config->format);
                out_config->format = DISP_CSC_TYPE_YUV420;
                out_config->bits   = DISP_DATA_8BITS;
                change++;
            }
        }
    } else {
        /* Check YUV420 sampling mode supported */
        if (out_config->format == DISP_CSC_TYPE_YUV420) {
            out_config->format = DISP_CSC_TYPE_YUV444;
            out_config->bits = DISP_DATA_8BITS;
            change++;
        }

        /* Check blacklist sink capability */
        int capability = _hwc_device_is_support_hdmi_mode(HDMI_USED, mode);
        if ((capability & 0x02) && out_config->format == DISP_CSC_TYPE_RGB) {
            ALOGE("resetDispConfig: sink not support RGB sampling");
            out_config->format = DISP_CSC_TYPE_YUV444;
            change++;
        } else if ((capability & 0x04) && out_config->format != DISP_CSC_TYPE_RGB) {
            ALOGE("resetDispConfig: sink not support YUV sampling");
            out_config->format = DISP_CSC_TYPE_RGB;
            change++;
        }
    }

    if (out_config->format == DISP_CSC_TYPE_YUV420) {
        if ((out_config->bits == DISP_DATA_10BITS) &&
                !(cap.pixel_encoding & PIXEL_ENCODING_YUV420_10BIT)) {
            ALOGE("resetDispConfig: sink not support yuv420 10bit");
            out_config->bits = DISP_DATA_8BITS;
            change++;
        }
    } else {
        if (out_config->bits == DISP_DATA_10BITS) {
            int support_10bit = 0;
            switch (out_config->format) {
            case DISP_CSC_TYPE_RGB:
                support_10bit = cap.supported_format[FMT_RGB888].bits & COLOR_DEPTH_10BIT;
                break;
            case DISP_CSC_TYPE_YUV444:
                support_10bit = cap.supported_format[FMT_YUV444].bits & COLOR_DEPTH_10BIT;
                break;
            case DISP_CSC_TYPE_YUV422:
                support_10bit = cap.supported_format[FMT_YUV422].bits & COLOR_DEPTH_10BIT;
                break;
            default:
                support_10bit = 0;
                break;
            ...
            if (!support_10bit) {
                ALOGE("resetDispConfig: sink not support 10bit on format %d", out_config->format);
                out_config->bits = DISP_DATA_8BITS;
                change++;
            }
        }
	...
	if (cap.rgb_only && out_config->format != DISP_CSC_TYPE_RGB) {
		ALOGW("Device only support RGB format!");
		out_config->format = DISP_CSC_TYPE_RGB;
		out_config->bits = DISP_DATA_8BITS;
		change++;
	}
	if (cap.dvihdmi == DISP_DVI) {
		ALOGD("Device only support dvi");
		if (out_config->dvi_hdmi != DISP_DVI) {
			out_config->dvi_hdmi = DISP_DVI;
			out_config->format = DISP_CSC_TYPE_RGB;
			out_config->bits = DISP_DATA_8BITS;
			change++;
		}
	} else if (cap.dvihdmi == DISP_HDMI) {
		ALOGD("Device support hdmi");
		if (out_config->dvi_hdmi != DISP_HDMI) {
			out_config->dvi_hdmi = DISP_HDMI;
			if (out_config->format == DISP_CSC_TYPE_RGB && !cap.rgb_only) {
				out_config->format = DISP_CSC_TYPE_YUV444;
				out_config->bits = DISP_DATA_8BITS;
			}
			change++;
		...
		ALOGW("unknow device type!");
	}

    if (out_config->eotf == DISP_EOTF_SMPTE2084) {
        unsigned int cap_mask = getTvCapabilities(&cap);
        if ((cap_mask & TV_CAPABILITY_HDR) == 0) {
            ALOGE("resetDispConfig: sink not support HDR");
            out_config->eotf = DISP_EOTF_GAMMA22;
            out_config->cs = mode >= DISP_TV_MOD_720P_50HZ ? DISP_BT709 : DISP_BT601;
            change++;
        }
    } else if (out_config->eotf == DISP_EOTF_GAMMA22) {
        disp_color_space cs = mode >= DISP_TV_MOD_720P_50HZ ? DISP_BT709 : DISP_BT601;
        if (out_config->cs != cs) {
            out_config->cs = cs;
            change++;
        }
...
struct hdmi_pixel_clock {
	int mode;
	int pixel_clock;
};

static int calcuTmdsCharRate(int mode, int format, int bits)
{
    const hdmi_pixel_clock pixel_clock_arrary[] = {
        {DISP_TV_MOD_3840_2160P_24HZ, 297},  /* VIC 93 */
        {DISP_TV_MOD_3840_2160P_25HZ, 297},  /* VIC 94 */
        {DISP_TV_MOD_3840_2160P_30HZ, 297},  /* VIC 95 */
        {DISP_TV_MOD_3840_2160P_50HZ, 594},  /* VIC 96 */
        {DISP_TV_MOD_3840_2160P_60HZ, 594},  /* VIC 97 */
    };
    int pixel_clock = 0;
    float tmds_rate = 0;
    float factor = (bits == DISP_DATA_10BITS) ? 1.25 : 1.0;

    for (size_t i = 0;
            i < sizeof(pixel_clock_arrary) / sizeof(pixel_clock_arrary[0]);
            i++) {
        if (pixel_clock_arrary[i].mode == mode) {
            pixel_clock = pixel_clock_arrary[i].pixel_clock;
            break;
        }
    }
    ...
    case DISP_CSC_TYPE_RGB:
    case DISP_CSC_TYPE_YUV444:
        tmds_rate = pixel_clock * factor;
        break;
    case DISP_CSC_TYPE_YUV422:
        tmds_rate = pixel_clock;
        break;
    case DISP_CSC_TYPE_YUV420:
        tmds_rate = pixel_clock * 0.5 * factor;
        break;
    default:
        break;
    }
    return ceil(tmds_rate);
}

int checkHDMITmdsCharRate(disp_device_config *config, int max_tmds_rate)
{
    int request_rate = calcuTmdsCharRate(config->mode,
                                         config->format,
                                         config->bits);
    ALOGD("Request tmds rate %d MHz, Max tmds char rate %d MHz",
          request_rate, max_tmds_rate);
    return !(request_rate <= max_tmds_rate);
}

int checkHdmiModeSupport(int disp, int mode)
{
    int support = 0;
    switch (mode) {
    case DISP_TV_MOD_3840_2160P_24HZ:
    ...
    case DISP_TV_MOD_3840_2160P_50HZ:
    case DISP_TV_MOD_3840_2160P_60HZ:
        return _hwc_device_is_support_hdmi_mode(disp, mode);
    default:
        return 1;
    }
}
...
int hwcOutputSwitch(DisplayInfo *psDisplayInfo, int type, int mode)
{
    ...
    case DISP_OUTPUT_TYPE_HDMI: {
		...
		/* check sink capability to detect */
		if (checkHdmiModeSupport(hwDisp, mode) != 1) {
			ALOGW("hwcOutputSwitch: not support mode %d", mode);
			return -1;
		}
		...
		if ((cfg.format == DISP_CSC_TYPE_RGB) && !cap.rgb_only) {
			/*
			 * If device support yuv output sampling,
			 * reset the output format to yuv444 default
			 */
			...
			if (!support_yuv420) {
				...
				if (is_yuv420_sampling_mode(mode)) {
					if (cap.video_format & VIDEO_FORMAT_YCBCR422) {
						...
						cfg.bits = (cap.pixel_encoding & PIXEL_ENCODING_YUV420_10BIT)
										? DISP_DATA_10BITS : DISP_DATA_8BITS;
		...
		if (is_yuv420_sampling_mode(mode)) {
			if (support_yuv420) {
				...
				psDisplayInfo->output_format = DISP_CSC_TYPE_YUV422;
				psDisplayInfo->output_bits = DISP_DATA_10BITS;
				ALOGW("not support yuv420 on 4K60Hz, use yuv422 10bit");
			}
		}
		...
		if (DISPLAY_OUTPUT_DATASPACE_MODE_HDR == actual_dataspce_mode) {
			if (!(TV_CAPABILITY_HDR & cap_mask)) {
				...
				cfg.bits = (cfg.format == DISP_CSC_TYPE_YUV422) ?
							DISP_DATA_10BITS : DISP_DATA_8BITS;
		...
		if (cap.dvihdmi == DISP_DVI) {
			ALOGD("Device only support DVI!");
			cfg.dvi_hdmi = DISP_DVI;
			cfg.format = DISP_CSC_TYPE_RGB;
			cfg.bits = DISP_DATA_8BITS;
		} else if (cap.dvihdmi == DISP_HDMI) {
			cfg.dvi_hdmi = DISP_HDMI;
			if (cfg.format == DISP_CSC_TYPE_RGB && !cap.rgb_only) {
				ALOGD("Switch to YUV444 on hdmi device, rgb_only = %d", cap.rgb_only);
				cfg.format = DISP_CSC_TYPE_YUV444;
				cfg.bits = DISP_DATA_8BITS;
			}
		}
...
__recheck:
		if (checkHDMITmdsCharRate(&cfg, cap.max_tmds_rate) !=0 ) {
			if (cfg.bits != DISP_DATA_8BITS) {
				ALOGE("Current depth = 10bit, beyond the tmds rate");
				cfg.bits = DISP_DATA_8BITS;
				goto __recheck;
			}
		...
		psDisplayInfo->output_bits = cfg.bits;
//		psDisplayInfo->setOutputformat = _hwc_to_output_format(cfg.format, cfg.bits);
