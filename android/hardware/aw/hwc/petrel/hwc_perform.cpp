...
int hwc_getOutputPixelFormat(int display, va_list /* args */)
{
    return _hwc_device_get_output_format(display);
}

int hwc_setOutputPixelFormat(int display, va_list args)
{
    int format = va_arg(args, int);
    ALOGD("hwc_setOutputPixelFormat: disp %d format %d",
        display, format);
    return _hwc_device_set_output_format(display, format);
}

int hwc_setDeviceConfig(int display, va_list args)
{
    struct disp_device_config *config =
        va_arg(args, struct disp_device_config *);
    ALOGD("hwc_setDeviceConfig: disp %d config %p", display, config);
    return _hwc_device_set_output_config(display, config);
}

int hwc_getOutputCurDataspaceMode(int display, va_list /* args */)
{
    return _hwc_device_get_cur_dataspace_mode(display);
}

int hwc_getOutputDataspaceMode(int display, va_list /* args */)
{
    return _hwc_device_get_dataspace_mode(display);
}

int hwc_setOutputDataspaceMode(int display, va_list args)
{
    int mode = va_arg(args, int);
    ALOGD("hwc_setOutputDataspaceMode: disp %d dataspaceMode %d",
        display, mode);
    return _hwc_device_set_dataspace_mode(display, mode);
}
...
{
    int mode = va_arg(args, int);
    int videoCropHeight = va_arg(args, int);
    return _hwc_device_set_3d_mode(display, (__display_3d_mode)mode, videoCropHeight);
}
...
int hwc_perform(int display, int cmd, ...)
{
    ...
    case HWC_GET_OUTPUT_PIXEL_FORMAT:
        ret = hwc_getOutputPixelFormat(display, args);
        break;
    case HWC_SET_OUTPUT_PIXEL_FORMAT:
        ret = hwc_setOutputPixelFormat(display, args);
        break;
    case HWC_GET_OUTPUT_CUR_DATASPACE_MODE:
        ret = hwc_getOutputCurDataspaceMode(display, args);
        break;
    case HWC_GET_OUTPUT_DATASPACE_MODE:
        ret = hwc_getOutputDataspaceMode(display, args);
        break;
    case HWC_SET_OUTPUT_DATASPACE_MODE:
        ret = hwc_setOutputDataspaceMode(display, args);
        break;
    case HWC_SET_DEVICE_CONFIG:
        ret = hwc_setDeviceConfig(display, args);
        break;
