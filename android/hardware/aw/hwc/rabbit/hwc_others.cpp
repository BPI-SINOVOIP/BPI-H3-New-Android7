...
int _hwc_device_set_output_mode(int disp, int out_type, int out_mode)
{
    ...
    if((0 != Globctx->ban4k)
      ...
      && (DISP_TV_MOD_3840_2160P_60HZ >= out_mode))
    {
        return -1;
    }
...
int _hwc_device_set_output_format(int disp, int format)
{
	...
	if (psDisplayInfo->setOutputformat == format)
		return 0;
...
int _hwc_device_get_output_format(int disp)
{
	...
	if ((DISP_CSC_TYPE_YUV444 == psDisplayInfo->output_format)
		&& (DISP_DATA_8BITS == psDisplayInfo->output_bits)) {
		return DISPLAY_OUTPUT_FORMAT_YUV444_8bit;
	} else if ((DISP_CSC_TYPE_YUV420 == psDisplayInfo->output_format)
		&& (DISP_DATA_10BITS == psDisplayInfo->output_bits)) {
		return DISPLAY_OUTPUT_FORMAT_YUV420_10bit;
	} else if ((DISP_CSC_TYPE_YUV422 == psDisplayInfo->output_format)
		&& (DISP_DATA_10BITS == psDisplayInfo->output_bits)) {
		return DISPLAY_OUTPUT_FORMAT_YUV422_10bit;
	} else if ((DISP_CSC_TYPE_RGB == psDisplayInfo->output_format)
		&& (DISP_DATA_8BITS == psDisplayInfo->output_bits)) {
