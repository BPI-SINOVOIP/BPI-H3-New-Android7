...
{
    {8, DISP_TV_MOD_NTSC,             720,    480, 60,  0x00},
    {8, DISP_TV_MOD_PAL,              720,    576, 50,  0x00},

    {5, DISP_TV_MOD_480I,             720,    480, 60,  0x00},
    {5, DISP_TV_MOD_576I,             720,    576, 60,  0x00},
    {5, DISP_TV_MOD_480P,             720,    480, 60,  0x00},
    {5, DISP_TV_MOD_576P,             720,    576, 50,  0x00},
    {5, DISP_TV_MOD_720P_50HZ,        1280,   720, 50,  0x00},
    {5, DISP_TV_MOD_720P_60HZ,        1280,   720, 60,  0x00},

    {1, DISP_TV_MOD_1080P_24HZ,       1920,   1080, 24, 0x00},
    {5, DISP_TV_MOD_1080P_50HZ,       1920,   1080, 50, 0x00},
    {5, DISP_TV_MOD_1080P_60HZ,       1920,   1080, 60, 0x00},
    {5, DISP_TV_MOD_1080I_50HZ,       1920,   1080, 50, 0x00},
    {5, DISP_TV_MOD_1080I_60HZ,       1920,   1080, 60, 0x00},
    ...
    {1, DISP_TV_MOD_1080P_24HZ_3D_FP, 1920,   1080, 24, 0x00},
    {1, DISP_TV_MOD_720P_50HZ_3D_FP,  1280,   720,  50, 0x00},
    {1, DISP_TV_MOD_720P_60HZ_3D_FP,  1280,   720,  60, 0x00},
    {1, DISP_TV_MODE_NUM,             0,        0,   0, 0x00},
};
...
int resetDispMode(int disp, int type, int mode)
{
    ...
    if((DISP_OUTPUT_TYPE_HDMI == type) && (0xFF == mode)) {
        ...
        if (mode == -1) {
            /* Can't get a valid mode from storage,
             * reset to default mode.
             */
            mode = _hwc_device_get_current_mode(HDMI_USED);
            ALOGE("Not a valid mode, reset to default mode: %d", mode);
        }
        ...
        if ((mode == 0xff) || (mode < 0)) {
            ALOGE("resetDispMode error: use default mode %d", DISP_DEFAULT_HDMI_MODE);
            mode = DISP_DEFAULT_HDMI_MODE;
        }
    }
...
int hwcOutputSwitch(DisplayInfo *psDisplayInfo, int type, int mode)
{
    ...
    {
    case DISP_OUTPUT_TYPE_HDMI: {
        blank_disp(getHwDispByType(type));
		...
		if (ret) {
			ALOGD("switch display mode failed %d", ret);
...
int checkEnhanceMode(DisplayInfo *psDisplayInfo)
{
	...
	int value = _hwc_device_get_enhance_mode(HWC_DISPLAY_PRIMARY);

	value = getValueFromProperty(ENHANCE_BRIGHT_PROPERTY);
	if ((0 > value) || (10 < value))
		value = 5;
...
static uint64_t vsync_do_trim_timestamp(hwc_vsync_modle_t *modle) {
	...
	if ((HWC_MIN_TS_ABS_DIF < modle->period_delta[cur])) {
		...
		ALOGV("trim the last timestamp(%d)=%llu", cur, modle->ts_slot[cur]);
	}
...
static int hwc_uevent(void)
{
    ...
    while(1)
    {
        ...
        if(err > 0 && (fds.revents & POLLIN))
        {
            ...
            if(count > 0)
            {
                IsVsync = !strcmp(buf, "change@/devices/platform/soc/disp");
