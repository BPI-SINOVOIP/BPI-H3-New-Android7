...
static int hwc_blank(struct hwc_composer_device_1* dev, int disp, int blank)
{
    ...
    if (disp == HWC_DISPLAY_PRIMARY) {
        ALOGD("hwc_blank: set screen %s", blank ? "off" : "on");
        Globctx->screenOff = blank;
    }
            ...
            int para0, int para1, int para2)
{
    ...
    case DISPLAY_CMD_SETMARGIN:
        ...
        //fixme: dual-display use the same margin para
        _hwc_device_set_margin(HWC_DISPLAY_EXTERNAL, para0, para1);
        break;
