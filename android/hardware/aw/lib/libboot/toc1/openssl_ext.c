...
int sunxi_certif_verify_itself(sunxi_certif_info_t *sunxi_certif, u8 *buf, u32 len)
{
	...
	ret = sunxi_sha256_calc(hash_of_certif, p_sign_to_calc, sign_src_len);
	if(ret)
	{
	...
	for(i=0;i<sunxi_certif->pubkey.e_len;i++)
			printf("sunxi_certif->pubkey.e:%x\n",(sunxi_certif->pubkey.e)[i]);
	printf("sunxi_certif->pubkey.n_len:%x\n",sunxi_certif->pubkey.n_len);
	for(i=0;i<sunxi_certif->pubkey.n_len;i++)
			printf("sunxi_certif->pubkey.n:%x\n",(sunxi_certif->pubkey.n)[i]);
	for(i=0;i<256;i++)
			printf("sign_in_certif:%x\n",sign_in_certif[i]);*/
	ret = sunxi_rsa2048_calc(hash_of_sign,
						 sunxi_certif->pubkey.e, sunxi_certif->pubkey.e_len,
	                     sunxi_certif->pubkey.n+1, sunxi_certif->pubkey.n_len-1,
