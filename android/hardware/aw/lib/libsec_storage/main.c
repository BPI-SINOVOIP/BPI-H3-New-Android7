...
int main(int argc, char **argv)
{
	...
	if (!strncmp(argv[1], "-r", 2))
	{
		if (argv[2])
		{	// api read
			ret = private_data_read(argv[2], buf, 256, &retLen);
			if(ret < 0)
			{
			...
			// private read
			memset(buf, 0x0, 256);
			ret = sunxi_private_store_read(argv[2], buf, 256, &retLen);
			if(ret < 0)
			{
	...
	else if (!strncmp(argv[1], "-w", 2))
	{
			if (argv[2] && argv[3])
			{
				ret = private_data_write(argv[2], argv[3], strnlen(argv[3], 128));
				if(ret <0 )
				{
					printf("Write %s fail\n", argv[2]);
					goto error ;
	...
	else if (!strncmp(argv[1], "-t", 2))
	{
			...
			switch (mode)
			{
				...
				case TEST_PRI_REBOOT:
							...
							break;
				default: printf("error: no test item\n");
			}
			printf("test done\n");
	}
