...
#define DRM_M2TS_PIDS_TO_ALLOCATE                       5           /* Number of PIDs to allocate to the poRegisteredPIDs array in the encryptor context */
...
/*
** 0x000F is a reserved value per M2TS spec (ISO 13818-1).
** We will use it as an internal "token" value.
** We will never include it in the stream.
** We should never see it in the input stream.
*/
#define DRM_M2TS_ALL_ENCRYPTOR_DATA_PID                 ((DRM_WORD)0x000F)
...
#define DRM_M2TS_NULL_PACKET_PID                        0x1FFF
#define DRM_M2TS_PLAYEREADY_BYTE_RANGE_ID               "PRBR"
...
#define DRM_M2TS_DIGICIPER_II_PMT_TABLE_ID              0xC0
...
typedef enum
{
    ...
    eDRM_M2TS_LOG_DETAILS_TYPE_PAT_PACKET_DATA,
    eDRM_M2TS_LOG_DETAILS_TYPE_PMT_PACKET_DATA,
    eDRM_M2TS_LOG_DETAILS_TYPE_PES_PACKET_DATA,
}
...
typedef enum
{
    ...
    eDRM_M2TS_PROPERTY_TYPE_MULTIPLE_VIDEO_SAMPLES_PER_ECM,
...
typedef struct __tagDRM_M2TS_LOG
{
    ...
    union {
        ...
        {
            DRM_WORD                 wPID;
            DRM_WORD                 wPacketLength;
            DRM_BOOL                 fDataAligned;
            DRM_UINT64               qwPTS;
            DRM_DWORD                dwPacketNumber;
        } PESPacketData;
    } Details;
...
typedef struct __tagDRM_M2TS_ENCRYPTOR_CONTEXT_PID_DATA
{
    DRM_WORD                         wPID;
    DRM_LICENSE_HANDLE               hLicense;
    DRM_M2TS_PRO_TYPE                ePROType;
    DRM_UINT64                       qwCurrentKeyDuration;              /* The duration in hns for the current key for this PID, 0 - no key rotation */
    DRM_BOOL                         fWaitingForNewKey;
    DRM_GUID                         oKeyId;
} DRM_M2TS_ENCRYPTOR_CONTEXT_PID_DATA;
...
typedef struct __tagDRM_M2TS_ENCRYPTOR_CONTEXT
{
    ...
    DRM_BOOL                         fOneVideoPerECM;
    DRM_BOOL                         fPendingByteRange;                 /* Found a byte range start marker but not processed yet */
    DRM_BYTE                         bScramblingControl;
    DRM_DWORD                        dwNextPacketNo;
    DRM_BOOL                         fAllStreamsUseSameEncryptorData;   /* TRUE if the same encryptor data should be used for all streams, FALSE otherwise */
    DRM_DWORD                        cRegisteredPIDsAllocated;          /* Number of items in poRegisteredPIDs array */
    DRM_DWORD                        cRegisteredPIDs;                   /* Number of items used in poRegisteredPIDs array */
    DRM_M2TS_ENCRYPTOR_CONTEXT_PID_DATA *poRegisteredPIDs;              /*
...
typedef struct __tagDRM_M2TS_SUBSAMPLE_CONTEXT
{
    ...
    DRM_WORD                         cbCountdownToProtected;    /*
...
#define DRM_M2TS_BYTE_RANGE_FLAG_BEGIN           0x1
#define DRM_M2TS_BYTE_RANGE_FLAG_END             0x2

typedef struct _tagDRM_M2TS_ByteRangePayload
{
    DRM_BYTE                         rgbByteRangeID[4]; /* with a fixed value of "PRBR" */
    DRM_DWORD                        dwByteRangeFlagsBigEndian;
    DRM_UINT64                       qwByteRangeOffsetBigEndian;
    DRM_DWORD                        dwByteRangeLengthBigEndian;
} DRM_M2TS_ByteRangePayload;

typedef struct
{
    DRM_WORD    wPID;               /* PID of the MPEG2 TS stream */
    DRM_BOOL    fAllowKeyRotation;  /* TRUE if the result of a key rotation due to exceeding the duration last specified for this stream in Drm_M2ts_SetKeyData */
    DRM_BYTE    bStreamType;        /* M2TS stream type specified in PMT packet, e.g. 0x02 (DRM_M2TS_STREAM_TYPE_MPEG2_VIDEO) */
} DRM_M2TS_STREAM_DATA;

PREFAST_PUSH_DISABLE_EXPLAINED(__WARNING_NONCONST_PARAM_25004, "Prefast Noise: Callback contexts should never be const");
typedef DRM_RESULT ( DRM_CALL *Drm_M2ts_Encrypt_Callback )(
    __inout_ecount_opt( 1 )     DRM_VOID                                    *f_pvEncryptCallbackContext,
    __in                  const DRM_M2TS_STREAM_DATA                        *f_pStreamData );
