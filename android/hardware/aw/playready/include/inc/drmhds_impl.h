...
#define CALC_SLOTCONTEXTLEN(cfg, pcb)                                                           \
do{ DRM_DWORD __dwSpaceRequired = 0;                                                            \
    ChkDR( DRM_DWordAdd( sizeof(_SlotContext), GetMemsize_ChildBlock(cfg), pcb ) );             \
    ChkDR( DRM_DWordAdd( *pcb, GetMemsize_DataBlock(), pcb ) );                                 \
    ChkDR( DRM_DWordAdd( sizeof(_SlotContext), sizeof(_ChildBlockHDR), &__dwSpaceRequired ) );  \
    ChkDR( DRM_DWordAdd( __dwSpaceRequired, GetMemsize_DataBlock(), &__dwSpaceRequired ) );     \
    ChkBOOL( *pcb >= __dwSpaceRequired, DRM_E_ARITHMETIC_OVERFLOW );                            \
} while( FALSE )
...
DRM_NO_INLINE DRM_API DRM_RESULT DRM_HDSIMPL_AllocBlockBuffer(
