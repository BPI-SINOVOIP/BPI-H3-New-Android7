...
static int set_fm_volume(struct codec_client *client, int path, int volume)
{
	...
	mixer_ctl_set_value(client->mixer_ctls->linein_to_l_r_output_mixer_gain_ctrl, 0, mixer_vol[val]);

	if (speaker_on){
		mixer_ctl_set_value(client->mixer_ctls->speaker_vol_ctrl, 0, spk_vol[val]);
	} else {
		mixer_ctl_set_value(client->mixer_ctls->headphone_vol_ctrl, 0, hp_vol[val]);
	} 
