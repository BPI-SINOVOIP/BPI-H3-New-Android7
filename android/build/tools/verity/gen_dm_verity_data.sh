...
IMG=$1
	...
	echo "Input image wrong!"
	exit 1
fi
echo "Generate verity block data for file $IMG"
TARGET_PATH=$OUT/verity
RUN_PATH=$DEVICE/verity
VERITY_TOOL="$RUN_PATH/veritysetup"
DM_MERGE_TOOL="$RUN_PATH/dm_merge"
#IMG=system.img
#IMG="/home/ryanchen/work/box/a80/android/out/target/product/jaws-optimus/system.img.new"
BLK_SIZE=4096
...
${VERITY_TOOL} format $IMG $TARGET_PATH/hash $TARGET_PATH/raw_table
...
${DM_MERGE_TOOL} -m  $TARGET_PATH/sign  $TARGET_PATH/table_tmp  $TARGET_PATH/metadata
...
dd of=$TARGET_PATH/verity_block if=$TARGET_PATH/hash_dev bs=${BLK_SIZE} count=`expr ${HASH_CNT} + ${METADATA_BLK_OFFSET}`>/dev/null 2>&1
