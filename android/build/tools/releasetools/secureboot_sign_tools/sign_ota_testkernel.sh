...
Usage:
	sign_ota_testkernel.sh -i [old_ota.zip] -k [key] -o [secureboot_signed_ota.zip] -f [sige flag]
EOT
}
...
DEFINE_string 'flag' '' 'sign flag' 'f'
...
echo ${OLD_OTA}
unzip -o $OLD_OTA boot.img  -d ${TEMP_DIR}/img
assert "unzip ota fail"
cp ${TEMP_DIR}/img/toc1.fex  ${TEMP_DIR}/img/toc1.fex_bk
cp ${TEMP_DIR}/img/boot.img ${TEMP_DIR}/img/boot.fex
cp ${ROOT_DIR}/img/boot.fex ${TEMP_DIR}/img/recovery.fex
...
cp toc1.fex_bk  toc1.fex
fi

zip -j $SIGNED_OTA toc1.fex
#zip -j $SIGNED_OTA toc0.fex
