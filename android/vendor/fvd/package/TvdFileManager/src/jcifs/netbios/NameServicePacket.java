...
abstract class NameServicePacket {
    ...
    static int readInt4(byte[] src, int srcIndex) {
        return ((src[srcIndex] & 0xFF) << 24) + ((src[srcIndex + 1] & 0xFF) << 16)
                + ((src[srcIndex + 2] & 0xFF) << 8) + (src[srcIndex + 3] & 0xFF);
    }
...
ArrayList<NbtAddress> tmp = new ArrayList<NbtAddress>();

    int nameTrnId;

    int opCode, resultCode, questionCount, answerCount, authorityCount, additionalCount;
    boolean received, isResponse, isAuthAnswer, isTruncated, isRecurDesired, isRecurAvailable,
            isBroadcast;
    ...
    int writeHeaderWireFormat(byte[] dst, int dstIndex) {
        ...
        dst[dstIndex + OPCODE_OFFSET] = (byte) ((isResponse ? 0x80 : 0x00) + ((opCode << 3) & 0x78)
                + (isAuthAnswer ? 0x04 : 0x00) + (isTruncated ? 0x02 : 0x00) + (isRecurDesired ? 0x01
                : 0x00));
        dst[dstIndex + OPCODE_OFFSET + 1] = (byte) ((isRecurAvailable ? 0x80 : 0x00)
                + (isBroadcast ? 0x10 : 0x00) + (resultCode & 0x0F));
    ...
    int readHeaderWireFormat(byte[] src, int srcIndex) {
        ...
        isRecurAvailable = ((src[srcIndex + OPCODE_OFFSET + 1] & 0x80) == 0) ? false : true;
    ...
    public String toString() {
        String opCodeString, resultCodeString, questionTypeString, questionClassString, recordTypeString, recordClassString;
        ...
        return new String("nameTrnId=" + nameTrnId + ",isResponse=" + isResponse + ",opCode="
                ...
                + questionCount + ",answerCount=" + answerCount + ",authorityCount="
                + authorityCount + ",additionalCount=" + additionalCount + ",questionName="
                + questionName + ",questionType=" + questionTypeString + ",questionClass="
                + (questionClass == IN ? "IN" : "0x" + Hexdump.toHexString(questionClass, 4))
                + ",recordName=" + recordName + ",recordType=" + recordTypeString + ",recordClass="
                + (recordClass == IN ? "IN" : "0x" + Hexdump.toHexString(recordClass, 4)) + ",ttl="
