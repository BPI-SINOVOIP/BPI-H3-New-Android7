...
class NameServiceClient implements Runnable {
    ...
    static final int DEFAULT_SO_TIMEOUT = 1000;
    ...
    static final int DEFAULT_RETRY_TIMEOUT = 500;

    static final int RESOLVER_LMHOSTS = 1;
    static final int RESOLVER_BCAST = 2;
    static final int RESOLVER_WINS = 3;

    private static final int SND_BUF_SIZE = Config.getInt("jcifs.netbios.snd_buf_size",
            DEFAULT_SND_BUF_SIZE);
    private static final int RCV_BUF_SIZE = Config.getInt("jcifs.netbios.rcv_buf_size",
            DEFAULT_RCV_BUF_SIZE);
    private static final int SO_TIMEOUT = Config.getInt("jcifs.netbios.soTimeout",
            DEFAULT_SO_TIMEOUT);
    private static final int RETRY_COUNT = Config.getInt("jcifs.netbios.retryCount",
            DEFAULT_RETRY_COUNT);
    private static final int RETRY_TIMEOUT = Config.getInt("jcifs.netbios.retryTimeout",
    ...
    void tryClose() {
        synchronized(LOCK) {
            /*
             * Yes, there is the potential to drop packets because we might
             * close the socket during a request. However the chances are slim
             * and the retry code should ensure the overall request is serviced.
             * The alternative complicates things more than I think is worth it.
             */
        ...
        synchronized(response) {
            while (max-- > 0) {
                try {
                    ...
                    while (timeout > 0) {
                        response.wait(SO_TIMEOUT+1500);

                        /*
                         * JetDirect printer can respond to regular broadcast
                         * query with node status so we need to check to make
                         * sure that the record type matches the question type
                         * and if not, loop around and try again.
                         */
                ...
                synchronized(LOCK) {
                    if (NbtAddress.isWINS(request.addr) == false)
                        break;
                    /*
                     * Message was sent to WINS but failed to receive response.
    ...
    NbtAddress[] getAllByNameX(Name name, InetAddress addr) throws UnknownHostException {
        int n;
        NameQueryRequest request = new NameQueryRequest(name);
        NameQueryResponse response = new NameQueryResponse();
    ...
    NbtAddress getByName( Name name, InetAddress addr )
        ...
        if (addr != null) { /*
                             * UniAddress calls always use this because it
                             * specifies addr
                             */
        ...
        /*
         * If a target address to query was not specified explicitly with the
         * addr parameter we fall into this resolveOrder routine.
         */
            ...
            try {
                switch (resolveOrder[i]) {
                    case RESOLVER_LMHOSTS:
                        ...
                        if (ans != null) {
                            ans.hostName.srcHashCode = 0; // just has to be
                                                          // different
                                                          // from other methods
                            return ans;
                        }
                    ...
                    case RESOLVER_BCAST:
                        ...
                        while (n-- > 0) {
                            ...
                            if (response.received && response.resultCode == 0) {
                                 ...
                                 * address being cached we must augment the
                                 * addresses name's hashCode to distinguish
                                 * those resolved by Lmhosts, WINS, or BCAST.
                                 * Otherwise a failed query from say WINS would
                                 * get pulled out of the cache for a BCAST on
                                 * the same name.
                                 */
                                response.addrEntry[0].hostName.srcHashCode = request.addr
                                        .hashCode();
    ...
    NbtAddress[] getNodeStatus(NbtAddress addr) throws UnknownHostException {
        ...
        while (n-- > 0) {
            ...
            if (response.received && response.resultCode == 0) {
                 ...
                 * for the addresses hostname or failed lookups for one type
                 * will be cached and cause other types to fail even though they
                 * may not be the authority for the name. For example, if a WINS
