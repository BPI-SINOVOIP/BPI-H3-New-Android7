...
public final class NbtAddress {
    ...
    /*
     * This is a special name that means all hosts. If you wish to find all
     * hosts on a network querying a workgroup group name is the preferred
     * method.
     */
    ...
    /**
     * A special generic name specified when connecting to a host for which a
     * name is not known. Not all servers respond to this name.
     */
     ...
     * Natrually the <code>jcifs.netbios.nameserver</code> property must be set.
     */
    ...
    /**
     * A Hybrid node tries to resolve a name using the nameserver first. If that
     * fails use the broadcast address. This is the default if a nameserver is
     * provided. This is the behavior of Microsoft Windows machines.
     */

    public static final int H_NODE = 3;

    static final InetAddress[] NBNS = Config.getInetAddressArray("jcifs.netbios.wins", ",",
            new InetAddress[0]);

    /*
     * Construct the shared static client object that will conduct all encoding
     * and decoding of NetBIOS name service messages as well as socket IO in a
     * synchronized fashon.
     */
    ...
    private static final int CACHE_POLICY = Config.getInt("jcifs.netbios.cachePolicy",
    ...
    static final byte[] UNKNOWN_MAC_ADDRESS = new byte[] {(byte) 0x00, (byte) 0x00, (byte) 0x00,
    ...
    static {
        ...
        /*
         * Determine the InetAddress of the local interface if one was not
         * specified.
         */
        ...
        if (localInetAddress == null) {
            try {
                ...
                /*
                 * Java cannot determine the localhost. This is basically a
                 * config issue on the host. There's not much we can do about
                 * it. Just to suppress NPEs that would result we can create a
                 * possibly bogus address. Pretty sure the below cannot actually
                 * thrown a UHE tho.
                 */
                try {
        ...
        /*
         * If a local hostname was not provided a name like JCIFS34_172_A6 will
         * be dynamically generated for the client. This is primarily
         * (exclusively?) used as a CallingName during session establishment.
         */
        ...
        /*
         * Create an NbtAddress for the local interface with the name deduced
         * above possibly with scope applied and cache it forever.
         */
        localName = new Name(localHostname, 0x00, Config.getProperty("jcifs.netbios.scope", null));
        localhost = new NbtAddress(localName, localInetAddress.hashCode(), false, B_NODE, false,
                false, true, false, UNKNOWN_MAC_ADDRESS);
     ...
     * NetBIOS name like "freto" or an IP address like "192.168.1.15". It cannot
    ...
    /**
     * Determines the address of a host given it's host name. NetBIOS names also
     * have a <code>type</code>. Types(aka Hex Codes) are used to distiquish the
     * various services on a host. <a href="../../../nbtcodes.html">Here</a> is
    ...
    /*
     * The additional <code>svr</code> parameter specifies the address to query.
     * This might be the address of a specific host, a name server, or a
     * broadcast address.
     */
    ...
    public static NbtAddress[] getAllByNameX( String host,
                                        int type,
                                        String scope,
                                        InetAddress svr )
                                        throws UnknownHostException {
        return CLIENT.getAllByNameX( new Name( host, type, scope ), svr );
    }
    /**
     * Retrieve all addresses of a host by it's address. NetBIOS hosts can have
            ...
            throw new UnknownHostException(
                    "no name with type 0x"
                            ...
                            + (((addr.hostName.scope == null) || (addr.hostName.scope.length() == 0)) ? " with no scope"
                                    : " with scope " + addr.hostName.scope) + " for host "
                            + addr.getHostAddress());
        }
    }
    ...
    NbtAddress(Name hostName, int address, boolean groupName, int nodeType, boolean isBeingDeleted,
            boolean isInConflict, boolean isActive, boolean isPermanent, byte[] macAddress) {

        /*
         * The NodeStatusResponse.readNodeNameArray method may also set this
         * information. These two places where node status data is populated
         * should be consistent. Be carefull!
         */
        this.hostName = hostName;
        this.address = address;
        this.groupName = groupName;
    ...
    /*
     * Guess next called name to try for session establishment. These methods
     * are used by the smb package.
     */
        ...
        if (calledName == hostName.name) {
            ...
            try {
                ...
                if (hostName.hexCode == 0x1D) {
                    ...
                    /*
                     * 'this' has been updated and should now have a real
                     * NetBIOS name
                     */
                    calledName = null;
     ...
     * WINS for example. Natrually it is not practical for every NbtAddress to
     * be populated will all state requiring a Node Status on every host
    ...
    /**
     * Returns this IP adress as a {@link java.lang.String} in the form
     * "%d.%d.%d.%d".
     */
    ...
    /**
     * Returned the hex code associated with this name(e.g. 0x20 is for the file
     * service)
     */
    ...
    /**
     * Returns a hashcode for this IP address. The hashcode comes from the IP
     * address and is not generated from the string representation. So because
     * NetBIOS nodes can have many names, all names associated with an IP will
     ...
     * are compared. Similar to the {@link #hashCode} method, the comparison is
     * based on the integer IP address and not the string representation.
     */
