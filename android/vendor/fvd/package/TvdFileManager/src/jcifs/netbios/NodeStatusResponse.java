...
class NodeStatusResponse extends NameServicePacket {
    ...
    /*
     * It is a little awkward but prudent to pass the quering address so that it
     * may be included in the list of results. IOW we do not want to create a
     * new NbtAddress object for this particular address from which the query is
     * constructed, we want to populate the data of the existing address that
    ...
    private int readNodeNameArray(byte[] src, int srcIndex) {
        ...
        try {
            for (int i = 0; i < numberOfNames; srcIndex += 18, i++) {
                ...
                /*
                 * The NbtAddress object used to query this node will be in the
                 * list returned by the Node Status. A new NbtAddress object
                 * should not be created for it because the original is
                 * potentially being actively referenced by other objects. We
                 * must populate the existing object's data explicitly (and
                 * carefully).
                 */
                if (!addrFound
                        && queryAddress.hostName.hexCode == hexCode
                        && (queryAddress.hostName == NbtAddress.UNKNOWN_NAME || queryAddress.hostName.name
                                .equals(n))) {
                    ...
                    addressArray[i] = new NbtAddress(new Name(n, hexCode, scope),
                            queryAddress.address, groupName, ownerNodeType, isBeingDeleted,
                            isInConflict, isActive, isPermanent, macAddress);
                }
            }
