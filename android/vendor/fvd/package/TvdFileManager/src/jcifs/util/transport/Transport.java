...
/**
 * This class simplifies communication for protocols that support multiplexing
 * requests. It encapsulates a stream and some protocol knowledge (provided by a
 * concrete subclass) so that connecting, disconnecting, sending, and receiving
 * can be syncronized properly. Apparatus is provided to send and receive
...
public abstract class Transport implements Runnable {
    ...
    /*
     * state values 0 - not connected 1 - connecting 2 - run connected 3 -
     * connected 4 - error
     */
    int state = 0;
    int substate = 0;
        ...
        synchronized(response_map) {
            ...
            try {
                ...
                while (!response.isReceived) {
                    ...
                    if (timeout <= 0) {
                        throw new TransportException(name + " timedout waiting for response to "
                                + request);
                    }
                }
    ...
    /*
     * Build a connection. Only one thread will ever call this method at any one
     * time. If this method throws an exception or the connect timeout expires
     * an encapsulating TransportException will be thrown from connect and the
     * transport will be in error.
     */
    ...
    public void run() {
        ...
        try {
             ...
             * thread.wait( timeout ) cannot reaquire the lock and return which
             * would render the timeout effectively useless.
             */
            doConnect();
        } catch (Exception ex) {
