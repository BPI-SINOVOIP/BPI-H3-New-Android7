...
public class Encdec {
    ...
    public static int dec_uint32le(byte[] src, int si) {
        return (src[si] & 0xFF) | ((src[si + 1] & 0xFF) << 8) | ((src[si + 2] & 0xFF) << 16)
                | ((src[si + 3] & 0xFF) << 24);
    }
    ...
    public static int enc_time(Date date, byte[] dst, int di, int enc) {
        ...
        switch (enc) {
            ...
            case TIME_1904_SEC_32BE:
                return enc_uint32be(
                        (int) ((date.getTime() / 1000L + SEC_BETWEEEN_1904_AND_1970) & 0xFFFFFFFF),
                        dst, di);
    ...
    public static Date dec_time(byte[] src, int si, int enc) {
        ...
        switch (enc) {
            ...
            case TIME_1904_SEC_32BE:
                return new Date(
                        ((dec_uint32be(src, si) & 0xFFFFFFFFL) - SEC_BETWEEEN_1904_AND_1970) * 1000L);
            case TIME_1904_SEC_32LE:
                return new Date(
                        ((dec_uint32le(src, si) & 0xFFFFFFFFL) - SEC_BETWEEEN_1904_AND_1970) * 1000L);
            case TIME_1601_NANOS_64BE:
