 ...
 * The <code>System</code> properties will then populate the <code>Config</code>
 * as well potentially overwriting properties from the file. Thus properties
 * provided on the commandline with the <code>-Dproperty.name=value</code> VM
 * parameter will override properties from the configuration file.
 * <p>
 * There are several ways to set jCIFS properties. See the <a
 * href="../overview-summary.html#scp">overview page of the API
...
public class Config {
    ...
    static {
        ...
        try {
            ...
            if (log.level >= 2) {
                log.println("WARNING: The default OEM encoding "
     ...
     * method is not called before attempting to create an SMB URL with the URL
     * class the following exception will occur: <blockquote>
     * 
     * <pre>
    ...
    public static void registerSmbURLHandler() {
        ...
        if (ver.startsWith("1.1.") || ver.startsWith("1.2.")) {
            throw new RuntimeException(
                    "jcifs-0.7.0b4+ requires Java 1.3 or above. You are running " + ver);
        }
    ...
    /**
     * Set the default properties of the static Properties used by
     * <tt>Config</tt>. This permits a different Properties object/file to be
     * used as the source of properties for use by the jCIFS library. The
     * Properties must be set <i>before jCIFS classes are accessed</i> as most
     * jCIFS classes load properties statically once. Using this method will
     * also override properties loaded using the <tt>-Djcifs.properties=</tt>
    ...
    /**
     * Retrieve a <code>String</code>. If the key cannot be found, the provided
     * <code>def</code> default parameter will be returned.
     */
     ...
     * converted to an <code>int</code>, the provided default argument will be
     * returned.
     */
     ...
     * converted to a <code>long</code>, the provided default argument will be
     * returned.
     */
    ...
    /**
     * Retrieve an <code>InetAddress</code>. If the address is not an IP address
     * and cannot be resolved <code>null</code> will be returned.
     */

    public static InetAddress getInetAddress(String key, InetAddress def) {
        // modified by chenjd, if it's the broadcast address, return value by
        // LocalNetwork
        String addr = null;
        if (LocalNetwork.B_ADDR.equals(key)) {
            addr = LocalNetwork.getCurBAddr();
        } else {
            addr = prp.getProperty(key);
        }
        if (addr != null) {
            try {
    ...
    /**
     * Retrieve an array of <tt>InetAddress</tt> created from a property value
     * containting a <tt>delim</tt> separated list of hostnames and/or
     * ipaddresses.
     */
