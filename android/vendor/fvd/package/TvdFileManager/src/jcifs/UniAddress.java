...
/**
 * <p>
 * Under normal conditions it is not necessary to use this class to use jCIFS
 * properly. Name resolusion is handled internally to the <code>jcifs.smb</code>
 * package.
 * <p>
 * This class is a wrapper for both {@link jcifs.netbios.NbtAddress} and
 * {@link java.net.InetAddress}. The name resolution mechanisms used will
 * systematically query all available configured resolution services including
 * WINS, broadcasts, DNS, and LMHOSTS. See <a href="../../resolver.html">Setting
 * Name Resolution Properties</a> and the <code>jcifs.resolveOrder</code>
 * property. Changing jCIFS name resolution properties can greatly affect the
 * behavior of the client and may be necessary for proper operation.
 * <p>
 * This class should be used in favor of <tt>InetAddress</tt> to resolve
 * hostnames on LANs and WANs that support a mixture of NetBIOS/WINS and DNS
 * resolvable hosts.
 */
...
public class UniAddress {
    ...
    public  static final String TAG = "UniAddress";
    ...
    static {
        ...
        if (ro == null || ro.length() == 0) {
            ...
            /*
             * No resolveOrder has been specified, use the default which is
             * LMHOSTS,WINS,BCAST,DNS or just LMHOSTS,BCAST,DNS if
             * jcifs.netbios.wins has not been specified.
             */
    ...
    static class QueryThread1 extends Thread {
        ...
        InetAddress svr;
        UnknownHostException uhe;
        QueryThread1( Sem sem, String host, int type,
        ...
        public void run() {
            try {
                Log.d(TAG,"QueryThread1 Run");
                ans = NbtAddress.getAllByNameX( host, type, scope, svr );
            } catch( UnknownHostException uhe ) {
                this.uhe = uhe;
            } catch( Exception ex ) {
                this.uhe = new UnknownHostException( ex.getMessage() );
                ...
                synchronized( sem ) {
                    Log.d(TAG,"QueryThread1 ,enter finally");
                    sem.count--;
                    sem.notify();
                }
            }
    ...
    static NbtAddress[] lookupServerOrWorkgroup(String name, InetAddress svr)
        ...
        QueryThread1 q1x = new QueryThread1( sem, name, 0x1E, null, svr );
        QueryThread1 q20 = new QueryThread1( sem, name, 0x00, null, svr );
                ...
                switch (resolveOrder[i]) {
                    ...
                    case RESOLVER_WINS:
                        ...
                        if (possibleNTDomainOrWorkgroup) {
                            ...
                            addr = NbtAddress.getByName(hostname, 0x20, null,
...
public static UniAddress[] getAllByNameX( String hostname,
        ...
        for( i = 0; i < resolveOrder.length; i++ ) {
            try {
                switch( resolveOrder[i] ) {
                    case RESOLVER_LMHOSTS:
                        Log.d(TAG,"Begin RESOLVER_LMHOST");
                        if(( addr = Lmhosts.getByName( hostname )) == null ) {
                            continue;
                        }
                        break;
                    case RESOLVER_WINS:
                         Log.d(TAG,"Begin RESOLVER_WINS");
                    ...
                    case RESOLVER_BCAST:
                        ...
                        Log.d(TAG,"BEGIN RESOLVER_BCAST,hostname.length ="+hostname.length());
                        ...
                        if( possibleNTDomainOrWorkgroup ) {
                            Log.d(TAG,"RESOLVER_BCAST,first,baddr = "+baddr);
                            Object[] Addr = lookupServerOrWorkgroup( hostname, baddr );
                            UniAddress[] uniAddr = new UniAddress[Addr.length];
                            if (Addr != null) {
                                for (int n = 0;n < Addr.length;n++) {
                                    uniAddr[n] = new UniAddress(Addr[n]);
                                }
                            }
                             return uniAddr;
                        } else {
                            //addr = NbtAddress.getByName( hostname, 0x20, null, baddr );
                            Log.d(TAG,"RESOLVER_BCAST");
                            addr = NbtAddress.getByName( hostname, 0x20, null, baddr );
                        }
                        break;
                    case RESOLVER_DNS:
                         Log.d(TAG,"BEGIN RESOLVER_DNS");
                        if( isAllDigits( hostname )) {
                            Log.d(TAG,"RESOLVER_DNS,throw UnknownHostException");
                            throw new UnknownHostException( hostname );
                        }
                        InetAddress[] iaddrs = InetAddress.getAllByName( hostname );
                        UniAddress[] addrs = new UniAddress[iaddrs.length];
                    ...
                    default:
                       {
                        Log.d(TAG,"in default,throw UnknownHostException");
                        throw new UnknownHostException( hostname );
                       }
                }
                UniAddress[] addrs = new UniAddress[1];
        ...
        Log.d(TAG,"after resolve,throw UnknownHostException");
        throw new UnknownHostException( hostname );
    }
    /**
     * Perform DNS SRV lookup on successively shorter suffixes of name and
     * return successful suffix or throw an UnknownHostException. import
     ...
     * nnfe) { uhe = new UnknownHostException(nnfe.getMessage()); } int dot =
    ...
    /**
     * Compare two addresses for equality. Two <tt>UniAddress</tt>s are equal if
     * they are both <tt>UniAddress</tt>' and refer to the same IP address.
     */
    ...
    /*
     * public boolean equals( Object obj ) { return obj instanceof UniAddress &&
     * addr.hashCode() == obj.hashCode(); }
     */

    /**
     * Guess first called name to try for session establishment. This method is
     * used exclusively by the <tt>jcifs.smb</tt> package.
     */
    ...
    /**
     * Guess next called name to try for session establishment. This method is
     * used exclusively by the <tt>jcifs.smb</tt> package.
     */
