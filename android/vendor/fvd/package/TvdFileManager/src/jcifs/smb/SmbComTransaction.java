...
abstract class SmbComTransaction extends ServerMessageBlock implements Enumeration {
    ...
    private static final int DEFAULT_MAX_DATA_COUNT = Config.getInt(
            "jcifs.smb.client.transaction_buf_size", SmbComTransaction.TRANSACTION_BUF_SIZE) - 512;
    ...
    byte subCommand;
    String name = "";
    int maxBufferSize; // set in SmbTransport.sendTransaction() before
                       // nextElement called
    ...
    public Object nextElement() {
        if (isPrimary) {
            ...
            // caclulate parameterDisplacement before calculating new
            // parameterCount
    ...
    int writeParameterWordsWireFormat(byte[] dst, int dstIndex) {
        ...
        dstIndex += 2;
        // writeInt2(( parameterCount == 0 ? 0 : parameterOffset ), dst,
        // dstIndex );
    ...
    public String toString() {
        return new String(super.toString() + ",totalParameterCount=" + totalParameterCount
                + ",totalDataCount=" + totalDataCount + ",maxParameterCount=" + maxParameterCount
                + ",maxDataCount=" + maxDataCount + ",maxSetupCount=" + (int) maxSetupCount
                + ",flags=0x" + Hexdump.toHexString(flags, 2) + ",timeout=" + timeout
                + ",parameterCount=" + parameterCount + ",parameterOffset=" + parameterOffset
                + ",parameterDisplacement=" + parameterDisplacement + ",dataCount=" + dataCount
                + ",dataOffset=" + dataOffset + ",dataDisplacement=" + dataDisplacement
                + ",setupCount=" + setupCount + ",pad=" + pad + ",pad1=" + pad1);
    }
}
