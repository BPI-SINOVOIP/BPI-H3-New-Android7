 ...
 * users have the necessary permissions to access objects based on these
 * entries.
 * <p>
 * To fully understand the information exposed by this class a description of
 * the access check algorithm used by Windows is required. The following is a
 * basic description of the algorithm. For a more complete description we
 * recommend reading the section on Access Control in Keith Brown's
 * "The .NET Developer's Guide to Windows Security" (which is also available
 * online).
 * <p>
 * Direct ACEs are evaluated first in order. The SID of the user performing the
 * operation and the desired access bits are compared to the SID and access mask
 * of each ACE. If the SID matches, the allow/deny flags and access mask are
 * considered. If the ACE is a "deny" ACE and <i>any</i> of the desired access
 * bits match bits in the access mask of the ACE, the whole access check fails.
 * If the ACE is an "allow" ACE and <i>all</i> of the bits in the desired access
 * bits match bits in the access mask of the ACE, the access check is
 * successful. Otherwise, more ACEs are evaluated until all desired access bits
 * (combined) are "allowed". If all of the desired access bits are not "allowed"
 * the then same process is repeated for inherited ACEs.
 * <p>
 * For example, if user <tt>WNET\alice</tt> tries to open a file with desired
 ...
 * <tt>WNET\alice</tt> is in the local <tt>Administrators</tt> group the access
 * check will succeed because the inherited ACE allows local
 * <tt>Administrators</tt> both <tt>FILE_READ_DATA</tt> and
 * <tt>FILE_WRITE_DATA</tt> access.
 */
...
public class ACE {
    ...
    /**
     * Returns true if this ACE is an inherited ACE and false if it is a direct
     * ACE.
     * <p>
     * Note: For reasons not fully understood, <tt>FLAGS_INHERITED</tt> may not
     * be set within all security descriptors even though the ACE was in face
     * inherited. If an inherited ACE is added to a parent the Windows ACL
     * editor will rebuild all children ACEs and set this flag accordingly.
     */
    ...
    /**
     * Returns the flags for this ACE. The </tt>isInherited()</tt> method checks
     * the <tt>FLAGS_INHERITED</tt> bit in these flags.
     */
    public int getFlags() {
        return flags;
    }
    ...
    /**
     * Returns the 'Apply To' text for inheritance of ACEs on directories such
     * as 'This folder, subfolder and files'. For files the text is always 'This
     * object only'.
     */
    ...
    /**
     * Returns the access mask accociated with this ACE. Use the constants for
     * <tt>FILE_READ_DATA</tt>, <tt>FILE_WRITE_DATA</tt>, <tt>READ_CONTROL</tt>,
     * <tt>GENERIC_ALL</tt>, etc with bitwise operators to determine which bits
     * of the mask are on or off.
     */
     ...
     * <p>
     * Note: This function should probably be changed to return SDDL fragments
