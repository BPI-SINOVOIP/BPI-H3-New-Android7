...
class SmbComOpenAndX extends AndXServerMessageBlock {
    ...
    int flags, desiredAccess, searchAttributes, fileAttributes, creationTime, openFunction,
            allocationSize;
    ...
    SmbComOpenAndX(String fileName, int access, int flags, ServerMessageBlock andx) {
        ...
        desiredAccess &= ~0x1; // Win98 doesn't like GENERIC_READ ?! -- get
                               // Access Denied.
    ...
    public String toString() {
        return new String("SmbComOpenAndX[" + super.toString() + ",flags=0x"
                + Hexdump.toHexString(flags, 2) + ",desiredAccess=0x"
                + Hexdump.toHexString(desiredAccess, 4) + ",searchAttributes=0x"
                + Hexdump.toHexString(searchAttributes, 4) + ",fileAttributes=0x"
                + Hexdump.toHexString(fileAttributes, 4) + ",creationTime="
                + new Date(creationTime) + ",openFunction=0x"
                + Hexdump.toHexString(openFunction, 2) + ",allocationSize=" + allocationSize
                + ",fileName=" + path + "]");
    }
}
