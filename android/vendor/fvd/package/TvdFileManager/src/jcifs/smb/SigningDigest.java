...
public class SigningDigest implements SmbConstants {
        ...
        try {
            switch (LM_COMPATIBILITY) {
                ...
                case 2:
                    ...
                    System.arraycopy(auth.getUnicodeHash(transport.server.encryptionKey), 0,
                            macSigningKey, 16, 24);
                    break;
                case 3:
                case 4:
    ...
    /**
     * Performs MAC signing of the SMB. This is done as follows. The signature
     ...
     */
    void sign(byte[] data, int offset, int length, ServerMessageBlock request,
    ...
    boolean verify(byte[] data, int offset, ServerMessageBlock response) {
        ...
        if (response.command == ServerMessageBlock.SMB_COM_READ_ANDX) {
            /*
             * SmbComReadAndXResponse reads directly from the stream into
             * separate byte[] b.
             */
        ...
        for (int i = 0; i < 8; i++) {
            if (signature[i] != data[offset + ServerMessageBlock.SIGNATURE_OFFSET + i]) {
                if (log.level >= 2) {
                    ...
                    Hexdump.hexdump(log, data, offset + ServerMessageBlock.SIGNATURE_OFFSET, 8);
                }
    ...
    public String toString() {
        return "LM_COMPATIBILITY=" + LM_COMPATIBILITY + " MacSigningKey="
                + Hexdump.toHexString(macSigningKey, 0, macSigningKey.length);
    }
}
