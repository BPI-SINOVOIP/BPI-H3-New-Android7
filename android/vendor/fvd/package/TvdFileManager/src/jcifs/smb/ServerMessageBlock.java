...
abstract class ServerMessageBlock extends Response implements Request, SmbConstants {
    ...
    static final byte[] header = {(byte) 0xFF, (byte) 'S', (byte) 'M', (byte) 'B', (byte) 0x00,
    ...
    static int readInt4(byte[] src, int srcIndex) {
        return (src[srcIndex] & 0xFF) + ((src[srcIndex + 1] & 0xFF) << 8)
                + ((src[srcIndex + 2] & 0xFF) << 16) + ((src[srcIndex + 3] & 0xFF) << 24);
    }
     ...
     * and well as their responses for each type however the actuall
     * implementations of the readXxxWireFormat and writeXxxWireFormat methods
     * may not be in place. For example at the time of this writing the
     * readXxxWireFormat for requests and the writeXxxWireFormat for responses
     * are not implemented and simply return 0. These would need to be completed
     ...
     * field should be used for calculating that. This would likely be zero but
     * an implemantation that encorporates the transport header(for efficiency)
     * might use a different initial bufferIndex. For example, to eliminate
     * copying data when writing NbtSession data one might manage that 4 byte
     * header specifically and therefore the initial bufferIndex, and thus
     * headerStart, would be 4).(NOTE: If one where looking for a way to improve
     * perfomance this is precisly what you would want to do as the
     * jcifs.netbios.SocketXxxputStream classes arraycopy all data read or
     * written into a new buffer shifted over 4!)
     */

    byte command, flags;
    int headerStart, length, batchLevel, errorCode, flags2, tid, pid, uid, mid, wordCount,
            byteCount;
    ...
    int writeString(String str, byte[] dst, int dstIndex, boolean useUnicode) {
        ...
        try {
            if (useUnicode) {
                ...
                System.arraycopy(str.getBytes("UnicodeLittleUnmarked"), 0, dst, dstIndex,
                        str.length() * 2);
        ...
        try {
            if (useUnicode) {
                ...
                while (src[srcIndex + len] != (byte) 0x00 || src[srcIndex + len + 1] != (byte) 0x00) {
                    ...
                    if (len > maxLen) {
                        if (log.level > 0)
                            Hexdump.hexdump(System.err, src, srcIndex, maxLen < 128 ? maxLen + 8
                                    : 128);
    ...
    int decode(byte[] buffer, int bufferIndex) {
        ...
        if (byteCount != 0) {
            ...
            // Don't think we can rely on n being correct here. Must use
            // byteCount.
    ...
    /*
     * For this packet deconstruction technique to work for other networking
     * protocols the InputStream may need to be passed to the readXxxWireFormat
     * methods. This is actually purer. However, in the case of smb we know the
     * wordCount and byteCount. And since every subclass of ServerMessageBlock
     ...
     * words)) we actually read. Incedentally this is important to the
     * AndXServerMessageBlock class that needs to potentially read in another
     * smb's parameter words and bytes based on information in it's andxCommand,
     * andxOffset, ...etc.
     */
    ...
    public String toString() {
        ...
        return new String("command=" + c + ",received=" + received + ",errorCode=" + str
