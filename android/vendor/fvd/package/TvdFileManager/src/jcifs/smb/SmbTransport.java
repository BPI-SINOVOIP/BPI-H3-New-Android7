...
public class SmbTransport extends Transport implements SmbConstants {
    ...
    boolean matches(UniAddress address, int port, InetAddress localAddr, int localPort) {
        return address.equals(this.address)
                ...
                && (localAddr == this.localAddr || (localAddr != null && localAddr
                        .equals(this.localAddr))) && localPort == this.localPort;
    }
    ...
    boolean isSignatureSetupRequired(NtlmPasswordAuthentication auth) {
        return (flags2 & ServerMessageBlock.FLAGS2_SECURITY_SIGNATURES) != 0 && digest == null
    ...
    private void negotiate(int port, ServerMessageBlock resp) throws IOException {
        /*
         * We cannot use Transport.sendrecv() yet because the Transport thread
         * is not setup until doConnect() returns and we want to supress all
         * communication until we have properly negotiated.
         */
        synchronized(sbuf) {
            /*
             * If jcifs.netbios.hostname is set this *probably* means there is a
             * policy regarding which hosts a user can connect from. This
            ...
            out.flush();
            /*
             * Note the Transport thread isn't running yet so we can read from
             * the socket here.
             */
    ...
    protected Request peekKey() throws IOException {
        ...
        for (;;) {
            /*
             * 01234567 00SSFSMB 0 - 0's S - size of payload FSMB - 0xFF SMB
             * magic #
             */

            if (sbuf[0] == (byte) 0x00 && sbuf[1] == (byte) 0x00 && sbuf[4] == (byte) 0xFF
                    && sbuf[5] == (byte) 'S' && sbuf[6] == (byte) 'M' && sbuf[7] == (byte) 'B') {
                break; /* all good */
            }
            /* out of phase maybe? */
        ...
        /*
         * Unless key returned is null or invalid Transport.loop() always calls
         * doRecv() after and no one else but the transport thread should call
         * doRecv(). Therefore it is ok to expect that the data in sbuf will be
         * preserved for copying into BUF in doRecv().
         */
    ...
    protected void doRecv(Response response) throws IOException {
        ...
        synchronized(BUF) {
            ...
            if (resp.command == ServerMessageBlock.SMB_COM_READ_ANDX
                    && (errorCode == 0 || errorCode == 0x80000005)) { // overflow
                                                                      // indicator
                                                                      // normal
                                                                      // for
                                                                      // pipe
            ...
            /*
             * Verification fails (w/ W2K3 server at least) if status is not 0.
             * This suggests MS doesn't compute the signature (correctly) for
             * error responses (perhaps for DOS reasons).
             */
                            ...
                            while (resp.hasMoreElements()) {
                                ...
                                if (timeout <= 0) {
                                    throw new TransportException(this
                                            + " timedout waiting for response to " + req);
                                }
                            }
    ...
    /*
     * Split DFS path like \fs1.example.com\root5\link2\foo\bar.txt into at most
     * 3 components (not including the first index which is always empty):
     * result[0] = "" result[1] = "fs1.example.com" result[2] = "root5"
    ...
    DfsReferral[] getDfsReferrals(NtlmPasswordAuthentication auth, String path, int rn)
            throws SmbException {
        ...
        for (int di = 0; di < drs.length; di++) {
            ...
            /*
             * NTLM HTTP Authentication must be re-negotiated with challenge
             * from 'server' to access DFS vol.
             */
    ...
    FileEntry[] getDfsRoots(String domainName, NtlmPasswordAuthentication auth) throws IOException {
        ...
        /*
         * Procedure: Lookup a DC in the target domain Ask the DC for a referral
         * for the domain (e.g. "\example.com") Do NetrDfsEnumEx on the server
         * returned in the referral to get roots in target domain
         */
        ...
        handle = DcerpcHandle.getHandle("ncacn_np:"
                + UniAddress.getByName(dr[0].server).getHostAddress() + "[\\PIPE\\netdfs]", auth);
        try {
