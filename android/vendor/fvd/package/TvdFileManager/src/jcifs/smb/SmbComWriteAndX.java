...
class SmbComWriteAndX extends AndXServerMessageBlock {
    ...
    private static final int READ_ANDX_BATCH_LIMIT = Config.getInt(
            "jcifs.smb.client.WriteAndX.ReadAndX", 1);
    private static final int CLOSE_BATCH_LIMIT = Config.getInt("jcifs.smb.client.WriteAndX.Close",
            1);
    ...
    SmbComWriteAndX(int fid, long offset, int remaining, byte[] b, int off, int len,
    ...
    void setParam(int fid, long offset, int remaining, byte[] b, int off, int len) {
        ...
        dataLength = len;
        digest = null; /*
                        * otherwise recycled commands like writeandx will choke
                        * if session closes in between
                        */
    }
    ...
    public String toString() {
        return new String("SmbComWriteAndX[" + super.toString() + ",fid=" + fid + ",offset="
                + offset + ",writeMode=" + writeMode + ",remaining=" + remaining + ",dataLength="
                + dataLength + ",dataOffset=" + dataOffset + "]");
    }
}
