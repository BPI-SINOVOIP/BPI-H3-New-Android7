...
public class DosFileFilter implements SmbFileFilter {
    ...
    /*
     * This filter can be considerably more efficient than other file filters as
     * the specifed wildcard and attributes are passed to the server for
     * filtering there (although attributes are largely ignored by servers they
     * are filtered locally by the default accept method).
     */
     ...
     * specified for this filter. The wildcard has no influence on this method
     * as the server should have performed that filtering already. The
