...
class Trans2FindFirst2Response extends SmbComTransactionResponse {
    ...
    class SmbFindFileBothDirectoryInfo implements FileEntry {
        ...
        public String toString() {
            return new String("SmbFindFileBothDirectoryInfo[" + "nextEntryOffset="
                    + nextEntryOffset + ",fileIndex=" + fileIndex + ",creationTime="
                    + new Date(creationTime) + ",lastAccessTime=" + new Date(lastAccessTime)
                    + ",lastWriteTime=" + new Date(lastWriteTime) + ",changeTime="
                    + new Date(changeTime) + ",endOfFile=" + endOfFile + ",allocationSize="
                    + allocationSize + ",extFileAttributes=" + extFileAttributes
                    + ",fileNameLength=" + fileNameLength + ",eaSize=" + eaSize
                    + ",shortNameLength=" + shortNameLength + ",shortName=" + shortName
                    + ",filename=" + filename + "]");
        }
    }
    ...
    String readString(byte[] src, int srcIndex, int len) {
        ...
        try {
            if (useUnicode) {
                ...
                /*
                 * On NT without Unicode the fileNameLength includes the '\0'
                 * whereas on win98 it doesn't. I guess most clients only
                 * support non-unicode so they don't run into this.
                 */

                /*
                 * UPDATE: Maybe not! Could this be a Unicode alignment issue. I
                 * hope so. We cannot just comment out this method and use
                 * readString of ServerMessageBlock.java because the arguments
                 * are different, however one might be able to reduce this.
                 */
    ...
    int readDataWireFormat(byte[] buffer, int bufferIndex, int len) {
        ...
        for (int i = 0; i < numEntries; i++) {
            ...
            /*
             * With NT, the shortName is in Unicode regardless of what is
             * negotiated.
             */

            // e.shortName = readString( buffer, bufferIndex + 70,
            // e.shortNameLength );
            e.filename = readString(buffer, bufferIndex + 94, e.fileNameLength);

            /*
             * lastNameOffset ends up pointing to either to the exact location
             * of the filename(e.g. Win98) or to the start of the entry
             * containing the filename(e.g. NT). Ahhrg! In either case the
             * lastNameOffset falls between the start of the entry and the next
             * entry.
             */

            if (lastNameBufferIndex >= bufferIndex
                    && (e.nextEntryOffset == 0 || lastNameBufferIndex < (bufferIndex + e.nextEntryOffset))) {
                lastName = e.filename;
        ...
        /*
         * last nextEntryOffset for NT 4(but not 98) is 0 so we must use
         * dataCount or our accounting will report an error for NT :~(
         */
    ...
    public String toString() {
        ...
        return new String(c + super.toString() + ",sid=" + sid + ",searchCount=" + numEntries
                + ",isEndOfSearch=" + isEndOfSearch + ",eaErrorOffset=" + eaErrorOffset
                + ",lastNameOffset=" + lastNameOffset + ",lastName=" + lastName + "]");
    }
}
