...
class Trans2GetDfsReferralResponse extends SmbComTransactionResponse {
    ...
    class Referral {
        ...
        int readWireFormat(byte[] buffer, int bufferIndex, int len) {
            ...
            if (version != 3 && version != 1) {
                throw new RuntimeException("Version " + version
                        + " referral not supported. Please report this to jcifs at samba dot org.");
            }
            bufferIndex += 2;
            ...
            if (version == 3) {
                ...
                if (nodeOffset > 0)
                    node = readString(buffer, start + nodeOffset, len,
                            (flags2 & FLAGS2_UNICODE) != 0);
            } else if (version == 1) {
        ...
        public String toString() {
            return new String("Referral[" + "version=" + version + ",size=" + size + ",serverType="
                    + serverType + ",flags=" + flags + ",proximity=" + proximity + ",ttl=" + ttl
    ...
    int readDataWireFormat(byte[] buffer, int bufferIndex, int len) {
        ...
        bufferIndex += 2;
        /*
         * Samba 2.2.8a will reply with Unicode paths even though ASCII is
         * negotiated so we must use flags2 (probably should anyway).
         */
    ...
    public String toString() {
        return new String("Trans2GetDfsReferralResponse[" + super.toString() + ",pathConsumed="
