...
abstract class AndXServerMessageBlock extends ServerMessageBlock {
    ...
    int getBatchLimit(byte command) {
        /*
         * the default limit is 0 batched messages before this one, meaning this
         * message cannot be batched.
         */
        return 0;
    }

    /*
     * We overload this method from ServerMessageBlock because we want
     * writeAndXWireFormat to write the parameterWords and bytes. This is so we
     * can write batched smbs because all but the first smb of the chaain do not
     * have a header and therefore we do not want to writeHeaderWireFormat. We
    ...
    /*
     * We overload this because we want readAndXWireFormat to read the parameter
     * words and bytes. This is so when commands are batched together we can
     * recursivly call readAndXWireFormat without reading the non-existent
     * header.
     */
    ...
    int writeAndXWireFormat(byte[] dst, int dstIndex) {
        ...
        /*
         * Normally, without intervention everything would batch with everything
         * else. If the below clause evaluates true the andx command will not be
         * written and therefore the response will not read a batched command
         * and therefore the 'received' member of the response object will not
         * be set to true indicating the send and sendTransaction methods that
         * the next part should be sent. This is a very indirect and simple
        ...
        /*
         * The message provided to batch has a batchLimit that is higher than
         * the current batchLevel so we will now encode that chained message.
         * Before doing so we must increment the batchLevel of the andx message
         * in case it itself is an andx message and needs to perform the same
         * check as above.
         */
        ...
        if (andx instanceof AndXServerMessageBlock) {
             ...
             * 
             * This is where we recursively invoke the provided andx smb object
             * to write it's parameter words and bytes to our outgoing array.
             * Incedentally when these andx smbs are created they are not
             * necessarily populated with header data because they're not
             * writing the header, only their body. But for whatever reason one
             * might wish to populate fields if the writeXxx operation needs
             * this header data for whatever reason. I copy over the uid here so
             * it appears correct in logging output. Logging of andx segments of
             * messages inadvertantly print header information because of the
             * way toString always makes a super.toString() call(see toString()
             * at the end of all smbs classes).
             */
    ...
    int readAndXWireFormat(byte[] buffer, int bufferIndex) {
        ...
        if (wordCount != 0) {
            /*
             * these fields are common to all andx commands so let's populate
             * them here
             */
            ...
            /*
             * no point in calling readParameterWordsWireFormat if there are no
             * more parameter words. besides, win98 doesn't return
             * "OptionalSupport" field
             */
        ...
        if (errorCode != 0 || andxCommand == (byte) 0xFF) {
            ...
            if (andx instanceof AndXServerMessageBlock) {
                ...
                if (andx.wordCount != 0) {
                    /*
                     * no point in calling readParameterWordsWireFormat if there
                     * are no more parameter words. besides, win98 doesn't
                     * return "OptionalSupport" field
                     */
    ...
    public String toString() {
        return new String(super.toString() + ",andxCommand=0x"
                + Hexdump.toHexString(andxCommand, 2) + ",andxOffset=" + andxOffset);
    }
}
