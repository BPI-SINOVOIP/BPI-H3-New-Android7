...
abstract class SmbComTransactionResponse extends ServerMessageBlock implements Enumeration {
    ...
    int readBytesWireFormat(byte[] buffer, int bufferIndex) {
        ...
        if (parameterCount > 0) {
            ...
            System.arraycopy(buffer, bufferIndex, txn_buf, bufParameterStart
                    + parameterDisplacement, parameterCount);
            bufferIndex += parameterCount;
        }
        if (dataCount > 0) {
            bufferIndex += pad1 = dataOffset - (bufferIndex - headerStart);
            System.arraycopy(buffer, bufferIndex, txn_buf, bufDataStart + dataDisplacement,
                    dataCount);
            bufferIndex += dataCount;
        }

        /*
         * Check to see if the entire transaction has been read. If so call the
         * read methods.
         */
    ...
    public String toString() {
        return new String(super.toString() + ",totalParameterCount=" + totalParameterCount
                + ",totalDataCount=" + totalDataCount + ",parameterCount=" + parameterCount
                + ",parameterOffset=" + parameterOffset + ",parameterDisplacement="
