...
class SmbComNTCreateAndX extends AndXServerMessageBlock {
    ...
    /*
     * Open the file or fail if it does not exist aka OPEN_EXISTING
     */

    static final int FILE_OPEN = 0x1;

    /*
     * Create the file or fail if it does not exist aka CREATE_NEW
     */

    static final int FILE_CREATE = 0x2;

    /*
     * Open the file or create it if it does not exist aka OPEN_ALWAYS
     */
    ...
    /*
     * Open the file and overwrite it's contents or create it if it does not
     * exist aka CREATE_ALWAYS (according to the wire when calling CreateFile)
     */
    ...
    private int rootDirectoryFid, extFileAttributes, shareAccess, createDisposition, createOptions,
            impersonationLevel;
    ...
    SmbComNTCreateAndX(String name, int flags, int access, int shareAccess, int extFileAttributes,
            int createOptions, ServerMessageBlock andx) {
        super(andx);
        this.path = name;
        ...
        securityFlags = (byte) 0x03; // SECURITY_CONTEXT_TRACKING |
    ...
    public String toString() {
        return new String("SmbComNTCreateAndX[" + super.toString() + ",flags=0x"
                + Hexdump.toHexString(flags0, 2) + ",rootDirectoryFid=" + rootDirectoryFid
                + ",desiredAccess=0x" + Hexdump.toHexString(desiredAccess, 4) + ",allocationSize="
                + allocationSize + ",extFileAttributes=0x"
                + Hexdump.toHexString(extFileAttributes, 4) + ",shareAccess=0x"
                + Hexdump.toHexString(shareAccess, 4) + ",createDisposition=0x"
                + Hexdump.toHexString(createDisposition, 4) + ",createOptions=0x"
                + Hexdump.toHexString(createOptions, 8) + ",impersonationLevel=0x"
                + Hexdump.toHexString(impersonationLevel, 4) + ",securityFlags=0x"
                + Hexdump.toHexString(securityFlags, 2) + ",name=" + path + "]");
    }
}
