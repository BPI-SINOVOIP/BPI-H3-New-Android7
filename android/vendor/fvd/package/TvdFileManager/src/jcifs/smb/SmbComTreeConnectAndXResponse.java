...
class SmbComTreeConnectAndXResponse extends AndXServerMessageBlock {
    ...
    int readBytesWireFormat(byte[] buffer, int bufferIndex) {
        ...
        /*
         * Problems here with iSeries returning ASCII even though useUnicode =
         * true Fortunately we don't really need nativeFileSystem for anything.
         * if( byteCount > bufferIndex - start ) { nativeFileSystem =
         * readString( buffer, bufferIndex ); bufferIndex += stringWireLength(
         * nativeFileSystem, bufferIndex ); }
         */
    ...
    public String toString() {
        String result = new String("SmbComTreeConnectAndXResponse[" + super.toString()
                + ",supportSearchBits=" + supportSearchBits + ",shareIsInDfs=" + shareIsInDfs
                + ",service=" + service + ",nativeFileSystem=" + nativeFileSystem + "]");
        return result;
    }
}
