 ...
 * SIDs are commonly represented using a textual format such as
 * <tt>S-1-5-21-1496946806-2192648263-3843101252-1029</tt> but they may also be
 * resolved to yield the name of the associated Windows account such as
...
public class SID extends rpc.sid_t {
    ...
    static final String[] SID_TYPE_NAMES = {"0", "User", "Domain group", "Domain", "Local group",
    ...
    static void resolveSids(DcerpcHandle handle, LsaPolicyHandle policyHandle, SID[] sids)
            throws IOException {
    ...
    static void resolveSids0(String authorityServerName, NtlmPasswordAuthentication auth, SID[] sids)
            throws IOException {
    ...
    static public void resolveSids(String authorityServerName, NtlmPasswordAuthentication auth,
            SID[] sids, int offset, int length) throws IOException {
     ...
     * <p>
     * This method will attempt to resolve SIDs using a cache and cache the
     ...
     *            however a member server will work as well and a domain
     *            controller may not return names for SIDs corresponding to
     *            local accounts for which the domain controller is not an
     *            authority.
     * @param auth
     *            The credentials that should be used to communicate with the
     *            named server. As usual, <tt>null</tt> indicates that default
     *            credentials should be used.
     * @param sids
     *            The SIDs that should be resolved. After this function is
     *            called, the names associated with the SIDs may be queried with
     *            the <tt>toDisplayString</tt>, <tt>getDomainName</tt>, and
     *            <tt>getAccountName</tt> methods.
     */
    static public void resolveSids(String authorityServerName, NtlmPasswordAuthentication auth,
    ...
    public static SID getServerSid(String server, NtlmPasswordAuthentication auth)
        ...
        try {
            ...
            return new SID(info.sid, SID.SID_TYPE_DOMAIN,
    ...
    /**
     * Construct a SID from a domain SID and an RID (relative identifier). For
     * example, a domain SID <tt>S-1-5-21-1496946806-2192648263-3843101252</tt>
     * and RID <tt>1029</tt> would yield the SID
     * <tt>S-1-5-21-1496946806-2192648263-3843101252-1029</tt>.
     */
     ...
     * <p>
     * SID types are described in the following table. <tt>
     * <table>
    ...
    /**
     * Return text represeting the SID type suitable for display to users. Text
     * includes 'User', 'Domain group', 'Local group', etc.
     */
    ...
    /**
     * Return the domain name of this SID unless it could not be resolved in
     * which case the numeric representation is returned.
     */
    ...
    /**
     * Return the sAMAccountName of this SID unless it could not be resolved in
     * which case the numeric RID is returned. If this SID is a domain SID, this
    ...
    /**
     * Return a String representing this SID ideal for display to users. This
     * method should return the same text that the ACL editor in Windows would
     * display.
     * <p>
     * Specifically, if the SID has been resolved and it is not a domain SID or
     * builtin account, the full DOMAIN\name form of the account will be
     * returned (e.g. MYDOM\alice or MYDOM\Domain Users). If the SID has been
     * resolved but it is is a domain SID, only the domain name will be returned
     * (e.g. MYDOM). If the SID has been resolved but it is a builtin account,
     * only the name component will be returned (e.g. SYSTEM). If the sid cannot
     * be resolved the numeric representation from toString() is returned.
     */
    public String toDisplayString() {
        ...
        if (domainName != null) {
            ...
            if (type == SID_TYPE_DOMAIN) {
                str = domainName;
            } else if (type == SID_TYPE_WKN_GRP || domainName.equals("BUILTIN")) {
                if (type == SID_TYPE_UNKNOWN) {
                    str = toString();
                } else {
                    str = acctName;
    ...
    /**
     * Manually resolve this SID. Normally SIDs are automatically resolved.
     ...
     */
    public void resolve(String authorityServerName, NtlmPasswordAuthentication auth)
            throws IOException {
        SID[] sids = new SID[1];
        sids[0] = this;
    ...
    static SID[] getGroupMemberSids0(DcerpcHandle handle, SamrDomainHandle domainHandle,
            SID domsid, int rid, int flags) throws IOException {
        ...
        try {
            ...
            NtlmPasswordAuthentication origin_auth = (NtlmPasswordAuthentication) handle
                    .getPrincipal();
    ...
    public SID[] getGroupMemberSids(String authorityServerName, NtlmPasswordAuthentication auth,
     ...
     * is an ArrayList of SIDs represents the local groups that the account is a
     * member of.
     * <p/>
     ...
     * @param auth
     *            The credentials required to query groups and group members.
     * @param flags
     *            Flags that control the behavior of the operation. When all
     *            name associated with SIDs will be required, the
     *            SID_FLAG_RESOLVE_SIDS flag should be used which causes all
     *            group member SIDs to be resolved together in a single more
     *            efficient operation.
     */
    static Map getLocalGroupsMap(String authorityServerName, NtlmPasswordAuthentication auth,
            int flags) throws IOException {
        ...
        try {
            ...
            for (int ei = 0; ei < rpc.sam.count; ei++) {
                ...
                SID[] mems = SID
                        .getGroupMemberSids0(handle, domainHandle, domsid, entry.idx, flags);
