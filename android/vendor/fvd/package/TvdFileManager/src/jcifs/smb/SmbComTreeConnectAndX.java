...
class SmbComTreeConnectAndX extends AndXServerMessageBlock {
    ...
    int writeParameterWordsWireFormat(byte[] dst, int dstIndex) {
        ...
        if (session.transport.server.security == SECURITY_SHARE
                && (session.auth.hashesExternal || session.auth.password.length() > 0)) {
    ...
    public String toString() {
        String result = new String("SmbComTreeConnectAndX[" + super.toString() + ",disconnectTid="
                + disconnectTid + ",passwordLength=" + passwordLength + ",password="
                + Hexdump.toHexString(password, passwordLength, 0) + ",path=" + path + ",service="
                + service + "]");
        return result;
    }
}
