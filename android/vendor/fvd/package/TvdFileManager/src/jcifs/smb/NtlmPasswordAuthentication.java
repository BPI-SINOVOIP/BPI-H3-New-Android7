 ...
 * are retrieved from the <tt>jcifs.smb.client.domain</tt>,
...
public final class NtlmPasswordAuthentication implements Principal, Serializable {
    ...
    // KGS!@#$%
    private static final byte[] S8 = {(byte) 0x4b, (byte) 0x47, (byte) 0x53, (byte) 0x21,
     ...
     * constructor is used internally be jCIFS when parsing SMB URLs.
     */
    ...
    /**
     * Create an <tt>NtlmPasswordAuthentication</tt> object from a domain,
     * username, and password. Parameters that are <tt>null</tt> will be
     ...
     * object which will be the case when NTLM HTTP Authentication is used.
     * There is no way to retrieve a users password in plain text unless it is
    ...
    public byte[] getUnicodeHash(byte[] challenge) {
        ...
        switch (LM_COMPATIBILITY) {
            ...
            case 5:
                /*
                 * if( clientChallenge == null ) { clientChallenge = new
                 * byte[8]; RANDOM.nextBytes( clientChallenge ); } return
                 * getNTLMv2Response(domain, username, password, null,
     ...
     *            placed.
     * @param offset
     *            The offset in the destination array at which the session key
     *            will start.
     */
     ...
     * Two <tt>NtlmPasswordAuthentication</tt> objects are equal if their
     * caseless domain and username fields are equal and either both hashes are
     * external and they are equal or both internally supplied passwords are
     * equal. If one <tt>NtlmPasswordAuthentication</tt> object has external
     * hashes (meaning negotiated via NTLM HTTP Authentication) and the other
     * does not they will not be equal. This is technically not correct however
     * the server 8 byte challage would be required to compute and compare the
     * password hashes but that it not available with this method.
     */
        ...
        if (obj instanceof NtlmPasswordAuthentication) {
            ...
            if (ntlm.domain.toUpperCase().equals(domain.toUpperCase())
                ...
                if (hashesExternal && ntlm.hashesExternal) {
                    ...
                    /*
                     * This still isn't quite right. If one npa object does not
                     * have external hashes and the other does then they will
                     * not be considered equal even though they may be.
                     */
