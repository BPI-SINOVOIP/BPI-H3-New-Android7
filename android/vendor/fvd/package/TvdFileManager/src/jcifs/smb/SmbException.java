 ...
 * Rather than represent each with it's own <code>Exception</code> class, this
 * class represents all of them. For many of the popular error codes, constants
 * and text messages like "The device is not ready" are provided.
 * <p>
 * The jCIFS client maps DOS error codes to NTSTATUS codes. This means that the
 * user may recieve a different error from a legacy server than that of a newer
 * varient such as Windows NT and above. If you should encounter such a case,
 * please report it to jcifs at samba dot org and we will change the mapping.
 */
...
public class SmbException extends IOException implements NtStatus, DosError, WinError {
    ...
    static String getMessageByCode(int errcode) {
        /*
         * Note there's a signedness error here because 0xC0000000 based values
         * are negative so it with NT_STATUS_SUCCESS (0) the binary search will
         * not be performed properly. The effect is that the code at index 1 is
         * never found (NT_STATUS_UNSUCCESSFUL). So here we factor out
         * NT_STATUS_SUCCESS as a special case (which it is).
         */
        if (errcode == 0) {
    ...
    public static String getMessageByWinerrCode(int errcode) {
        int min = 0;
