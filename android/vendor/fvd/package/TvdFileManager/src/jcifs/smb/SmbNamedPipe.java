 ...
 * <li> <code>TransactNamedPipe</code> A message-type pipe call that writes to
 * and reads from an existing pipe descriptor in one operation.
 * <li> <code>CreateFile</code>, <code>ReadFile</code>, <code>WriteFile</code>,
 ...
 * the <code>CallNamedPipe</code> interface.</td>
 * </tr>
 * <tr>
 * <td width="20%">
 * 
 ...
 * <td>
 * Open the Named Pipe foo for reading and writing. The pipe will behave like
 * the <code>TransactNamedPipe</code> interface.</td>
 * </tr>
 * <tr>
 * <td width="20%">
 * 
 ...
 * though the <code>CreateFile</code>, <code>ReadFile</code>,
...
public class SmbNamedPipe extends SmbFile {
    ...
    /**
     * Pipe operations should behave like the <code>CallNamedPipe</code> Win32
     * Named Pipe function.
     */
    ...
    /**
     * Pipe operations should behave like the <code>TransactNamedPipe</code>
     ...
     * parameter should be at least one of the <code>PIPE_TYPE</code> flags
     * combined with the bitwise OR operator <code>|</code>. See the examples
     * listed above.
     */

    public SmbNamedPipe(String url, int pipeType) throws MalformedURLException,
    ...
    /**
     * Return the <code>InputStream</code> used to read information from this
     * pipe instance. Presumably data would first be written to the
     * <code>OutputStream</code> associated with this Named Pipe instance
     * although this is not a requirement (e.g. a read-only named pipe would
     * write data to this stream on connection). Reading from this stream may
     * block. Therefore it may be necessary that an addition thread be used to
     * read and write to a Named Pipe.
     */
    ...
    /**
     * Return the <code>OutputStream</code> used to write information to this
     * pipe instance. The act of writing data to this stream will result in
     * response data recieved in the <code>InputStream</code> associated with
     * this Named Pipe instance (unless of course it does not elicite a response
     * or the pipe is write-only).
     */
    ...
    public OutputStream getNamedPipeOutputStream() throws IOException {
        if (pipeOut == null) {
            if ((pipeType & PIPE_TYPE_CALL) == PIPE_TYPE_CALL
                ...
                pipeOut = new SmbFileOutputStream(this, false, (pipeType & 0xFFFF00FF)
                        | SmbFile.O_EXCL);
            }
        }
        return pipeOut;
