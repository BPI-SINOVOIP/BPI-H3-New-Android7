...
/**
 * This class represents a resource on an SMB network. Mainly these resources
 * are files and directories however an <code>SmbFile</code> may also refer to
 * servers and workgroups. If the resource is a file or directory the methods of
 * <code>SmbFile</code> follow the behavior of the well known
 ...
 * workgroups.
 * <p>
 * <font color="#800000"><i>Important: all SMB URLs that represent workgroups,
 * servers, shares, or directories require a trailing slash '/'. </i></font>
 * <p>
 * When using the <tt>java.net.URL</tt> class with 'smb://' URLs it is necessary
 * to first call the static <tt>jcifs.Config.registerSmbURLHandler();</tt>
 * method. This is required to register the SMB protocol handler.
 * <p>
 * The userinfo component of the SMB URL (<tt>domain;user:pass</tt>) must be URL
 * encoded if it contains reserved characters. According to RFC 2396 these
 * characters are non US-ASCII characters and most meta characters however jCIFS
 * will work correctly with anything but '@' which is used to delimit the
 * userinfo component from the server and '%' which is the URL escape character
 * itself.
 * <p>
 * The server component may a traditional NetBIOS name, a DNS name, or IP
 * address. These name resolution mechanisms and their resolution order can be
 * changed (See <a href="../../../resolver.html">Setting Name Resolution
 * Properties</a>). The servername and path components are not case sensitive
 * but the domain, username, and password components are. It is also likely that
 ...
 * <td>
 * A relativly sophisticated example that references a file <code>msmith1</code>
 ...
 * <td>
 * This references only a server. The behavior of some methods is different in
 * this context(e.g. you cannot <code>delete</code> a server) however as you
 ...
 * <code>myworkgroup</code> happends to be a workgroup(which is indeed suggested
 ...
 * <code>smb://workgroup/</code> lists servers, the <code>smb://</code> URL
 * lists all available workgroups on a netbios LAN. Again, in this context many
 ...
 * <td>
 * The server name may also be a DNS name as it is in this example. See <a
 ...
 * <td>
 * SMB URLs support some query string parameters. In this example the
 * <code>SERVER</code> parameter is used to override the server name service
 ...
 * slightly different from the corresponding <code>java.io.File</code> usage; a
 ...
 * <tr bgcolor="#ccccff">
 * <td colspan="3">
 * <b>Examples Of SMB URLs When Augmented With A Second Constructor
 * Parameter</b></td>
 * <tr>
 * <td width="20%">
...
public class SmbFile extends URLConnection implements SmbConstants {
    ...
    // share access
    /**
     * When specified as the <tt>shareAccess</tt> constructor parameter, other
     * SMB clients (including other threads making calls into jCIFS) will not be
     * permitted to access the target file and will receive "The file is being
     * accessed by another process" message.
     */
    public static final int FILE_NO_SHARE = 0x00;
    /**
     * When specified as the <tt>shareAccess</tt> constructor parameter, other
     * SMB clients will be permitted to read from the target file while this
     * file is open. This constant may be logically OR'd with other share access
     * flags.
     */
    public static final int FILE_SHARE_READ = 0x01;
    /**
     * When specified as the <tt>shareAccess</tt> constructor parameter, other
     * SMB clients will be permitted to write to the target file while this file
     * is open. This constant may be logically OR'd with other share access
     * flags.
     */
    public static final int FILE_SHARE_WRITE = 0x02;
    /**
     * When specified as the <tt>shareAccess</tt> constructor parameter, other
     * SMB clients will be permitted to delete the target file while this file
     * is open. This constant may be logically OR'd with other share access
     * flags.
     */
    ...
    /**
     * A file with this bit on as returned by <tt>getAttributes()</tt> is a
     * volume
     */
    ...
    public static final String TAG = "SmbFile";
    ...
    static {
        ...
        attrExpirationPeriod = Config.getLong("jcifs.smb.client.attrExpirationPeriod",
    ...
    private String canon; // Initially null; set by getUncPath; dir must end
                          // with '/'
    ...
    private DfsReferral dfsReferral = null; // For getDfsPath() and
                                            // getServerWithDfs()
     ...
     *             If the server or workgroup of the <tt>context</tt> file
     *             cannot be determined
     */

    public SmbFile(SmbFile context, String name) throws MalformedURLException, UnknownHostException {
        this(context.isWorkgroup0() ? new URL(null, "smb://" + name, Handler.SMB_HANDLER)
     ...
     *            A URL string
     * @param name
     *            A path string relative to the <code>context</code> paremeter
     * @throws MalformedURLException
     ...
     * @throws MalformedURLException
     *             If the <code>url</code> parameter does not follow the
     *             prescribed syntax
     */
     ...
     * <tt>shareAccess</tt> parameter controls what permissions other clients
     * have when trying to access the same file while this instance is still
     ...
     *            The credentials the client should use for authentication
     * @param shareAccess
     *            Specifies what access other clients have while this file is
     *            open.
     * @throws MalformedURLException
     *             If the <code>url</code> parameter does not follow the
     *             prescribed syntax
     ...
     * file or directory. The second parameter is a relative path from the
     * <code>context</code>. See the description above for examples of using the
     * second <code>name</code> parameter. The <tt>shareAccess</tt> parameter
     * controls what permissions other clients have when trying to access the
     * same file while this instance is still open. This value is either
     ...
     *             do not follow the prescribed syntax
     */
    public SmbFile(SmbFile context, String name, int shareAccess) throws MalformedURLException,
    ...
    SmbFile(SmbFile context, String name, int type, int attributes, long createTime,
            long lastModified, long size) throws MalformedURLException, UnknownHostException {
        this(context.isWorkgroup0() ? new URL(null, "smb://" + name + "/", Handler.SMB_HANDLER)
                : new URL(context.url, name + ((attributes & ATTR_DIRECTORY) > 0 ? "/" : "")));

        /*
         * why was this removed before? DFS? copyTo? Am I going around in
         * circles?
         */
        auth = context.auth;
        ...
        /*
         * why? am I going around in circles? this.type = type == TYPE_WORKGROUP
         * ? 0 : type;
         */
        this.type = type;
    ...
    void resolveDfs(ServerMessageBlock request) throws SmbException {
        ...
        DfsReferral dr = dfs.resolve(hostName, tree.share, unc, auth);
        if (dr != null) {
            UniAddress addr;
            SmbTransport trans;
        ...
        } else if (tree.inDomainDfs && !(request instanceof NtTransQuerySecurityDesc)
                && !(request instanceof SmbComClose) && !(request instanceof SmbComFindClose2)) {
    ...
    UniAddress getFirstAddress() throws UnknownHostException {
        ...
        if (host.length() == 0) {
            try {
                NbtAddress addr = NbtAddress.getByName(NbtAddress.MASTER_BROWSER_NAME, 0x01, null);
                addresses = new UniAddress[1];
                addresses[0] = UniAddress.getByName(addr.getHostAddress());
            } catch (UnknownHostException uhe) {
                NtlmPasswordAuthentication.initDefaults();
               NtlmPasswordAuthentication.DEFAULT_DOMAIN = "WORKGROUP";
                addresses = UniAddress
                        .getAllByName(NtlmPasswordAuthentication.DEFAULT_DOMAIN, true);
            }
...
UniAddress[] getFirstAddressX(SmbFileFilter ff) throws UnknownHostException {
        addressIndex = 0;
        ...
        if (host.length() == 0) {
            try {
                Log.d(TAG,"getFirstAddress,host.length() ==0");
                NbtAddress addr = NbtAddress.getByName(
                        NbtAddress.MASTER_BROWSER_NAME, 0x01, null);
                UniAddress[] addressX = new UniAddress[1];
                addressX[0] = UniAddress.getByName( addr.getHostAddress() );
                if (addressX[0] != null) {
                    try {
                        SmbFile fX = new SmbFile(this,addressX[0].getHostAddress(),0x04,ATTR_DIRECTORY,0L, 0L, 0L); //0x04 means server
                        if (ff != null && false == ff.accept(fX)) {
                        Log.d(TAG,"accept failed!");
                    }
                    } catch (SmbException e ) {
                        Log.d(TAG,"getFirstAddressX: excepiton");
                    } catch (MalformedURLException e) {
                        Log.d(TAG,"get MalformedURLException");
                    }
                }
                NtlmPasswordAuthentication.DEFAULT_DOMAIN = "WORKGROUP";
                addresses =UniAddress.getAllByNameX( NtlmPasswordAuthentication.DEFAULT_DOMAIN, true );
                flag = true;
            } catch( UnknownHostException uhe ) {
                NtlmPasswordAuthentication.initDefaults();
                Log.d(TAG,"getFirstAddress,UnKownHostException");
                if (!flag) {
                    NtlmPasswordAuthentication.DEFAULT_DOMAIN = "WORKGROUP";
                    addresses =UniAddress.getAllByNameX( NtlmPasswordAuthentication.DEFAULT_DOMAIN, true );
                }
        ...
        if (addresses!= null) {
            for (int i=0;i<addresses.length;i++) {
                 try {
                     SmbFile f = new SmbFile(this,addresses[i].getHostAddress(),0x04,ATTR_DIRECTORY,0L, 0L, 0L); //0x04 means server,
                     if (ff != null && false == ff.accept(f)) {
                         Log.d(TAG,"add accept failed!");
                         return null;
                     }
                 } catch (SmbException e ) {
                     Log.d(TAG,"Smb excepiton");
                 } catch (MalformedURLException e) {
                     Log.d(TAG,"get MalformedURLException");
                 }
            }
    ...
    void connect0X(SmbFileFilter ff) throws SmbException {
        try {
            connectX(ff);
        } catch( UnknownHostException uhe ) {
            throw new SmbException( "Failed to connect to server", uhe );
    ...
    void doConnect() throws IOException {
        ...
        try {
            ...
            if (share == null) { // IPC$ - try "anonymous" credentials
                ...
                tree.treeConnect(null, null);
            } else if ((a = NtlmAuthenticator
                    .requestNtlmPasswordAuthentication(url.toString(), sae)) != null) {
                auth = a;
    ...
    public void connectX(SmbFileFilter ff) throws IOException {
        SmbTransport trans;
        SmbSession ssn;
        UniAddress addr;
        if( isConnected() ) {
    ...
    int open0(int flags, int access, int attrs, int options) throws SmbException {
        ...
        if (tree.session.transport.hasCapability(ServerMessageBlock.CAP_NT_SMBS)) {
            ...
            SmbComNTCreateAndX request = new SmbComNTCreateAndX(unc, flags, access, shareAccess,
                    attrs, options, null);
     ...
     * username for example: <tt>
     * String username = f.getPrincipal().getName();
     * </tt> The <tt>Principal</tt> object returned will never be <tt>null</tt>
    ...
    /**
     * Returns the last component of the target URL. This will effectively be
     * the name of the file or directory represented by this
     * <code>SmbFile</code> or in the case of URLs that only specify a server or
     * workgroup, the server or workgroup will be returned. The name of the root
     * URL <code>smb://</code> is also <code>smb://</code>. If this
                ...
                switch (state) {
                    ...
                    case 1:
                        if (in[i] == '/') {
                            ...
                            i++;
                            break;
                        } else if ((i + 1) < length && in[i] == '.' && in[i + 1] == '.'
                                && ((i + 2) >= length || in[i + 2] == '/')) {
                            i += 2;
                            if (o == 1)
                                break;
    ...
    /**
     * Retrieves the share associated with this SMB resource. In the case of
    ...
    Info queryPath(String path, int infoLevel) throws SmbException {
         ...
         * SMB_COM_QUERY_INFORMATION so if NT_SMBs are requested by the server
         * than in this case that's what it will get regardless of what
         * jcifs.smb.client.useNTSmbs is set to(overrides
         * negotiatedCapabilities).
         */

        /*
         * We really should do the referral before this in case the redirected
         * target has different capabilities. But the way we have been doing
         * that is to call exists() which calls this method so another technique
         * will be necessary to support DFS referral _to_ Win95/98/ME.
         */
    ...
    /**
     * Tests to see if the SMB resource exists. If the resource refers only to a
     * server, this method determines if the server exists on the network and is
     * advertising SMB services. If this resource refers to a workgroup, this
     * method determines if the workgroup name is valid on the local SMB
     * network. If this <code>SmbFile</code> refers to the root
    ...
    /**
     * Tests to see if the file this <code>SmbFile</code> represents exists and
     * is not marked read-only. By default, resources are considered to be
    ...
    /**
     * If the path of this <code>SmbFile</code> falls within a DFS volume, this
     * method will return the referral path to which it maps. Otherwise
    ...
    /**
     * Retrieve the last time the file represented by this <code>SmbFile</code>
     * was modified. The value returned is suitable for constructing a
     ...
     * <li>all available NetBIOS workgroups or domains if this resource is the
     * top level URL <code>smb://</code>,
     ...
     * <li>all browseable shares of a server including printers, IPC services,
     * or disk volumes if this resource is a server URL in the form
     ...
     * <code>SmbFile</code> objects. This method is much more efficient than the
     * regular <code>list</code> method when querying attributes of each file in
     * the result set.
     * <p>
    ...
    /**
     * The CIFS protocol provides for DOS "wildcards" to be used as a
     * performance enhancement. The client does not have to filter the names and
     * the server does not have to return all directory entries.
     * <p>
     * The wildcard expression may consist of two special meta characters in
     * addition to the normal filename characters. The '*' character matches any
     * number of characters in part of a name. If the expression begins with one
     * or more '?'s then exactly that many characters will be matched whereas if
    ...
    SmbFile[] listFiles(String wildcard, int searchAttributes, SmbFilenameFilter fnf,
    ...
    void doEnum(ArrayList list, boolean files, String wildcard, int searchAttributes,
    ...
    void doShareEnum(ArrayList list, boolean files, String wildcard, int searchAttributes,
            SmbFilenameFilter fnf, SmbFileFilter ff) throws SmbException, UnknownHostException,
        ...
        if (dfs.isTrustedDomain(getServer(), auth)) {
            /*
             * The server name is actually the name of a trusted domain. Add DFS
             * roots to the list.
             */
            try {
        ...
        while (iter.hasNext()) {
            ...
            if (name.length() > 0) {
                ...
                SmbFile f = new SmbFile(this, name, e.getType(), ATTR_READONLY | ATTR_DIRECTORY,
                        0L, 0L, 0L);
    ...
    FileEntry[] doDfsRootEnum() throws IOException {
        ...
        handle = DcerpcHandle.getHandle("ncacn_np:" + getAddress().getHostAddress()
                + "[\\PIPE\\netdfs]", auth);
        try {
    ...
    FileEntry[] doMsrpcShareEnum() throws IOException {
         ...
         * multiple IP addresses such as when domain-based DFS is in play.
         * Because of this, to ensure that we query each IP individually without
         * re-resolving the hostname and getting a different IP, we must use the
         * current addresses IP rather than just url.getHost() like we were
         * using prior to 1.2.16.
         */
    ...
    void doNetServerEnum(ArrayList list, boolean files, String wildcard, int searchAttributes,
            SmbFilenameFilter fnf, SmbFileFilter ff) throws SmbException, UnknownHostException,
            MalformedURLException {
    ...
    void doFindFirstNext(ArrayList list, boolean files, String wildcard, int searchAttributes,
            SmbFilenameFilter fnf, SmbFileFilter ff) throws SmbException, UnknownHostException,
            MalformedURLException {
        SmbComTransaction req;
        ...
        for (;;) {
            for (int i = 0; i < resp.numEntries; i++) {
                ...
                if (name.length() > 0) {
                    SmbFile f = new SmbFile(this, name, TYPE_FILESYSTEM, e.getAttributes(),
    ...
    /**
     * Changes the name of the file this <code>SmbFile</code> represents to the
     * name designated by the <code>SmbFile</code> argument.
     * <p/>
     * <i>Remember: <code>SmbFile</code>s are immutible and therefore the path
    ...
    void copyTo0(SmbFile dest, byte[][] b, int bsize, WriterThread w, SmbComReadAndX req,
        ...
        if (isDirectory()) {
            ...
            try {
                for (i = 0; i < files.length; i++) {
                    ndest = new SmbFile(dest, files[i].getName(), files[i].type,
                            files[i].attributes, files[i].createTime, files[i].lastModified,
                            files[i].size);
            ...
            try {
                ...
                try {
                    dest.open(SmbFile.O_CREAT | SmbFile.O_WRONLY | SmbFile.O_TRUNC, FILE_WRITE_DATA
                            | FILE_WRITE_ATTRIBUTES, attributes, 0);
                } catch (SmbAuthException sae) {
                    if ((dest.attributes & ATTR_READONLY) != 0) {
                        ...
                        dest.open(SmbFile.O_CREAT | SmbFile.O_WRONLY | SmbFile.O_TRUNC,
                                FILE_WRITE_DATA | FILE_WRITE_ATTRIBUTES, attributes, 0);
                    } else {
                        throw sae;
                    }
                }
                ...
                dest.send(new Trans2SetFileInformation(dest.fid, attributes, createTime,
                        lastModified), new Trans2SetFileInformationResponse());
                dest.close(0L);
            } catch (Exception ex) {
                if (log.level > 1)
     ...
     * <tt>dest</tt> parameter. This file and the destination file do not need
     * to be on the same host. This operation does not copy extended file
     * attibutes such as ACLs but it does copy regular attributes as well as
     * create and last write times. This method is almost twice as efficient as
     * manually copying as it employs an additional write thread to read and
    ...
    public void copyTo(SmbFile dest) throws SmbException {
        ...
        /*
         * At this point the maxBufferSize values are from the server exporting
         * the volumes, not the one that we will actually end up performing IO
         * with. If the server hosting the actual files has a smaller maxBufSize
         * this could be incorrect. To handle this properly it is necessary to
         * redirect the tree to the target server first before establishing
         * buffer size. These exists() calls facilitate that.
         */
        resolveDfs(null);
     ...
     * <code>SmbFile</code>. If the target is a directory, the contents of the
    ...
    void delete(String fileName) throws SmbException {
        ...
        if ((attributes & ATTR_DIRECTORY) != 0) {
            ...
            try {
                 ...
                 * listFiles may generate undesireable "cannot find the file
                 * specified".
                 */
    ...
    /**
     * Returns the length of this <tt>SmbFile</tt> in bytes. If this object is a
    ...
    /**
     * Creates a directory with the path specified by this <code>SmbFile</code>.
     * For this method to be successful, the target must not already exist. This
     * method will fail when used with <code>smb://</code>,
     * <code>smb://workgroup/</code>, <code>smb://server/</code>, or
    ...
    /**
     * Creates a directory with the path specified by this <tt>SmbFile</tt> and
     * any parent directories that do not exist. This method will fail when used
     * with <code>smb://</code>, <code>smb://workgroup/</code>,
    ...
    /**
     * Create a new file but fail if it already exists. The check for existance
     * of the file and it's creation are an atomic operation with respect to
    ...
    /**
     * Set the last modified time of the file. The time is specified as
     * milliseconds from Jan 1, 1970 which is the same as that which is returned
     * by the <tt>lastModified()</tt>, <tt>getLastModified()</tt>, and
    ...
    /**
     * Set the attributes of this file. Attributes are composed into a bitset by
    ...
    /**
     * Computes a hashCode for this file based on the URL string and IP address
     * if the server. The hashing function uses the hashcode of the server
     * address, the canonical representation of the URL, and does not compare
    ...
    /**
     * Tests to see if two <code>SmbFile</code> objects are equal. Two SmbFile
     * objects are equal when they reference the same SMB resource. More
     * specifically, two <code>SmbFile</code> objects are equals if their server
     * IP addresses are equal and the canonicalized representation of their
     * URLs, minus authentication parameters, are case insensitivly and
    ...
    public boolean equals(Object obj) {
        if (obj instanceof SmbFile) {
            ...
            /*
             * If uncertain, pathNamesPossiblyEqual returns true. Comparing
             * canonical paths is definitive.
             */
    ...
    /*
     * public boolean equals( Object obj ) { return obj instanceof SmbFile &&
    ...
    /**
     * Return an array of Access Control Entry (ACE) objects representing the
    ...
    /**
     * Return an array of Access Control Entry (ACE) objects representing the
     * share permissions on the share exporting this file or directory. If no
     * DACL is present, null is returned. If the DACL is empty, an array with 0
     * elements is returned.
     * <p>
     * Note that this is different from calling <tt>getSecurity</tt> on a share.
     * There are actually two different ACLs for shares - the ACL on the share
     * and the ACL on the folder being shared. Go to <i>Computer Management</i>
    ...
    /**
     * Return an array of Access Control Entry (ACE) objects representing the
     * security descriptor associated with this file or directory.
     * <p>
     ...
     * or <tt>toString()</tt> is called, the names will attempt to be resolved.
     * If the names cannot be resolved (e.g. due to temporary network failure),
     * the said methods will return default values (usually <tt>S-X-Y-Z</tt>
     * strings of fragments of).
     * <p>
     * Alternatively <tt>getSecurity(true)</tt> may be used to resolve all SIDs
     * together and detect network failures.
     */
