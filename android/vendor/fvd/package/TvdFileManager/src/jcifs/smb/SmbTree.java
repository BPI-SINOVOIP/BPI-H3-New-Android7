...
class SmbTree {
    ...
    boolean matches(String share, String service) {
        return this.share.equalsIgnoreCase(share)
                && (service == null || service.startsWith("??") || this.service
    ...
    void send(ServerMessageBlock request, ServerMessageBlock response) throws SmbException {
        ...
        if (inDfs && !service.equals("IPC") && request.path != null && request.path.length() > 0) {
            /*
             * When DFS is in action all request paths are full UNC paths minus
             * the first backslash like \server\share\path\to\file as opposed to
             * normally \path\to\file
             */
        ...
        try {
            ...
            if (se.getNtStatus() == se.NT_STATUS_NETWORK_NAME_DELETED) {
                /*
                 * Someone removed the share while we were connected. Bastards!
                 * Disconnect this tree so that it reconnects cleanly should the
                 * share reappear in this client's lifetime.
                 */
                treeDisconnect(true);
            }
            throw se;
    ...
    void treeConnect(ServerMessageBlock andx, ServerMessageBlock andxResponse) throws SmbException {
        ...
        synchronized(transport.setupDiscoLock) {
            synchronized(transport) {
                ...
                /*
                 * The hostname to use in the path is only known for sure if the
                 * NetBIOS session has been successfully established.
                 */
                ...
                if (session.transport.log.level >= 4)
                    session.transport.log
                            .println("treeConnect: unc=" + unc + ",service=" + service);

                SmbComTreeConnectAndXResponse response = new SmbComTreeConnectAndXResponse(
                        andxResponse);
    ...
    public String toString() {
        return "SmbTree[share=" + share + ",service=" + service + ",tid=" + tid + ",inDfs=" + inDfs
                + ",inDomainDfs=" + inDomainDfs + ",treeConnected=" + treeConnected + "]";
    }
}
