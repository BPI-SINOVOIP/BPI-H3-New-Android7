...
public class Type3Message extends NtlmMessage {
    ...
    public Type3Message(Type2Message type2) {
        ...
        switch (LM_COMPATIBILITY) {
            ...
            case 5:
                ...
                /*
                 * setNTResponse(getNTLMv2Response(type2, domain, user,
                 * password, clientChallenge));
                 */
                break;
            default:
                setLMResponse(getLMResponse(type2, password));
    ...
    public byte[] toByteArray() {
        try {
            ...
            if (domainName != null && domainName.length() != 0) {
                domain = unicode ? domainName.toUpperCase().getBytes("UnicodeLittleUnmarked")
            ...
            if (userName != null && userName.length() != 0) {
                user = unicode ? userName.getBytes("UnicodeLittleUnmarked") : userName
                        .toUpperCase().getBytes(oem);
            }
            int userLength = (user != null) ? user.length : 0;
            String workstationName = getWorkstation();
            byte[] workstation = null;
            if (workstationName != null && workstationName.length() != 0) {
                workstation = unicode ? workstationName.getBytes("UnicodeLittleUnmarked")
    ...
    public static byte[] getLMv2Response(Type2Message type2, String domain, String user,
        ...
        if (type2 == null || domain == null || user == null || password == null
                || clientChallenge == null) {
            return null;
        }
        return NtlmPasswordAuthentication.getLMv2Response(domain, user, password,
    ...
    private void parse(byte[] material) throws IOException {
        ...
        int flags;
        String charset;
        if (lmResponseOffset == 52 || ntResponseOffset == 52 || domainOffset == 52
                || userOffset == 52 || workstationOffset == 52) {
            ...
            charset = ((flags & NTLMSSP_NEGOTIATE_UNICODE) != 0) ? "UnicodeLittleUnmarked"
                    : getOEMEncoding();
        }
        setFlags(flags);
        setLMResponse(lmResponse);
        /*
         * NTLMv2 issues w/cross-domain authentication; leave NT empty if NTLMv2
         * was sent by the client. NTLM response will always be 24 bytes; NTLMv2
         * response will always be longer. - Kevin Tapperson
         */
