...
public class Type2Message extends NtlmMessage {
        ...
        try {
            ...
            if ((flags & (NTLMSSP_TARGET_TYPE_DOMAIN | NTLMSSP_TARGET_TYPE_SERVER | NTLMSSP_TARGET_TYPE_SHARE)) != 0) {
                if (targetName != null && targetName.length() != 0) {
                    target = (flags & NTLMSSP_NEGOTIATE_UNICODE) != 0 ? targetName
                            .getBytes("UnicodeLittleUnmarked") : targetName.toUpperCase().getBytes(
                            getOEMEncoding());
                } else {
    ...
    public static int getDefaultFlags(Type1Message type1) {
        ...
        flags |= ((type1Flags & NTLMSSP_NEGOTIATE_UNICODE) != 0) ? NTLMSSP_NEGOTIATE_UNICODE
                : NTLMSSP_NEGOTIATE_OEM;
    ...
    private void parse(byte[] material) throws IOException {
        ...
        if (bytes.length != 0) {
            target = new String(bytes,
                    ((flags & NTLMSSP_NEGOTIATE_UNICODE) != 0) ? "UnicodeLittleUnmarked"
                            : getOEMEncoding());
        }
        setTarget(target);
        for (int i = 24; i < 32; i++) {
