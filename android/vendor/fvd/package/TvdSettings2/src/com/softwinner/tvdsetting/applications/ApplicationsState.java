...
public class ApplicationsState {
    ...
    static ApplicationsState getInstance(Application app) {
        synchronized (sLock) {
            Locale tmpLoc = app.getResources().getConfiguration().locale;
            if (sInstance == null || !locale.getCountry().equals(tmpLoc.getCountry())) {
        ...
        ArrayList<AppEntry> rebuild(AppFilter filter, Comparator<AppEntry> comparator) {
            synchronized (mRebuildSync) {
                ...
                while (mRebuildResult == null) {
                    ...
                    //check 8 times over,take too much time,just give up
                    if(tryTime > 8){
                    	if (DEBUG) Log.v(TAG, "wait MSG_REBUILD_LIST time out! wait" + 250*tryTime);
                    	break;
                    }
                    tryTime++;
                    try {
                        mRebuildSync.wait(250/*waitend - now*/);//just 250ms once,try 8 time
                    } catch (InterruptedException e) {
                    }
                }
            ...
            synchronized (mRebuildSync) {
                if (!mRebuildRequested) {
                    ...
                    if (!mRebuildAsync) {
                        ...
                        if (DEBUG) Log.i(TAG, "mRebuildSync.notifyAll(),mRebuildResult=" + mRebuildResult);
                    } else {
                    ...
                    if (entry != null) {
                        synchronized (entry) {
                            ...
                            if (entry.size != newSize ||
                                ...
                                if (true) Log.i(TAG, "Set size of " + entry.label + " " + entry
                                        + ": " + entry.size + ": " + entry.codeSize + ": " + entry.dataSize + ": " + entry.cacheSize);
                                sizeChanged = true;
                            }
                        }
                        if (sizeChanged) {
