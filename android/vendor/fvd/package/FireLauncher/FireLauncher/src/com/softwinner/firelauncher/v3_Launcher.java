...
public class v3_Launcher extends Activity implements onJsonLoaderFinishListener,v3_SectionParent.onTopInterface{
	...
	Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			...
			case LOAD_WEATHER_INFO:
                /*
				if(!isLoaded) {
					if(Utils.isNetworkConnected(v3_Launcher.this)) {
						String myCode = null;
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		if (isIgnoreKeyCode(keyCode, keyAction, current)) {
			...
			lastKeyCode = keyCode;
			if(false){ //(tabWidget.hasFocus() && keyCode == KeyEvent.KEYCODE_DPAD_UP	&& keyAction == KeyEvent.ACTION_DOWN) {
	...
	private void setupViews() {
		...
		LauncherView = inflater.inflate(R.layout.v3_launcher, null);
		...
		viewPager = (launcherViewPager) findViewById(R.id.view_pager);
		dotGroup = (LinearLayout)findViewById(R.id.dot_group);
		viewPager.setUp(this,dotGroup);
...
//					}
					...
					String code = WeatherUtil.getInstance(v3_Launcher.this).getCityCode(cityName);
					if(code == null) return; 
					cityCode = code;
	...
	private void USBChange(boolean state) {
		LayoutInflater.from(v3_Launcher.this).inflate(R.layout.launcher_dialog_layout, null);
...
//	                	}
	...
	private void refreshWeacherUI() {
		...
		if(ids[0] == 0 && ids[1] == 0){
			...
			isLoaded = false;
			mHandler.sendEmptyMessageDelayed(LOAD_WEATHER_INFO, INTERVAL);
			info = "";
			temp = "";
