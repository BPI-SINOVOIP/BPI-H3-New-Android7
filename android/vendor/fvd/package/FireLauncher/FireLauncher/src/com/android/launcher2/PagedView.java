...
import com.softwinner.firelauncher.Launcher;
...
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    ...
    protected static final int PAGE_SNAP_ANIMATION_DURATION = 350;
    ...
    private static final int MIN_SNAP_VELOCITY = 3000;
    ...
    private boolean mHasScrollIndicator = false;
...
//            mIsPageMoving = false;
    ...
    protected boolean computeScrollHelper() {
        if (mScroller.computeScrollOffset()) {
        	...
        	if(DEBUG) Log.d(TAG, "computeScrollHelper scrollTo");
        	...
        	if(DEBUG) Log.d(TAG, "update page: current "+mCurrentPage + ", next "+mNextPage);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (childCount > 0) {
            ...
            mMaxScrollX = getChildOffset(index) - getRelativeChildOffset(index) + (getPageAt(index).getMeasuredWidth()-getMeasuredWidth());
        } else {
            mMaxScrollX = 0;
        }
    }
    ...
    protected void screenScrolled(int screenCenter) {
        ...
        if (mFadeInAdjacentScreens && !isInOverscroll) {
            for (int i = 0; i < getChildCount(); i++) {
                ...
                if (child != null) {
                	// 比较每个child中间位置和此时屏幕中间位置的差值，差值再和每个child的宽相除得到一个相对值，|相对值| > 1 则透明显示此child
    ...
    protected int getRelativeChildOffset(int index) {
        if (mChildRelativeOffsets != null && mChildRelativeOffsets[index] != -1) {
            ...
            int offset = 0;
            if(getMeasuredWidth() - padding - getChildWidth(index) > 0)
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        if(DEBUG) Log.d(TAG, "mOverScrollX "+mOverScrollX + " , halfScreenSize "+halfScreenSize);
    ...
    //被requestFocus(int, android.graphics.Rect)调用，寻找一个child并调用requestFocus
    @Override
...
//        }
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_SCROLLING) {
                ...
                if (((isSignificantMove && !isDeltaXLeft && !isFling) ||
                	...
                	int childX = getChildOffset(mCurrentPage);
                	int childW = getPageAt(mCurrentPage).getMeasuredWidth();
                	if(scrollX>=childX && scrollX+W<=childX+childW) {
    ...
    protected void snapToPage(int whichPage, int delta, int duration) {
        ...
        Log.w(TAG, "snapToPage pageBeginMoving "+ mCurrentPage + "-->"+ whichPage);
        pageBeginMoving();
