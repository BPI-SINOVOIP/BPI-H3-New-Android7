 ...
 * Revision;
 *
 * 11/27/02 - first revision. 11/01/03 - Terje Bakken - fixed missing escaping
 * of reserved XML characters 11/19/04 - Theo Beisch <theo.beisch@gmx.de> -
 * Added "&" and "\"" "\\" to toXMLString(). 11/19/04 - Theo Beisch
 * <theo.beisch@gmx.de> - Changed XML::output() to use short notation when the
 * tag value is null. 12/02/04 - Brian Owens <brian@b-owens.com> - Fixed
 * toXMLString() to convert from "'" to "&apos;" instead of "\". 11/07/05 -
 * Changed toString() to return as utf-8 string. 02/08/08 - Added addValue().
 *
 ******************************************************************/
...
public class Node {
    ...
    /**
     * Inovoke {@link #getIndentLevelString(int, String)} with
     * <code>"   "</code> as String
     *
    ...
    public void outputAttributes(PrintWriter ps) {
        ...
        if (getName().startsWith("dlna:")) {
            ps.print(" xmlns:dlna=\"urn:schemas-dlna-org:device-1-0\"");
        } else if (getName().equals("root")) {
            ps.print(" xmlns=\"urn:schemas-upnp-org:device-1-0\"");
        } else if (getName().equals("scpd")) {
            ps.print(" xmlns=\"urn:schemas-upnp-org:service-1-0\"");
        }
    }
    ...
    public void output(PrintWriter ps, int indentLevel, boolean hasChildNode) {
        ...
        if (!(hasNodes() && hasChildNode)) {
            ...
            if (value == null || value.length() == 0) {
                // Not using the short notation <node /> because it cause
