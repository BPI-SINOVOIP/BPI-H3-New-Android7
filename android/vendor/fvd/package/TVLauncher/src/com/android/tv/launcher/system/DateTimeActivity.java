...
import com.android.tv.launcher.widget.picker.PickerConstant;
...
public class DateTimeActivity extends BaseSettingsActivity implements ActionAdapter.Listener {
    ...
    private Calendar mDummyDate; //用来显示示例的时间的
    ...
    public void onActionClicked(Action action) {
        ...
        switch (behavior) {
            case ON:
                if (mState == ActionType.TIME_CHOOSE_FORMAT) {//"使用24小时格式"
                    setTime24Hour(true);
                    updateTimeAndDateStrings();
                } else if (mState == ActionType.AUTO_DATE_TIME) {//"自动确定日期和时间"
