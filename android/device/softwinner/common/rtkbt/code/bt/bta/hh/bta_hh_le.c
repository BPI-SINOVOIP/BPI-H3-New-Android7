...
static void bta_hh_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC *p_data)
{
...
#ifdef BLUETOOTH_RTK
    tBTM_SEC_DEV_REC    *p_dev_rec = btm_find_dev (p_data->enc_cmpl.remote_bda);
#endif
#if BTA_HH_DEBUG
    ...
    switch (event)
    {
        ...
        case BTA_GATTC_ENC_CMPL_CB_EVT: /* 17 */
            ...
            if (p_dev_cb) {
#ifdef BLUETOOTH_RTK
                if((p_dev_rec->sec_flags & (BTM_SEC_LE_ENCRYPTED | BTM_SEC_LE_AUTHENTICATED)) != 0)
                {
                    if(p_dev_cb->status == BTA_HH_ERR_SEC && p_dev_cb->reason == BTM_ERR_PROCESSING) {
                        p_dev_cb->status = BTA_HH_OK;
                        p_dev_cb->reason = BTM_SUCCESS;
                    }
