...
#define BTE_DID_CONF_FILE "bt_did-rtk.conf"
#else  // !defined(OS_GENERIC)
#define BTE_DID_CONF_FILE "/etc/bluetooth/bt_did-rtk.conf"
...
/*
   set sleep info: use broadcom or realtek sleep driver
   bcm: broadcom
   rtl: realtek
*/
static void btif_set_sleep_info()
{
    aw_get_wifi_module_info();
    if (module_info.vendor_id == 0)
    {
        if (osi_property_set(PERSIST_BTSLEEP_PROPERTY, "bcm") < 0)
            BTIF_TRACE_ERROR("Failed to set bt module info in prop %s", PERSIST_BTSLEEP_PROPERTY);
    }
    else if (module_info.vendor_id == 1)
    {
        if (osi_property_set(PERSIST_BTSLEEP_PROPERTY, "rtl") < 0)
            BTIF_TRACE_ERROR("Failed to set bt module info in prop %s", PERSIST_BTSLEEP_PROPERTY);
    }
}

static void btif_set_rtk_uart_usb()
{
    aw_get_wifi_module_info();
    if (strcmp(module_info.mod_name, "rtl8723bu") == 0)
    {
        if (osi_property_set("persist.service.bdroid.usb", "1") < 0)
            BTIF_TRACE_ERROR("Failed to set bt module info in prop %s", "persist.service.bdroid.rtkbtusb");
    } else {
        if (osi_property_set("persist.service.bdroid.uart", "1") < 0)
            BTIF_TRACE_ERROR("Failed to set bt module info in prop %s", "persist.service.bdroid.rtkbtuart");
    }
}
