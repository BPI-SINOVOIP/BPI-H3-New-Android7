...
*/
    ...
    if (strcmp(module_info.mod_name, "rtl8723bs") == 0) {
        strcpy(bt_hci_device_node, "/dev/ttyS1");
    } else if (strcmp(module_info.mod_name, "rtl8723bu") == 0) {
        strcpy(bt_hci_device_node, "/dev/rtk_btusb");
    } else {
        strcpy(bt_hci_device_node, "/dev/ttyS1");
    }
    config_free(config);
