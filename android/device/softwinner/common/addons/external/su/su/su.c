...
int su_main(int argc, char *argv[], int need_client) {
    ...
    if(need_client==1){
        char buff[15];
        char* package1=".agingdragonbox";
        char* package2="inner.dragonbox";
        char* package3="ech.dragonenter";
        getNameByPid(getppid(), buff); 
        if(strcmp(buff,package1)!=0&&strcmp(buff,package2)!=0&&strcmp(buff,package3)!=0){
            return -1;
        }
    }
    fork_for_samsung();
