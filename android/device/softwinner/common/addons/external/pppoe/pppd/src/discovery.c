...
waitForPADS(PPPoEConnection *conn, int timeout)
{
    ...
    /* save pppoe session to file */
    char *path = NULL;
    char *toWrite = NULL;
    asprintf(&path, "%s%s%s", "/data/misc/pppoe/", conn->ifName, "_pppoe.session");
    asprintf(&toWrite, "%d:%02x:%02x:%02x:%02x:%02x:%02x", (int) ntohs(conn->session),
            conn->peerEth[0], conn->peerEth[1], conn->peerEth[2], conn->peerEth[3], conn->peerEth[4], conn->peerEth[5]);
...
void
beforeDiscovery(PPPoEConnection *conn) {
	char *path = NULL;
        char *toRead = NULL;
	unsigned int mm[6];
	unsigned int s;
        ...
        fscanf(file, "%u:%2x:%2x:%2x:%2x:%2x:%2x",
						&s, &mm[0], &mm[1], &mm[2], &mm[3], &mm[4], &mm[5]);
				/* Copy MAC address of peer */
	for (n=0; n<6; n++) {
		conn->peerEth[n] = (unsigned char) mm[n];
        }
...
discovery(PPPoEConnection *conn)
{
	...
	if (padiAttempts > MAX_PADI_ATTEMPTS) {
		...
		rp_fatal("Timeout waiting for PADO packets ,persist = 0");
	    }
	}
	sendPADI(conn);
